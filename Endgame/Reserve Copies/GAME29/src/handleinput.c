...
void spy() {/*
	...
	RoomDoorRect.x = 9 * BG_WIDTH / 10 - DOOR_WIDTH;
	backgroundRect.x = 0;
	//playerRect.x = PLAYER_WIDTH;
	vitalikRect.x = SCREEN_WIDTH - 200;
	...
	while (quit)
	{
		...
		temp = backgroundRect.x;
		if (currentKeyStates[SDL_SCANCODE_LEFT]) {
			if (backgroundRect.x < 0 && playerRect.x == SCREEN_WIDTH / 2 - PLAYER_WIDTH / 2 && playerRect.x == SCREEN_WIDTH / 2 - PLAYER_WIDTH / 2) {
				backgroundRect.x += PLAYER_SPEED - 5 * PLAYER_SPEED / 6 * crawl;
				RoomDoorRect.x += PLAYER_SPEED - 5 * PLAYER_SPEED / 6 * crawl;
			}
			else if (playerRect.x > 0) {
				playerRect.x -= PLAYER_SPEED - 5 * PLAYER_SPEED / 6 * crawl;
			}
			lastMoveLeft = 1;
			lastMoveRight = 0;
