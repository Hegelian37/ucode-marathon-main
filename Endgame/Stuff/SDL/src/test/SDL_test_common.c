...
int SDLTest_CommonArg(SDLTest_CommonState *state, int index)
{
    ...
    if (SDL_strcasecmp(argv[index], "--trackmem") == 0) {
        /* Already handled in SDLTest_CommonCreateState(void) */
        return 1;
    }
...
SDL_bool SDLTest_CommonInit(SDLTest_CommonState *state)
{
    ...
    if (state->flags & SDL_INIT_VIDEO) {
        ...
        if (state->verbose & VERBOSE_RENDER) {
            n = SDL_GetNumRenderDrivers(void);
            if (n == 0) {
        ...
        state->displayID = SDL_GetPrimaryDisplay(void);
...
{
    switch (SDL_GetSystemTheme(void)) {
#define CASE(X)               \
        ...
        if (file) {
            ...
            if (image) {
                if (SDL_RWread(file, image, length) != length) {
                    SDL_Log("Couldn't read %s: %s\n", SCREENSHOT_FILE, SDL_GetError(void));
                    SDL_free(image);
                    image = NULL;
                }
            }
...
int SDLTest_CommonEventMainCallbacks(SDLTest_CommonState *state, const SDL_Event *event)
{
    ...
    case SDL_EVENT_WINDOW_CLOSE_REQUESTED:
    {
        ...
        if (window) {
            ...
            SDLTest_CleanupTextDrawing(void);
            for (i = 0; i < state->num_windows; ++i) {
    ...
    case SDL_EVENT_KEY_DOWN:
    {
        ...
        case SDLK_v:
            if (withAlt) {
                ...
                char *text = SDL_GetPrimarySelectionText(void);
                if (*text) {
                ...
                if (withShift) {
                    ...
                    SDLTest_PasteScreenShot(void);
                } else {
                    /* Ctrl-V paste awesome text! */
                    char *text = SDL_GetClipboardText(void);
                    if (*text) {
                        SDL_Log("Clipboard: %s\n", text);
                    } else {
        ...
        case SDLK_m:
            ...
            if (withShift) {
                SDL_Window *current_win = SDL_GetKeyboardFocus(void);
                if (current_win) {
        ...
        case SDLK_r:
            if (withControl) {
                ...
                SDL_SetRelativeMouseMode(!SDL_GetRelativeMouseMode(void));
            }
            break;
        case SDLK_t:
            if (withControl) {
...
void SDLTest_CommonDrawWindowInfo(SDL_Renderer *renderer, SDL_Window *window, float *usedHeight)
{
    ...
    (void)SDL_snprintf(text, sizeof(text), "SDL_GetCurrentVideoDriver: %s", SDL_GetCurrentVideoDriver(void));
    ...
    SDLTest_PrintModState(text, sizeof(text), SDL_GetModState(void));
