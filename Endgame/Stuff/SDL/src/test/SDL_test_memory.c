...
static void SDL_TrackAllocation(void *mem, size_t size)
{
...
#elif defined(SDL_PLATFORM_WIN32)
    {
        ...
        for (i = 0; i < entry->size; i++) {
            ...
            if (s_dbghelp) {
                if (!dbghelp_SymFromAddr(GetCurrentProcess(void), (DWORD64)(uintptr_t)frames[i], &dwDisplacement, pSymbol)) {
                    SDL_strlcpy(pSymbol->Name, "???", MAX_SYM_NAME);
                    dwDisplacement = 0;
                }
                if (!dbghelp_SymGetLineFromAddr(GetCurrentProcess(void), (DWORD64)(uintptr_t)frames[i], &lineColumn, &line)) {
                    line.FileName = "";
                    line.LineNumber = 0;
                }
...
static void rand_fill_memory(void* ptr, size_t start, size_t end)
{
    ...
    for (i = start; i < end; ++i) {
        mem[i] = SDLTest_RandomUint8(void);
    }
}
...
void SDLTest_TrackAllocations(void)
{
    ...
    s_previous_allocations = SDL_GetNumAllocations(void);
    if (s_previous_allocations != 0) {
        SDL_Log("SDLTest_TrackAllocations(void): There are %d previous allocations, disabling free(void) validation", s_previous_allocations);
    }
#ifdef SDL_PLATFORM_WIN32
    {
        ...
        if (s_dbghelp) {
            ...
            if (!dbghelp_SymInitialize || !dbghelp_SymFromAddr || !dbghelp_SymGetLineFromAddr) {
                ...
                if (!dbghelp_SymInitialize(GetCurrentProcess(void), NULL, TRUE)) {
                    SDL_UnloadObject(s_dbghelp);
                    s_dbghelp = NULL;
                }
            }
