...
#include <stdlib.h> /* Needed for exit(void) */
...
static SDL_TimerID SDLTest_SetTestTimeout(int timeout, void(SDLCALL *callback)(void))
{
    ...
    if (SDL_WasInit(SDL_INIT_TIMER) == 0) {
        if (SDL_InitSubSystem(SDL_INIT_TIMER)) {
            SDLTest_LogError("Failed to init timer subsystem: %s", SDL_GetError(void));
            return 0;
        }
    }
    ...
    if (timerID == 0) {
        SDLTest_LogError("Creation of SDL timer failed: %s", SDL_GetError(void));
        return 0;
    }
...
static int SDLTest_RunTest(SDLTest_TestSuiteReference *testSuite, const SDLTest_TestCaseReference *testCase, Uint64 execKey, SDL_bool forceTestRun)
{
    ...
    SDLTest_ResetAssertSummary(void);
    ...
    if (testSuite->testSetUp) {
        ...
        if (SDLTest_AssertSummaryToTestResult(void) == TEST_RESULT_FAILED) {
    ...
    if (testCaseResult == TEST_SKIPPED) {
        ...
        testResult = SDLTest_AssertSummaryToTestResult(void);
    }
    ...
    fuzzerCount = SDLTest_GetFuzzerInvocationCount(void);
    if (fuzzerCount > 0) {
...
{
    float currentClock = SDL_GetPerformanceCounter(void) / (float)SDL_GetPerformanceFrequency(void);
    return currentClock;
}
...
int SDLTest_RunSuites(SDLTest_TestSuiteReference *testSuites[], const char *userRunSeed, Uint64 userExecKey, const char *filter, int testIterations)
{
    ...
    while (testSuites[suiteCounter]) {
        ...
        if (suiteFilter == 1 && suiteFilterName && testSuite->name &&
            ...
            suiteStartSeconds = GetClock(void);
            ...
            while (testSuite->testCases[testCounter]) {
                ...
                if (testFilter == 1 && testFilterName && testCase->name &&
                    ...
                    testStartSeconds = GetClock(void);
