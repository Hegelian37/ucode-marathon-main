...
/* Determines how often we wake to call SDL_PumpEvents(void) in SDL_WaitEventTimeout_Device(void) */
...
/* this is to make (void)SDL_snprintf(void) calls cleaner. */
...
int SDL_StartEventLoop(void)
{
    ...
    if (!SDL_EventQ.lock) {
        SDL_EventQ.lock = SDL_CreateMutex(void);
    ...
    if (SDL_event_watchers_lock == NULL) {
        SDL_event_watchers_lock = SDL_CreateMutex(void);
        if (SDL_event_watchers_lock == NULL) {
            SDL_UnlockMutex(SDL_EventQ.lock);
            return -1;
        }
    ...
    if (SDL_event_memory_lock == NULL) {
        SDL_event_memory_lock = SDL_CreateMutex(void);
        if (SDL_event_memory_lock == NULL) {
            SDL_UnlockMutex(SDL_EventQ.lock);
            return -1;
        }
...
static void SDL_PumpEventsInternal(SDL_bool push_sentinel)
{
    ...
    SDL_UpdateAudio(void);
#endif

#ifndef SDL_CAMERA_DISABLED
    SDL_UpdateCamera(void);
#endif
    ...
    SDL_SendPendingSignalEvents(void); /* in case we had a signal handler fire, etc. */
...
static int SDL_WaitEventTimeout_Device(SDL_VideoDevice *_this, SDL_Window *wakeup_window, SDL_Event *event, Uint64 start, Sint64 timeoutNS)
{
    ...
    SDL_bool need_periodic_poll = SDL_events_need_periodic_poll(void);
    ...
    for (;;) {
        ...
        /* Pump events on entry and each time we wake to ensure:
           ...
           c) Periodic processing that takes place in some platform PumpEvents(void) functions happens
           d) Signals received in WaitEventTimeout(void) are turned into SDL events
        */
        ...
        if (timeoutNS > 0) {
            Sint64 elapsed = SDL_GetTicksNS(void) - start;
...
#ifndef SDL_JOYSTICK_DISABLED
    need_polling = SDL_WasInit(SDL_INIT_JOYSTICK) && SDL_update_joysticks && SDL_JoysticksOpened(void);
#endif

#ifndef SDL_SENSOR_DISABLED
    need_polling = need_polling ||
                   (SDL_WasInit(SDL_INIT_SENSOR) && SDL_update_sensors && SDL_SensorsOpened(void));
#endif
...
SDL_bool SDL_WaitEventTimeoutNS(SDL_Event *event, Sint64 timeoutNS)
{
    ...
    if (_this && _this->WaitEventTimeout && _this->SendWakeupEvent && !SDL_events_need_polling(void)) {
    ...
    for (;;) {
        ...
        if (timeoutNS > 0) {
            Uint64 now = SDL_GetTicksNS(void);
            if (now >= expiration) {
...
int SDL_PushEvent(SDL_Event *event)
{
    if (!event->common.timestamp) {
        event->common.timestamp = SDL_GetTicksNS(void);
    }
