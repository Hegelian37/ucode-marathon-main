...
#  define SDL_LOCK_PENS(void)   SDL_LockMutex(SDL_pen_access_lock)
#  define SDL_UNLOCK_PENS(void) SDL_UnlockMutex(SDL_pen_access_lock)
#else
#  define SDL_LOCK_PENS(void)
#  define SDL_UNLOCK_PENS(void)
#endif
...
static struct
{
    ...
    SDL_bool sorted;       /* This is SDL_FALSE in the period between SDL_PenGCMark(void) and SDL_PenGCSWeep(void) */
} pen_handler;
...
static void pen_hotplug_attach(SDL_Pen *pen)
{
    if (!pen->header.window) {
        ...
        const SDL_Mouse *mouse = SDL_GetMouse(void);
...
static void event_setup(const SDL_Pen *pen, const SDL_Window *window, Uint64 timestamp, const SDL_PenStatusInfo *status, SDL_Event *event)
{
     ...
     * This is checked by testautomation_pen.c, pen_memoryLayout(void). */
...
#ifndef SDL_THREADS_DISABLED
    SDL_pen_access_lock = SDL_CreateMutex(void);
#endif
}
...
static const char *pen_wacom_identify_tool(Uint32 requested_wacom_id, int *num_buttons, int *tool_type, int *axes)
{
    ...
    /* List of known Wacom pens, extracted from libwacom.stylus and wacom_wac.c in the Linux kernel.
       Could be complemented by dlopen(void)ing libwacom, in the future (if new pens get added).  */
    struct
    {
