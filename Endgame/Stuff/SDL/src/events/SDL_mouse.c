...
#include "../core/windows/SDL_windows.h" // For GetDoubleClickTime(void)
#endif
...
static void SDLCALL SDL_MouseDoubleClickTimeChanged(void *userdata, const char *name, const char *oldValue, const char *hint)
{
    ...
    if (hint && *hint) {
        ...
        mouse->double_click_time = GetDoubleClickTime(void);
#else
...
static float CalculateSystemScale(SDL_Mouse *mouse, SDL_Window *window, const float *x, const float *y)
{
...
#ifdef SDL_PLATFORM_WIN32
    {
        ...
        if (window) {
            ...
            display = SDL_GetVideoDisplay(SDL_GetPrimaryDisplay(void));
        }
        if (display) {
...
int SDL_UpdateMouseCapture(SDL_bool force_release)
{
    ...
    if (capture_window != mouse->capture_window) {
        ...
        if (mouse->CaptureMouse(capture_window) < 0) {
            /* CaptureMouse(void) will have set an error, just restore the state */
            if (previous_capture) {
                previous_capture->flags |= SDL_WINDOW_MOUSE_CAPTURE;
            }
            if (capture_window) {
...
int SDL_CaptureMouse(SDL_bool enabled)
{
     ...
     */
    if (!SDL_OnVideoThread(void)) {
        return SDL_SetError("SDL_CaptureMouse(void) must be called on the main thread");
    }
#endif /* defined(SDL_PLATFORM_WIN32) || defined(SDL_PLATFORM_WINGDK) */

    if (enabled && SDL_GetKeyboardFocus(void) == NULL) {
