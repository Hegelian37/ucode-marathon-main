...
#define SDL_PEN_DEBUG_UNKNOWN_WACOM 0  /* Pretend that any attached Wacom device is of an unknown make \
                                          affects: SDL_PenModifyFromWacomID(void) */
...
#define SDL_PEN_FLAG_NEW      (1ul << 29) /* Pen was registered in most recent call to SDL_PenRegisterBegin(void) */
#define SDL_PEN_FLAG_DETACHED (1ul << 30) /* Detached (not re-registered before last SDL_PenGCSweep(void)) */
#define SDL_PEN_FLAG_STALE    (1ul << 31) /* Not re-registered since last SDL_PenGCMark(void) */
...
typedef struct SDL_Pen
{
    ...
    /* Backend: SHOULD initialise this block when pen is first registered if it can
       Otherwise: Set to sane default values during SDL_PenModifyEnd(void) */
 ...
 *
 * The pen pointer is only valid until the next call to SDL_PenModifyEnd(void) or SDL_PenGCSweep(void)
 *
 ...
 *
 * Acquires the pen mutex, which is held until the next call to SDL_PenModifyEnd(void) .
 *
 ...
 *
 * Usage:
 * - SDL_PenModifyStart(void)
 * - update pen object, in any order:
 *     - SDL_PenModifyAddCapabilities(void)
 ...
 * \returns SDL_Pen pointer; only valid until the call to SDL_PenModifyEnd(void)
 */
extern SDL_Pen *SDL_PenModifyBegin(Uint32 instance_id);

/**
 * (Only for backend driver) Add capabilities to a pen (cf. SDL_PenModifyBegin(void)).
 *
 * Adds capabilities to a pen obtained via SDL_PenModifyBegin(void).  Can be called more than once.
 *
 ...
 *
 * This function does NOT call SDL_PenModifyAddCapabilities(void) ifself, since some backends may
 ...
 *
 * Releases the pen mutex acquired by SDL_PenModifyBegin(void) .
 *
 ...
 *
 * Must not be called while the pen mutex is held (by SDL_PenModifyBegin(void) ).
 *
 * SDL_PenGCMark(void) / SDL_PenGCSweep(void) provide a simple mechanism for
 * detaching all known pens that are not discoverable.  This allows
 * backends to use the same code for pen discovery and for
 * hotplugging:
 *
 *  - SDL_PenGCMark(void) and start backend-specific discovery
 *  - for each discovered pen: SDL_PenModifyBegin(void) + SDL_PenModifyEnd(void) (this will retain existing state)
 *  - SDL_PenGCSweep(void)  (will now detach all pens that were not re-registered).
 */
extern void SDL_PenGCMark(void);

/**
 * (Only for backend driver) Detach pens that haven't been reported attached since the last call to SDL_PenGCMark(void).
 *
 * Must not be called while the pen mutex is held (by SDL_PenModifyBegin(void) ).
 *
 * See SDL_PenGCMark(void) for details.
 *
 ...
 *
 * \param timestamp Event timestamp in nanoseconds, or 0 to ask SDL to use SDL_GetTicksNS(void) .
 ...
 *        SDL_GetTicksNS(void) . In particular, SDL_GetTicksNS(void) reports nanoseconds since the start
 ...
 *
 * \param timestamp Event timestamp in nanoseconds, or 0 to ask SDL to use SDL_GetTicksNS(void) .
 *        See SDL_SendPenMotion(void) for a discussion about how to handle timestamps.
 * \param instance_id Pen
