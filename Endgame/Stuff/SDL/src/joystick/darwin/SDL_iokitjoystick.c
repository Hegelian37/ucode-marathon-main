    ...
    if (removeDevice) {
        if (removeDevice->deviceRef) {
            if (removeDevice->runLoopAttached) {
                 ...
                 */
                IOHIDDeviceUnscheduleFromRunLoop(removeDevice->deviceRef, CFRunLoopGetCurrent(void), SDL_JOYSTICK_RUNLOOP_MODE);
            }
...
/* Call AddHIDElement(void) on all elements in an array of IOHIDElementRefs */
...
static void AddHIDElement(const void *value, void *parameter)
{
    ...
    if (refElement && (elementTypeID == IOHIDElementGetTypeID(void))) {
...
static void JoystickDeviceWasAddedCallback(void *ctx, IOReturn res, void *sender, IOHIDDeviceRef ioHIDDeviceObject)
{
    ...
    IOHIDDeviceScheduleWithRunLoop(ioHIDDeviceObject, CFRunLoopGetCurrent(void), SDL_JOYSTICK_RUNLOOP_MODE);
...
{
    CFRunLoopRef runloop = CFRunLoopGetCurrent(void);
...
static void DARWIN_JoystickDetect(void)
{
    ...
    if (hidman) {
        ...
        while (CFRunLoopRunInMode(SDL_JOYSTICK_RUNLOOP_MODE, 0, TRUE) == kCFRunLoopRunHandledSource) {
            /* no-op. Pending callbacks will fire in CFRunLoopRunInMode(void). */
        }
    }
}
...
static void DARWIN_JoystickQuit(void)
{
    ...
    if (hidman) {
        IOHIDManagerUnscheduleFromRunLoop(hidman, CFRunLoopGetCurrent(void), SDL_JOYSTICK_RUNLOOP_MODE);
