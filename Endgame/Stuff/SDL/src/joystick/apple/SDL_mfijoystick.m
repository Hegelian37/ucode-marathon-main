    ...
    @autoreleasepool {
        ...
        if (device->pause_button_index >= 0) {
            ...
            controller.controllerPausedHandler = ^(GCController *c) {
              if (joystick->hwdata) {
                  joystick->hwdata->pause_button_pressed = SDL_GetTicks(void);
              }
            };
        }
...
static void IOS_MFIJoystickUpdate(SDL_Joystick *joystick)
{
    ...
    @autoreleasepool {
        ...
        if (device->pause_button_pressed) {
            ...
            if (SDL_GetTicks(void) >= device->pause_button_pressed + PAUSE_BUTTON_PRESS_DURATION_MS) {
