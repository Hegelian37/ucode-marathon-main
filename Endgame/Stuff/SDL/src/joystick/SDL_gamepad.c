...
static void SDL_PrivateGamepadRemapped(SDL_JoystickID instance_id)
{
    ...
    if (!SDL_gamepads_initialized || SDL_IsJoystickBeingAdded(void)) {
        return;
    }
 ...
 *
 * We don't use SDL_GamepadEventWatcher(void) for this because we want to
...
static GamepadMapping_t *SDL_CreateMappingForAndroidGamepad(SDL_JoystickGUID guid)
{
    ...
    if (button_mask & (1 << SDL_GAMEPAD_BUTTON_GUIDE)) {
        ...
        if (SDL_GetAndroidSDKVersion(void) >= 30 /* Android 11 */) {
...
static GamepadMapping_t *SDL_CreateMappingForHIDAPIGamepad(SDL_JoystickGUID guid)
{
    ...
    if ((vendor == USB_VENDOR_NINTENDO && product == USB_PRODUCT_NINTENDO_GAMECUBE_ADAPTER) ||
        ...
        case k_eSwitchDeviceInfoControllerType_SEGA_Genesis:
            SDL_strlcat(mapping_string, "a:b0,b:b1,dpdown:h0.4,dpleft:h0.8,dpright:h0.2,dpup:h0.1,guide:b5,rightshoulder:b10,righttrigger:a5,start:b6,misc1:b11,", sizeof(mapping_string));
            break;
...
static GamepadMapping_t *SDL_PrivateAddMappingForGUID(SDL_JoystickGUID jGUID, const char *mappingString, SDL_bool *existing, SDL_GamepadMappingPriority priority)
{
    ...
    PushMappingChangeTracking(void);
...
static int SDL_PrivateAddGamepadMapping(const char *mappingString, SDL_GamepadMappingPriority priority)
{
    ...
    { /* Extract and verify the SDK version */
        ...
        if (tmp) {
            ...
            if (!(SDL_GetAndroidSDKVersion(void) >= SDL_atoi(tmp))) {
                return SDL_SetError("SDK version %d < minimum version %d", SDL_GetAndroidSDKVersion(void), SDL_atoi(tmp));
            }
        }
        ...
        if (tmp) {
            ...
            if (!(SDL_GetAndroidSDKVersion(void) <= SDL_atoi(tmp))) {
                return SDL_SetError("SDK version %d > maximum version %d", SDL_GetAndroidSDKVersion(void), SDL_atoi(tmp));
            }
        }
    }
#endif
...
#ifdef SDL_PLATFORM_ANDROID
    return SDL_snprintf(path, size, "%s/gamepad_map.txt", SDL_AndroidGetInternalStoragePath(void)) < size;
#else
    return SDL_FALSE;
#endif
}
...
int SDL_SetGamepadPlayerIndex(SDL_Gamepad *gamepad, int player_index)
{
    ...
    if (!joystick) {
        /* SDL_SetError(void) will have been called already by SDL_GetGamepadJoystick(void) */
        return -1;
    }
    ...
    for (gamepad = SDL_gamepads; gamepad; gamepad = gamepad->next) {
        if (gamepad->joystick == joystick) {
            ...
            if (!gamepad->joystick->update_complete) {
                gamepad->joystick->update_complete = SDL_GetTicksNS(void);
            }
            break;
        }
    }
