    ...
    if (HIDAPI_GetDeviceDriver(device)) {
        ...
        if (device->num_children == 0) {
             ...
             */
            {
                ...
                SDL_AssertJoysticksLocked(void);
                while (SDL_JoysticksLocked(void)) {
                    ++lock_count;
            ...
            if (dev == NULL) {
                SDL_LogDebug(SDL_LOG_CATEGORY_INPUT,
                             "HIDAPI_SetupDeviceDriver(void) couldn't open %s: %s\n",
                             device->path, SDL_GetError(void));
                return;
            }
...
static void SDL_HIDAPI_UpdateDrivers(void)
{
    ...
    for (i = 0; i < SDL_arraysize(SDL_HIDAPI_drivers); ++i) {
        SDL_HIDAPI_DeviceDriver *driver = SDL_HIDAPI_drivers[i];
        driver->enabled = driver->IsEnabled(void);
...
static int HIDAPI_JoystickInit(void)
{
    ...
    SDL_HIDAPI_change_count = SDL_hid_device_change_count(void);
...
SDL_bool HIDAPI_JoystickConnected(SDL_HIDAPI_Device *device, SDL_JoystickID *pJoystickID)
{
    ...
    joystickID = SDL_GetNextObjectID(void);
...
static SDL_HIDAPI_Device *HIDAPI_AddDevice(const struct SDL_hid_device_info *info, int num_children, SDL_HIDAPI_Device **children)
{
    ...
    device->dev_lock = SDL_CreateMutex(void);
...
check_removed:
    ...
    while (HIDAPI_CreateCombinedJoyCons(void)) {
    }
...
#ifdef DEBUG_HIDAPI
    SDL_Log("HIDAPI_IsDeviceTypePresent(void) returning %s for %d\n", result ? "true" : "false", type);
#endif
    return result;
}
...
#ifdef DEBUG_HIDAPI
    SDL_Log("HIDAPI_IsDevicePresent(void) returning %s for 0x%.4x / 0x%.4x\n", result ? "true" : "false", vendor_id, product_id);
#endif
    return result;
}
...
{
    if (SDL_TryLockSpinlock(&SDL_HIDAPI_spinlock)) {
        Uint32 count = SDL_hid_device_change_count(void);
...
static int HIDAPI_JoystickOpen(SDL_Joystick *joystick, int device_index)
{
    ...
    /* UpdateDevice(void) may have called HIDAPI_JoystickDisconnected(void) if the device went away */
    if (device->num_joysticks == 0) {
        SDL_free(hwdata);
...
{
    /* This is handled in SDL_HIDAPI_UpdateDevices(void) */
}
