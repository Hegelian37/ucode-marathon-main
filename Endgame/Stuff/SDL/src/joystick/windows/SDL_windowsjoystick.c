...
{
#ifdef SDL_JOYSTICK_RAWINPUT
    RAWINPUT_UnregisterNotifications(void);
#endif
...
static int SDL_CreateDeviceNotification(SDL_DeviceNotificationData *data)
{
    ...
    data->coinitialized = WIN_CoInitialize(void);
...
static int SDLCALL SDL_JoystickThread(void *_data)
{
    ...
    while (s_bJoystickThreadQuit == SDL_FALSE) {
...
#else
        {
            ...
            if (SDL_XINPUT_Enabled(void) && XINPUTGETCAPABILITIES) {
...
{
    s_mutexJoyStickEnum = SDL_CreateMutex(void);
    if (!s_mutexJoyStickEnum) {
        return -1;
    }

    s_condJoystickThread = SDL_CreateCondition(void);
...
void WINDOWS_AddJoystickDevice(JoyStick_DeviceData *device)
{
    ...
    device->nInstanceID = SDL_GetNextObjectID(void);
...
{
    if (SDL_DINPUT_JoystickInit(void) < 0) {
        WINDOWS_JoystickQuit(void);
        return -1;
    }

    if (SDL_XINPUT_JoystickInit(void) < 0) {
        WINDOWS_JoystickQuit(void);
        return -1;
    }
    ...
    if (s_bJoystickThread) {
        if (SDL_StartJoystickThread(void) < 0) {
            return -1;
        }
    } else {
        if (SDL_CreateDeviceNotification(&s_notification_data) < 0) {
