...
{
    return SDL_SetError("%s(void) DirectX error 0x%8.8lx", function, code);
}
...
int SDL_DINPUT_JoystickInit(void)
{
    ...
    if (!instance) {
        ...
        dinput = NULL;
        return SDL_SetError("GetModuleHandle(void) failed with error code %lu.", GetLastError(void));
    }
