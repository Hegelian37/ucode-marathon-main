...
static void RAWINPUT_InitWindowsGamingInput(RAWINPUT_DeviceContext *ctx)
{
    ...
    if (!wgi_state.initialized) {
        ...
        if (FAILED(WIN_RoInitialize(void))) {
            return;
        }
...

        {
            ...
            if (WindowsCreateStringReferenceFunc && RoGetActivationFactoryFunc) {
                ...
                if (wgi_state.gamepad_statics) {
                    ...
                    if (!SUCCEEDED(hr)) {
                        SDL_SetError("add_GamepadAdded(void) failed: 0x%lx\n", hr);
                    }
                    ...
                    if (!SUCCEEDED(hr)) {
                        SDL_SetError("add_GamepadRemoved(void) failed: 0x%lx\n", hr);
                    }
                }
            }
        }
...
#define CHECK(expression)  \
    ...
    device->joystick_id = SDL_GetNextObjectID(void);
...
static int RAWINPUT_JoystickInit(void)
{
    ...
    if (!WIN_IsWindowsVistaOrGreater(void)) {
        /* According to bug 6400, this doesn't work on Windows XP */
        return -1;
    }

    if (WIN_LoadHIDDLL(void) < 0) {
        return -1;
    }

    SDL_RAWINPUT_inited = SDL_TRUE;

    RAWINPUT_DetectDevices(void);
    ...
    if (unmapped_guide_pressed) {
        if (guide_button_candidate.joystick && !guide_button_candidate.last_joystick) {
            ...
            if (ctx->guide_hack) {
                ...
                SDL_SendJoystickButton(SDL_GetTicksNS(void), guide_button_candidate.joystick, (Uint8)guide_button, SDL_PRESSED);
            }
        ...
        if (ctx->guide_hack) {
            ...
            SDL_SendJoystickButton(SDL_GetTicksNS(void), joystick, (Uint8)guide_button, SDL_RELEASED);
        }
...
static void RAWINPUT_JoystickDetect(void)
{
    ...
    if (remote_desktop != SDL_RAWINPUT_remote_desktop) {
        ...
        WINDOWS_RAWINPUTEnabledChanged(void);

        if (remote_desktop) {
            RAWINPUT_RemoveDevices(void);
...
static int RAWINPUT_JoystickOpen(SDL_Joystick *joystick, int device_index)
{
    ...
    if (device->is_xinput) {
        ...
        if (ctx->xinput_enabled && (WIN_LoadXInputDLL(void) < 0 || !XINPUTGETSTATE)) {
...
static int RAWINPUT_JoystickRumble(SDL_Joystick *joystick, Uint16 low_frequency_rumble, Uint16 high_frequency_rumble)
{
    ...
    if (!rumbled && ctx->xinput_correlated) {
        ...
        if (XINPUTSETSTATE(ctx->xinput_slot, &XVibration) == ERROR_SUCCESS) {
            ...
            return SDL_SetError("XInputSetState(void) failed");
        }
    }
#endif /* SDL_JOYSTICK_RAWINPUT_XINPUT */
...
#define SDL_SendJoystickAxis(timestamp, joystick, axis, value) \
    ...
    Uint64 timestamp = SDL_GetTicksNS(void);
    ...
    for (i = 0; i < nbuttons; ++i) {
        SDL_SendJoystickButton(timestamp, joystick, (Uint8)i, (button_mask & (1 << i)) ? SDL_PRESSED : SDL_RELEASED);
    }
...
#undef AddTriggerToMatchState
    ...
    if (ctx->is_xinput) {
        ...
        ctx->last_state_packet = SDL_GetTicks(void);
    }
#endif
}
...
static void RAWINPUT_UpdateOtherAPIs(SDL_Joystick *joystick)
{
    ...
    RAWINPUT_UpdateWindowsGamingInput(void);
    if (ctx->wgi_correlated &&
        ...
        if (RAWINPUT_WindowsGamingInputSlotMatches(&match_state_xinput, ctx->wgi_slot, xinput_correlated)) {
            ...
            /* Only un-correlate if this is consistent over multiple Update(void) calls - the timing of polling/event
            ...
            if (ctx->wgi_uncorrelate_count >= 5) {
                ...
                if (ctx->guide_hack) {
                    SDL_SendJoystickButton(0, joystick, (Uint8)guide_button, SDL_RELEASED);
                }
            }
        }
    }
    if (!ctx->wgi_correlated) {
        ...
        if (RAWINPUT_MissingWindowsGamingInputSlot(void)) {
    ...
    if (ctx->xinput_enabled) {
        ...
        if (!ctx->xinput_correlated) {
            Uint8 new_correlation_count = 0;
            if (RAWINPUT_MissingXInputSlot(void)) {
                Uint8 correlation_id = 0;
                Uint8 slot_idx = 0;
    ...
    if (!has_trigger_data && ctx->xinput_enabled && ctx->xinput_correlated) {
        ...
        if (xinput_state[ctx->xinput_slot].connected) {
            ...
            if (ctx->guide_hack) {
                SDL_SendJoystickButton(timestamp, joystick, (Uint8)guide_button, (xinput_state[ctx->xinput_slot].state.Gamepad.wButtons & XINPUT_GAMEPAD_GUIDE) ? SDL_PRESSED : SDL_RELEASED);
            }
            if (ctx->trigger_hack) {
...
#ifdef SDL_JOYSTICK_RAWINPUT_WGI
    if (!has_trigger_data && ctx->wgi_correlated) {
        RAWINPUT_UpdateWindowsGamingInput(void); /* May detect disconnect / cause uncorrelation */
        if (ctx->wgi_correlated) {           /* Still connected */
            ...
            if (ctx->guide_hack) {
                SDL_SendJoystickButton(timestamp, joystick, (Uint8)guide_button, (state->Buttons & GamepadButtons_GUIDE) ? SDL_PRESSED : SDL_RELEASED);
            }
            if (ctx->trigger_hack) {
