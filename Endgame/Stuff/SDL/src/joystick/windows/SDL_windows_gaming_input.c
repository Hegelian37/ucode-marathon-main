...
static SDL_bool SDL_IsXInputDevice(Uint16 vendor, Uint16 product)
{
    ...
    if (!SDL_XINPUT_Enabled(void)
#ifdef SDL_JOYSTICK_RAWINPUT
        && !RAWINPUT_IsEnabled(void)
#endif
    ) {
        return SDL_FALSE;
    }
...
static SDL_JoystickType GetGameControllerType(__x_ABI_CWindows_CGaming_CInput_CIGameController *game_controller)
{
     ...
     */
    WGI_LoadOtherControllerStatics(void);
...
static HRESULT STDMETHODCALLTYPE IEventHandler_CRawGameControllerVtbl_InvokeAdded(__FIEventHandler_1_Windows__CGaming__CInput__CRawGameController *This, IInspectable *sender, __x_ABI_CWindows_CGaming_CInput_CIRawGameController *e)
{
    ...
    /* We can get delayed calls to InvokeAdded(void) after WGI_JoystickQuit(void) */
    if (SDL_JoysticksQuitting(void) || !SDL_JoysticksInitialized(void)) {
    ...
    if (SUCCEEDED(hr)) {
        ...
        if (!ignore_joystick) {
            ...
            if (controllers) {
                ...
                SDL_JoystickID joystickID = SDL_GetNextObjectID(void);
...
static HRESULT STDMETHODCALLTYPE IEventHandler_CRawGameControllerVtbl_InvokeRemoved(__FIEventHandler_1_Windows__CGaming__CInput__CRawGameController *This, IInspectable *sender, __x_ABI_CWindows_CGaming_CInput_CIRawGameController *e)
{
    ...
    /* Can we get delayed calls to InvokeRemoved(void) after WGI_JoystickQuit(void)? */
    if (!SDL_JoysticksInitialized(void)) {
        SDL_UnlockJoysticks(void);
        return S_OK;
    }
...
static int WGI_JoystickInit(void)
{
    ...
    if (FAILED(WIN_RoInitialize(void))) {
        return SDL_SetError("RoInitialize(void) failed");
    }
...
#undef RESOLVE
...
#ifndef SDL_PLATFORM_WINRT
    {
         ...
         * This results in Windows_Gaming_Input!GameController::~GameController(void) invoking an unloaded DLL and crashing.
        ...
        if (!cookie) {
            ...
            if (FAILED(hr)) {
                return WIN_SetErrorFromHRESULT("CoIncrementMTAUsage(void) failed", hr);
            }
        }
    }
#endif

    WGI_LoadRawGameControllerStatics(void);
...
static void WGI_JoystickUpdate(SDL_Joystick *joystick)
{
    ...
    if (SUCCEEDED(hr) && (!timestamp || timestamp != hwdata->timestamp)) {
        ...
        if (all_zero) {
            ...
            /* FIXME: What units are the timestamp we get from GetCurrentReading(void)? */
