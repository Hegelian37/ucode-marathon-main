...
{
    SDL_assert(SDL_JoysticksLocked(void));
}
...
static SDL_bool SDL_GetDriverAndJoystickIndex(SDL_JoystickID instance_id, SDL_JoystickDriver **driver, int *driver_index)
{
    ...
    if (instance_id > 0) {
        for (i = 0; i < SDL_arraysize(SDL_joystick_drivers); ++i) {
            num_joysticks = SDL_joystick_drivers[i]->GetCount(void);
...
static SDL_bool SDL_SetJoystickIDForPlayerIndex(int player_index, SDL_JoystickID instance_id)
{
    ...
    if (existing_instance > 0) {
        SDL_SetJoystickIDForPlayerIndex(SDL_FindFreePlayerIndex(void), existing_instance);
    }
    return SDL_TRUE;
}
...
int SDL_InitJoysticks(void)
{
    ...
    if (SDL_joystick_lock == NULL) {
        SDL_joystick_lock = SDL_CreateMutex(void);
    }
    ...
    SDL_InitSteamVirtualGamepadInfo(void);

    status = -1;
    for (i = 0; i < SDL_arraysize(SDL_joystick_drivers); ++i) {
        if (SDL_joystick_drivers[i]->Init(void) >= 0) {
            status = 0;
        }
    }
    SDL_UnlockJoysticks(void);
...
SDL_JoystickID *SDL_GetJoysticks(int *count)
{
    ...
    {
        for (i = 0; i < SDL_arraysize(SDL_joystick_drivers); ++i) {
            total_joysticks += SDL_joystick_drivers[i]->GetCount(void);
        }
...
const SDL_SteamVirtualGamepadInfo *SDL_GetJoystickInstanceVirtualGamepadInfo(SDL_JoystickID instance_id)
{
    ...
    if (SDL_SteamVirtualGamepadEnabled(void) &&
...
static void AttemptSensorFusion(SDL_Joystick *joystick, SDL_bool invert_sensors)
{
     ...
     */
    if (SDL_GetNaturalDisplayOrientation(SDL_GetPrimaryDisplay(void)) == SDL_ORIENTATION_LANDSCAPE) {
...
SDL_Joystick *SDL_OpenJoystick(SDL_JoystickID instance_id)
{
    ...
    joystick->led_expiration = SDL_GetTicks(void);
...
SDL_PropertiesID SDL_GetJoystickProperties(SDL_Joystick *joystick)
{
    ...
    SDL_LockJoysticks(void);
    {
        ...
        if (joystick->props == 0) {
            joystick->props = SDL_CreateProperties(void);
        }
...
int SDL_RumbleJoystick(SDL_Joystick *joystick, Uint16 low_frequency_rumble, Uint16 high_frequency_rumble, Uint32 duration_ms)
{
    ...
    SDL_LockJoysticks(void);
    {
        ...
        if (low_frequency_rumble == joystick->low_frequency_rumble &&
            ...
            joystick->rumble_resend = SDL_GetTicks(void) + SDL_RUMBLE_RESEND_MS;
        }
        ...
        if (retval == 0) {
            ...
            if ((low_frequency_rumble || high_frequency_rumble) && duration_ms) {
                joystick->rumble_expiration = SDL_GetTicks(void) + SDL_min(duration_ms, SDL_MAX_RUMBLE_DURATION_MS);
    ...
    SDL_LockJoysticks(void);
    {
        ...
        if (retval == 0) {
            ...
            if ((left_rumble || right_rumble) && duration_ms) {
                joystick->trigger_rumble_expiration = SDL_GetTicks(void) + SDL_min(duration_ms, SDL_MAX_RUMBLE_DURATION_MS);
            } else {
...
int SDL_SetJoystickLED(SDL_Joystick *joystick, Uint8 red, Uint8 green, Uint8 blue)
{
    ...
    SDL_LockJoysticks(void);
    {
        ...
        if (isfreshvalue || SDL_GetTicks(void) >= joystick->led_expiration) {
            retval = joystick->driver->SetLED(joystick, red, green, blue);
            joystick->led_expiration = SDL_GetTicks(void) + SDL_LED_MIN_REPEAT_MS;
        } else {
...
/*
 * Close a joystick previously opened with SDL_OpenJoystick(void)
 */
...
void SDL_QuitJoysticks(void)
{
    ...
    for (i = SDL_arraysize(SDL_joystick_drivers) - 1; i >= 0; --i) {
        SDL_joystick_drivers[i]->Quit(void);
    }
    ...
    SDL_QuitSteamVirtualGamepadInfo(void);
...
static SDL_bool SDL_PrivateJoystickShouldIgnoreEvent(void)
{
    ...
    if (SDL_HasWindows(void) && SDL_GetKeyboardFocus(void) == NULL) {
...
void SDL_PrivateJoystickAdded(SDL_JoystickID instance_id)
{
    ...
    if (player_index < 0 && SDL_IsGamepad(instance_id)) {
        player_index = SDL_FindFreePlayerIndex(void);
    }
    if (player_index >= 0) {
...
int SDL_SendJoystickAxis(Uint64 timestamp, SDL_Joystick *joystick, Uint8 axis, Sint16 value)
{
     ...
     * events.
     */
    if (SDL_PrivateJoystickShouldIgnoreEvent(void)) {
...
void SDL_UpdateJoysticks(void)
{
    ...
    if (SDL_UpdateSteamVirtualGamepadInfo(void)) {
        SendSteamHandleUpdateEvents(void);
    }
     ...
     */
    for (i = 0; i < SDL_arraysize(SDL_joystick_drivers); ++i) {
        SDL_joystick_drivers[i]->Detect(void);
    }
