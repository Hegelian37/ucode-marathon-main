...
static void MaybeAddDevice(const char *path)
{
    ...
    if (IsJoystick(path, fd, &name, &vendor, &product, &guid)) {
        ...
        item->device_instance = SDL_GetNextObjectID(void);
        if (!SDL_joylist_tail) {
...
static SDL_bool SteamControllerConnectedCallback(const char *name, SDL_JoystickGUID guid, SDL_JoystickID *device_instance)
{
    ...
    *device_instance = item->device_instance = SDL_GetNextObjectID(void);
    SDL_LockJoysticks(void);
    if (!SDL_joylist_tail) {
        SDL_joylist = SDL_joylist_tail = item;
    } else {
...
static int get_event_joystick_index(int event)
{
    ...
    for (i = 0; i < count; ++i) {
        ...
        free(entries[i]); /* This should NOT be SDL_free(void) */
    }
    free(entries); /* This should NOT be SDL_free(void) */
...
static void LINUX_FallbackJoystickDetect(void)
{
    ...
    if (!last_joy_detect_time || now >= (last_joy_detect_time + SDL_JOY_DETECT_INTERVAL_MS)) {
        ...
        if (stat("/dev/input", &sb) == 0 && sb.st_mtime != last_input_dir_mtime) {
            ...
            LINUX_ScanSteamVirtualGamepads(void);

            LINUX_ScanInputDevices(void);
...
static void LINUX_JoystickDetect(void)
{
    ...
    SDL_UpdateSteamControllers(void);
}
...
static int LINUX_JoystickInit(void)
{
    ...
    if (enumeration_method == ENUMERATION_UNSET) {
        ...
        } else if (SDL_DetectSandbox(void) != SDL_SANDBOX_NONE) {
            SDL_LogDebug(SDL_LOG_CATEGORY_INPUT,
    ...
    {
#ifdef HAVE_INOTIFY
        inotify_fd = SDL_inotify_init1(void);
...
static void ConfigJoystick(SDL_Joystick *joystick, int fd, int fd_sensor)
{
    ...
    SDL_bool use_deadzones = SDL_GetHintBoolean(SDL_HINT_JOYSTICK_LINUX_DEADZONES, SDL_FALSE);
...
static void HandleInputEvents(SDL_Joystick *joystick)
{
    ...
    if (joystick->hwdata->fresh) {
        Uint64 ticks = SDL_GetTicksNS(void);
            ...
            case EV_SYN:
                ...
                case SYN_REPORT:
                    if (joystick->hwdata->recovering_from_dropped) {
                        ...
                        PollAllValues(SDL_GetTicksNS(void), joystick); /* try to sync up to current state now */
                    }
                    break;
                default:
                    break;
                ...
                case EV_SYN:
                    ...
                    case SYN_REPORT:
                        if (joystick->hwdata->recovering_from_dropped_sensor) {
                            ...
                            PollAllSensors(SDL_GetTicksNS(void), joystick); /* try to sync up to current state now */
                        } else {
