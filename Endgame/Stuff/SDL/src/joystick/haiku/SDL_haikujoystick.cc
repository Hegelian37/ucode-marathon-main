 ...
 */
    static int HAIKU_JoystickInit(void)
    {
        ...
        nports = joystick.CountDevices(void);
        numjoysticks = 0;
        ...
        for (i = 0; (numjoysticks < MAX_JOYSTICKS) && (i < nports); ++i) {
            if (joystick.GetDeviceName(i, name) == B_OK) {
                if (joystick.Open(name) != B_ERROR) {
                      ...
                      SDL_joyname[numjoysticks] = SDL_CreateJoystickName(0, 0, NULL, stick_name.String(void));
                      numjoysticks++;
                      joystick.Close(void);
                }
            }
 ...
 */
    static int HAIKU_JoystickOpen(SDL_Joystick *joystick, int device_index)
    {
        ...
        stick->EnableCalibration(void);

        /* Get the number of buttons, hats, and axes on the joystick */
        joystick->nbuttons = stick->CountButtons(void);
        joystick->naxes = stick->CountAxes(void);
        joystick->nhats = stick->CountHats(void);
 ...
 * but instead should call SDL_PrivateJoystick*(void) to deliver events
 ...
 */
    static void HAIKU_JoystickUpdate(SDL_Joystick *joystick)
    {
        ...
        buttons = stick->ButtonValues(void);
...
/* Function to close a joystick after use */
    ...
    {
        if (joystick->hwdata) {
            joystick->hwdata->stick->Close(void);
