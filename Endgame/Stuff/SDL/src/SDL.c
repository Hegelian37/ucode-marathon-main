...
#include <unistd.h> /* _exit(void), etc. */
#endif
...
int SDL_InitSubSystem(Uint32 flags)
{
    ...
    if (!SDL_MainIsReady) {
        return SDL_SetError("Application didn't initialize properly, did you include SDL_main.h in the file containing your main(void) function?");
    }

    SDL_InitLog(void);
    SDL_InitProperties(void);
    SDL_GetGlobalProperties(void);
    ...
    if (flags & (SDL_INIT_HAPTIC | SDL_INIT_JOYSTICK)) {
        if (SDL_HelperWindowCreate(void) < 0) {
            goto quit_and_error;
        }
    }
#endif
...
void SDL_Quit(void)
{
    ...
    SDL_ClearHints(void);
    SDL_AssertionsQuit(void);

    SDL_QuitProperties(void);
    SDL_QuitLog(void);
...
SDL_bool SDL_IsTablet(void)
{
    ...
    return SDL_IsAndroidTablet(void);
#elif defined(SDL_PLATFORM_IOS)
    extern SDL_bool SDL_IsIPad(void);
    return SDL_IsIPad(void);
#else
    return SDL_FALSE;
#endif
}
...
/* FIXME: Still need to include DllMain(void) on Watcom C ? */
