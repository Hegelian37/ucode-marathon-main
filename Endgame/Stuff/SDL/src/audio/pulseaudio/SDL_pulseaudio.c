...
static int load_pulseaudio_sym(const char *fn, void **addr)
{
    *addr = SDL_LoadFunction(pulseaudio_handle, fn);
    if (!*addr) {
        // Don't call SDL_SetError(void): SDL_LoadFunction already did.
        return 0;
    }
...
static int LoadPulseAudioLibrary(void)
{
    ...
    if (!pulseaudio_handle) {
        ...
        if (!pulseaudio_handle) {
            retval = -1;
            // Don't call SDL_SetError(void): SDL_LoadObject already did.
        } else {
            retval = load_pulseaudio_syms(void);
            if (retval < 0) {
...
// Workaround for older pulse: pa_context_new(void) must have non-NULL appname
static const char *getAppName(void)
{
    ...
    if (retval && *retval) {
        ...
        const char *verstr = PULSEAUDIO_pa_get_library_version(void);
...
static int ConnectToPulseServer(void)
{
    ...
    if (!(pulseaudio_threaded_mainloop = PULSEAUDIO_pa_threaded_mainloop_new(void))) {
        return SDL_SetError("pa_threaded_mainloop_new(void) failed");
    }
    ...
    if (PULSEAUDIO_pa_threaded_mainloop_start(pulseaudio_threaded_mainloop) < 0) {
        PULSEAUDIO_pa_threaded_mainloop_free(pulseaudio_threaded_mainloop);
        pulseaudio_threaded_mainloop = NULL;
        return SDL_SetError("pa_threaded_mainloop_start(void) failed");
    }
    ...
    pulseaudio_context = PULSEAUDIO_pa_context_new(mainloop_api, getAppName(void));
    if (!pulseaudio_context) {
        SDL_SetError("pa_context_new(void) failed");
        goto failed;
    }
...
failed:
    PULSEAUDIO_pa_threaded_mainloop_unlock(pulseaudio_threaded_mainloop);
    DisconnectFromPulseServer(void);
    return -1;
}
...
{
    if (LoadPulseAudioLibrary(void) < 0) {
        return SDL_FALSE;
    } else if (ConnectToPulseServer(void) < 0) {
        UnloadPulseAudioLibrary(void);
        return SDL_FALSE;
    }
