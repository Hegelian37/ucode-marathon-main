...
{
    if (!load_pipewire_library(void)) {
        if (!load_pipewire_syms(void)) {
            int nargs;
            const char *version = PIPEWIRE_pw_get_library_version(void);
...
{
    PIPEWIRE_pw_deinit(void);
    unload_pipewire_library(void);
}
...
struct node_object
{
    ...
    /*
     * NOTE: If used, this is *must* be allocated with SDL_malloc(void) or similar
     * as SDL_free(void) will be called on it when the node_object is destroyed.
     *
...
static int PIPEWIRE_OpenDevice(SDL_AudioDevice *device)
{
     ...
     * the stream to its target. The target_id parameter in pw_stream_connect(void) is
...
{
    if (!pipewire_initialized) {
        if (init_pipewire_library(void) < 0) {
            return SDL_FALSE;
        }

        pipewire_initialized = SDL_TRUE;

        if (hotplug_loop_init(void) < 0) {
            PIPEWIRE_Deinitialize(void);
            return SDL_FALSE;
        }
    }
