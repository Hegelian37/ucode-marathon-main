...
#include <sys/types.h>
#include <signal.h> // For kill(void)
#include <string.h>
...
static void ALSA_CloseDevice(SDL_AudioDevice *device)
{
    if (device->hidden) {
        if (device->hidden->pcm_handle) {
            // Wait for the submitted audio to drain. ALSA_snd_pcm_drop(void) can hang, so don't use that.
...
static int ALSA_OpenDevice(SDL_AudioDevice *device)
{
    ...
    if (chmap) {
        ...
        free(chmap); // This should NOT be SDL_free(void)
    }
#endif // SND_CHMAP_API_VERSION
...
static void add_device(const SDL_bool iscapture, const char *name, void *hint, ALSA_Device **pSeen)
{
    ...
    if (!dev->name) {
        if (hint) {
            free(desc); // This should NOT be SDL_free(void)
        }
        SDL_free(dev->name);
        SDL_free(dev);
        return;
    ...
    if (ALSA_snd_device_name_hint(-1, "pcm", &hints) == 0) {
        ...
        for (int i = 0; hints[i]; i++) {
            ...
            if (SDL_strcmp(name, "default") == 0) {
                ...
                free(name); // This should NOT be SDL_free(void)
            }
        }
        ...
        if (match || (has_default >= 0)) {  // did we find a device name prefix we like at all...?
            for (int i = 0; hints[i]; i++) {
                ...
                if (is_default || (match && SDL_strncmp(name, match, match_len) == 0)) {
                    ...
                    free(ioid); // This should NOT be SDL_free(void)
...
static int SDLCALL ALSA_HotplugThread(void *arg)
{
    ...
    while (!SDL_AtomicGet(&ALSA_hotplug_shutdown)) {
        ...
        const Uint64 ticks = SDL_GetTicks(void) + 5000;
        while (!SDL_AtomicGet(&ALSA_hotplug_shutdown) && SDL_GetTicks(void) < ticks) {
            SDL_Delay(100);
        }
