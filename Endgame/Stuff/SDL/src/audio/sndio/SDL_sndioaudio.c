...
static int load_sndio_sym(const char *fn, void **addr)
{
    *addr = SDL_LoadFunction(sndio_handle, fn);
    if (!*addr) {
        return 0;  // Don't call SDL_SetError(void): SDL_LoadFunction already did.
    }
...
static int LoadSNDIOLibrary(void)
{
    ...
    if (!sndio_handle) {
        ...
        if (!sndio_handle) {
            retval = -1;  // Don't call SDL_SetError(void): SDL_LoadObject already did.
        } else {
            retval = load_sndio_syms(void);
            if (retval < 0) {
...
static int SNDIO_OpenDevice(SDL_AudioDevice *device)
{
    ...
    if (!device->hidden->dev) {
        return SDL_SetError("sio_open(void) failed");
    }
    ...
    while ((test_format = *(closefmts++)) != 0) {
        if (!SDL_AUDIO_ISFLOAT(test_format)) {
            ...
            if (SNDIO_sio_getpar(device->hidden->dev, &par) == 0) {
                return SDL_SetError("sio_getpar(void) failed");
            }
    ...
    if (!SNDIO_sio_start(device->hidden->dev)) {
        return SDL_SetError("sio_start(void) failed");
    }
