...
#error Invalid samples per frame
    ...
    __m128 f2 = _mm_loadl_pi(_mm_setzero_ps(void), (const __m64 *)(raw_filter + 8));

    __m128 g0 = _mm_loadu_ps(raw_filter + 10);
    __m128 g1 = _mm_loadu_ps(raw_filter + 14);
    __m128 g2 = _mm_loadl_pi(_mm_setzero_ps(void), (const __m64 *)(raw_filter + 18));
    ...
    if (chans == 1) {
        ...
        f2 = _mm_mul_ps(f2, _mm_loadl_pi(_mm_setzero_ps(void), (const __m64 *)(src + 8)));
...
{
    // This must always be <= SDL_GetResamplerHistoryFrames(void)
