...
static BOOL CALLBACK FindAllDevs(LPGUID guid, LPCWSTR desc, LPCWSTR module, LPVOID userdata)
{
    ...
    if (guid != NULL) { // skip default device
        ...
        if (str) {
            ...
            SDL_free(str); // SDL_AddAudioDevice(void) makes a copy of this string.
        }
    }
...
static int DSOUND_OpenDevice(SDL_AudioDevice *device)
{
    ...
    if (!test_format) {
        if (tried_format) {
            return -1; // CreateSecondary(void) should have called SDL_SetError(void).
        }
        return SDL_SetError("%s: Unsupported audio format", "directsound");
    }

    // Playback buffers will auto-start playing in DSOUND_WaitDevice(void)
