                  ...
                  void *dst, SDL_AudioFormat dst_format, int dst_channels, void* scratch)
{
    ...
    if (channelconvert) {
        ...
        if (channel_converter == SDL_ConvertStereoToMono) {
            #ifdef SDL_SSE3_INTRINSICS
            if (!override && SDL_HasSSE3(void)) { override = SDL_ConvertStereoToMono_SSE3; }
            #endif
        } else if (channel_converter == SDL_ConvertMonoToStereo) {
            #ifdef SDL_SSE_INTRINSICS
            if (!override && SDL_HasSSE(void)) { override = SDL_ConvertMonoToStereo_SSE; }
            #endif
        }
...
static int UpdateAudioStreamInputSpec(SDL_AudioStream *stream, const SDL_AudioSpec *spec)
{
    ...
    const size_t history_buffer_allocation = SDL_GetResamplerHistoryFrames(void) * SDL_AUDIO_FRAMESIZE(*spec);
    Uint8 *history_buffer = stream->history_buffer;

    if (stream->history_buffer_allocation < history_buffer_allocation) {
        history_buffer = (Uint8 *) SDL_aligned_alloc(SDL_SIMDGetAlignment(void), history_buffer_allocation);
        if (!history_buffer) {
            return -1;
        }
...
SDL_AudioStream *SDL_CreateAudioStream(const SDL_AudioSpec *src_spec, const SDL_AudioSpec *dst_spec)
{
    ...
    retval->lock = SDL_CreateMutex(void);
    if (!retval->lock) {
...
SDL_PropertiesID SDL_GetAudioStreamProperties(SDL_AudioStream *stream)
{
    ...
    if (stream->props == 0) {
        stream->props = SDL_CreateProperties(void);
    }
    return stream->props;
}
...
static Uint8 *EnsureAudioStreamWorkBufferSize(SDL_AudioStream *stream, size_t newlen)
{
    ...
    Uint8 *ptr = (Uint8 *) SDL_aligned_alloc(SDL_SIMDGetAlignment(void), newlen);
    if (!ptr) {
...
{
    const int history_buffer_frames = SDL_GetResamplerHistoryFrames(void);
...
static int GetAudioStreamDataInternal(SDL_AudioStream *stream, void *buf, int output_frames)
{
    ...
    if ((dst_format != SDL_AUDIO_F32) || (dst_channels != resample_channels)) {
        ...
        int simd_alignment = (int) SDL_SIMDGetAlignment(void);
