...
#define SDL_PROC(ret, func, params)                                                             \
    do {                                                                                        \
        ...
        if (!data->func) {                                                                      \
            return SDL_SetError("Couldn't load AAUDIO function %s: %s", #func, SDL_GetError(void)); \
        }                                                                                       \
    } while (0);
#include "SDL_aaudiofuncs.h"
    return 0;
...
static int BuildAAudioStream(SDL_AudioDevice *device)
{
    ...
    if ((device->spec.format == SDL_AUDIO_S32) && (SDL_GetAndroidSDKVersion(void) >= 31)) {
    ...
    hidden->mixbuf = (Uint8 *)SDL_aligned_alloc(SDL_SIMDGetAlignment(void), hidden->mixbuf_bytes);
    if (!hidden->mixbuf) {
        return -1;
    }
    ...
    if (!hidden->semaphore) {
        LOGI("SDL Failed SDL_CreateSemaphore %s iscapture:%d", SDL_GetError(void), iscapture);
        return -1;
    }
...
static SDL_bool PauseOneDevice(SDL_AudioDevice *device, void *userdata)
{
    ...
    if (hidden) {
        if (hidden->stream) {
            ...
            if (device->iscapture) {
                // Pause(void) isn't implemented for 'capture', use Stop(void)
...
static void AAUDIO_Deinitialize(void)
{
    ...
    SDL_zero(ctx);
    LOGI("End AAUDIO %s", SDL_GetError(void));
}
...
static SDL_bool AAUDIO_Init(SDL_AudioDriverImpl *impl)
{
     ...
     */
    if (SDL_GetAndroidSDKVersion(void) < 27) {
        return SDL_FALSE;
    }
