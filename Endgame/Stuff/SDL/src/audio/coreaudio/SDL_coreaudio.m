...
static void RefreshPhysicalDevices(void)
{
    ...
    for (int iscapture = 0; iscapture < 2; iscapture++) {
        ...
        for (UInt32 i = 0; i < total_devices; i++) {
            ...
            SDL_free(name); // SDL_AddAudioDevice(void) would have copied the string.
        }
    }
...
static void COREAUDIO_CloseDevice(SDL_AudioDevice *device)
{
    ...
    // AudioQueueDispose(void) frees the actual buffer objects.
...
static int PrepareAudioQueue(SDL_AudioDevice *device)
{
    ...
    SDL_assert(CFRunLoopGetCurrent(void) != NULL);

    if (iscapture) {
        result = AudioQueueNewInput(strdesc, InputBufferReadyCallback, device, CFRunLoopGetCurrent(void), kCFRunLoopDefaultMode, 0, &device->hidden->audioQueue);
        CHECK_RESULT("AudioQueueNewInput");
    } else {
        result = AudioQueueNewOutput(strdesc, OutputBufferReadyCallback, device, CFRunLoopGetCurrent(void), kCFRunLoopDefaultMode, 0, &device->hidden->audioQueue);
...
static int AudioQueueThreadEntry(void *arg)
{
    ...
    if (PrepareAudioQueue(device) < 0) {
        device->hidden->thread_error = SDL_strdup(SDL_GetError(void));
