...
{
    if (index >= 0 && index < SDL_GetNumAudioDrivers(void)) {
...
static SDL_LogicalAudioDevice *ObtainLogicalAudioDevice(SDL_AudioDeviceID devid, SDL_AudioDevice **_device) SDL_NO_THREAD_SAFETY_ANALYSIS    // !!! FIXME: SDL_ACQUIRE
{
    ...
    if (!SDL_GetCurrentAudioDriver(void)) {
...
static SDL_AudioDevice *ObtainPhysicalAudioDevice(SDL_AudioDeviceID devid)  // !!! FIXME: SDL_ACQUIRE
{
    ...
    } else if (!SDL_GetCurrentAudioDriver(void)) {  // (the `islogical` path, above, checks this in ObtainLogicalAudioDevice.)
        SDL_SetError("Audio subsystem is not initialized");
    } else {
        SDL_LockRWLockForReading(current_audio.device_hash_lock);
...
static SDL_AudioDevice *CreatePhysicalAudioDevice(const char *name, SDL_bool iscapture, const SDL_AudioSpec *spec, void *handle, SDL_AtomicInt *device_count)
{
    ...
    device->lock = SDL_CreateMutex(void);
    if (!device->lock) {
        SDL_free(device->name);
        SDL_free(device);
        return NULL;
    ...
    device->close_cond = SDL_CreateCondition(void);
...
{
    if (SDL_GetCurrentAudioDriver(void)) {
        SDL_QuitAudio(void); // shutdown driver if already running.
    }
    ...
    SDL_ChooseAudioConverters(void);
    SDL_SetupAudioResampler(void);

    SDL_RWLock *device_hash_lock = SDL_CreateRWLock(void);  // create this early, so if it fails we don't have to tear down the whole audio subsystem.
    if (!device_hash_lock) {
        return -1;
    }
...
void SDL_QuitAudio(void)
{
    ...
    current_audio.impl.DeinitializeStart(void);
    ...
    current_audio.impl.Deinitialize(void);
...
static void ClosePhysicalAudioDevice(SDL_AudioDevice *device)
{
    ...
    // YOU MUST PROTECT KEY POINTS WITH SerializePhysicalDeviceClose(void) WHILE THE THREAD JOINS
    SDL_UnlockMutex(device->lock);
...
static int OpenPhysicalAudioDevice(SDL_AudioDevice *device, const SDL_AudioSpec *inspec)
{
    ...
    device->work_buffer = (Uint8 *)SDL_aligned_alloc(SDL_SIMDGetAlignment(void), device->work_buffer_size);
    ...
    if (device->spec.format != SDL_AUDIO_F32) {
        device->mix_buffer = (Uint8 *)SDL_aligned_alloc(SDL_SIMDGetAlignment(void), device->work_buffer_size);
...
int SDL_SetAudioPostmixCallback(SDL_AudioDeviceID devid, SDL_AudioPostmixCallback callback, void *userdata)
{
    ...
    if (logdev) {
        if (callback && !device->postmix_buffer) {
            device->postmix_buffer = (float *)SDL_aligned_alloc(SDL_SIMDGetAlignment(void), device->work_buffer_size);
...
// This is an internal function, so SDL_PumpEvents(void) can check for pending audio device events.
