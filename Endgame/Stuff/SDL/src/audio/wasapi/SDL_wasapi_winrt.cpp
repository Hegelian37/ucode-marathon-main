...
class SDL_WasapiDeviceEventHandler
{
  public:
    ...
    ~SDL_WasapiDeviceEventHandler(void);
    ...
    : iscapture(_iscapture), completed_semaphore(SDL_CreateSemaphore(0))
{
    ...
    Platform::String ^ selector = _iscapture ? MediaDevice::GetAudioCaptureSelector(void) : MediaDevice::GetAudioRenderSelector(void);
    Platform::Collections::Vector<Platform::String ^> properties;
    properties.Append(SDL_PKEY_AudioEngine_DeviceFormat);
    watcher = DeviceInformation::CreateWatcher(selector, properties.GetView(void));
    if (!watcher)
        return; // uhoh.
...
SDL_WasapiDeviceEventHandler::~SDL_WasapiDeviceEventHandler(void)
{
    if (watcher) {
...
void SDL_WasapiDeviceEventHandler::OnDeviceAdded(DeviceWatcher ^ sender, DeviceInformation ^ info)
{
    ...
    char *utf8dev = WIN_StringToUTF8(info->Name->Data(void));
    if (utf8dev) {
        SDL_AudioSpec spec;
        SDL_zero(spec);
        ...
        LPWSTR devid = SDL_wcsdup(info->Id->Data(void));
        if (devid) {
...
{
    SDL_assert(sender == this->watcher);
    WASAPI_DisconnectDevice(FindWinRTAudioDevice(info->Id->Data(void)));
}
...
void SDL_WasapiDeviceEventHandler::OnDefaultRenderDeviceChanged(Platform::Object ^ sender, DefaultAudioRenderDeviceChangedEventArgs ^ args)
{
    ...
    SDL_DefaultAudioDeviceChanged(FindWinRTAudioDevice(args->Id->Data(void)));
}
...
void SDL_WasapiDeviceEventHandler::WaitForCompletion(void)
{
    if (this->completed_semaphore) {
...
void WASAPI_EnumerateEndpoints(SDL_AudioDevice **default_output, SDL_AudioDevice **default_capture)
{
    ...
    playback_device_event_handler->WaitForCompletion(void);
    defdevid = MediaDevice::GetDefaultAudioRenderId(AudioDeviceRole::Default);
    if (defdevid) {
        *default_output = FindWinRTAudioDevice(defdevid->Data(void));
    }

    capture_device_event_handler = new SDL_WasapiDeviceEventHandler(SDL_TRUE);
    capture_device_event_handler->WaitForCompletion(void);
    defdevid = MediaDevice::GetDefaultAudioCaptureId(AudioDeviceRole::Default);
    if (defdevid) {
        *default_capture = FindWinRTAudioDevice(defdevid->Data(void));
    }
}
...
{
public:
    SDL_WasapiActivationHandler(void) : completion_semaphore(SDL_CreateSemaphore(0)) { SDL_assert(completion_semaphore != NULL); }
...
void SDL_WasapiActivationHandler::WaitForCompletion(void)
{
...
{
    ((SDL_WasapiActivationHandler *)handler)->Release(void);
}
...
int WASAPI_ActivateDevice(SDL_AudioDevice *device)
{
    ...
    ComPtr<SDL_WasapiActivationHandler> handler = Make<SDL_WasapiActivationHandler>(void);
    if (handler == nullptr) {
        return SDL_SetError("Failed to allocate WASAPI activation handler");
    }

    handler.Get(void)->AddRef(void); // we hold a reference after ComPtr destructs on return, causing a Release, and Release ourselves in WASAPI_PlatformDeleteActivationHandler(void), etc.
    device->hidden->activation_handler = handler.Get(void);

    IActivateAudioInterfaceAsyncOperation *async = nullptr;
    const HRESULT ret = ActivateAudioInterfaceAsync(devid, __uuidof(IAudioClient), nullptr, handler.Get(void), &async);

    if (FAILED(ret) || async == nullptr) {
        if (async != nullptr) {
            async->Release(void);
        }
        handler.Get(void)->Release(void);
    ...
    handler.Get(void)->WaitForCompletion(void);  // block here until we have an answer, so this is synchronous to us after all.
