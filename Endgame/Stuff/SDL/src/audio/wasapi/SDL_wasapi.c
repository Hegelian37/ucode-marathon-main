    ...
    while (((task = SDL_AtomicGetPtr((void **) &ManagementThreadPendingTasks)) != NULL) || !SDL_AtomicGet(&ManagementThreadShutdown)) {
        if (!task) {
            ...
            if (task->task_complete_sem) {                               // something waiting on result?
                task->errorstr = SDL_strdup(SDL_GetError(void));
...
int WASAPI_ProxyToManagementThread(ManagementThreadTask task, void *userdata, int *wait_on_result)
{
    ...
    if ((wait_on_result) && (SDL_GetCurrentThreadID(void) == SDL_GetThreadID(ManagementThread))) {
...
{
    if (WASAPI_PlatformInit(void) == -1) {
        return -1;
    }

    ManagementThreadLock = SDL_CreateMutex(void);
    if (!ManagementThreadLock) {
        WASAPI_PlatformDeinit(void);
        return -1;
    }

    ManagementThreadCondition = SDL_CreateCondition(void);
...
static int ManagementThreadEntry(void *userdata)
{
    ...
    if (ManagementThreadPrepare(void) < 0) {
        data->errorstr = SDL_strdup(SDL_GetError(void));
        SDL_PostSemaphore(data->ready_sem); // unblock calling thread.
        return 0;
    }

    SDL_PostSemaphore(data->ready_sem); // unblock calling thread.
    ManagementThreadMainloop(void);
...
static int WASAPI_PlayDevice(SDL_AudioDevice *device, const Uint8 *buffer, int buflen)
{
    if (device->hidden->render) { // definitely activated?
        // WasapiFailed(void) will mark the device for reacquisition or removal elsewhere.
...
#if defined(SDL_PLATFORM_WINRT) || defined(SDL_PLATFORM_GDK) // CreateEventEx(void) arrived in Vista, so we need an #ifdef for XP.
...
{
    DeinitManagementThread(void);
}

static SDL_bool WASAPI_Init(SDL_AudioDriverImpl *impl)
{
    if (InitManagementThread(void) < 0) {
        return SDL_FALSE;
    }
