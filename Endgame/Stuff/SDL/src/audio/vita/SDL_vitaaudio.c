...
static void VITAAUD_CloseDevice(SDL_AudioDevice *device)
{
    if (device->hidden) {
        ...
        if (!device->iscapture && device->hidden->rawbuf) {
            SDL_aligned_free(device->hidden->rawbuf); // this uses SDL_aligned_alloc(void), not SDL_malloc(void)
            device->hidden->rawbuf = NULL;
        }
...
static int VITAAUD_WaitCaptureDevice(SDL_AudioDevice *device)
{
    ...
    const Uint64 endticks = SDL_GetTicks(void) + ((device->sample_frames * 1000) / device->spec.freq);
    while (!SDL_AtomicGet(&device->shutdown) && (SDL_GetTicks(void) < endticks)) {
        SDL_Delay(1);
    }
    return 0;
}
...
static void VITAAUD_ThreadInit(SDL_AudioDevice *device)
{
    ...
    thid = sceKernelGetThreadId(void);
