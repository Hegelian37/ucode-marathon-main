...
static int EMSCRIPTENCAMERA_AcquireFrame(SDL_CameraDevice *device, SDL_Surface *frame, Uint64 *timestampNS)
{
    ...
    *timestampNS = SDL_GetTicksNS(void);  // best we can do here.
...
static void EMSCRIPTENCAMERA_CloseDevice(SDL_CameraDevice *device)
{
    if (device) {
        MAIN_THREAD_EM_ASM({
            ...
            SDL3.camera.stream.getTracks(void).forEach(track => track.stop(void));  // stop all recording.
...
static int EMSCRIPTENCAMERA_OpenDevice(SDL_CameraDevice *device, const SDL_CameraSpec *spec)
{
    MAIN_THREAD_EM_ASM({
        ...
        function grabNextCameraFrame(void) {  // !!! FIXME: this (currently) runs as a requestAnimationFrame callback, for lack of a better option.
            const SDL3 = Module['SDL3'];
            if ((typeof(SDL3) === 'undefined') || (typeof(SDL3.camera) === 'undefined') || (typeof(SDL3.camera.stream) === 'undefined')) {
            ...
            const now = performance.now(void);
            if (now >= nextframems) {
        ...
        navigator.mediaDevices.getUserMedia(constraints)
            .then((stream) => {
                const settings = stream.getVideoTracks(void)[0].getSettings(void);
                ...
                SDL3.camera.rgba = 0;
                SDL3.camera.next_frame_time = performance.now(void);

                video.play(void);
                video.addEventListener('loadedmetadata', (void) => {
                    grabNextCameraFrame(void);  // start this loop going.
                });
            })
            .catch((err) => {
...
static void EMSCRIPTENCAMERA_DetectDevices(void)
{
    ...
    //  don't want that during SDL_Init(void).
    if (supported) {
