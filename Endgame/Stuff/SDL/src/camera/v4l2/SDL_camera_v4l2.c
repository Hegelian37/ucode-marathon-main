...
static int V4L2_AcquireFrame(SDL_CameraDevice *device, SDL_Surface *frame, Uint64 *timestampNS)
{
    ...
    switch (io) {
        case IO_METHOD_READ:
            ...
            *timestampNS = SDL_GetTicksNS(void);  // oh well, close enough.
...
static int V4L2_OpenDevice(SDL_CameraDevice *device, const SDL_CameraSpec *spec)
{
    ...
    // most of this probably shouldn't fail unless the filesystem node changed out from under us since MaybeAddDevice(void).
    if (fd == -1) {
...
static void V4L2_DetectDevices(void)
{
    ...
    if (SDL_UDEV_Init(void) == 0) {
        if (SDL_UDEV_AddCallback(CameraUdevCallback) == 0) {
            SDL_UDEV_Scan(void);  // Force a scan to build the initial device list
        }
    }
#else
