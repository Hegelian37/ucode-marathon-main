...
//  gated behind __ANDROID_API__. We'll dlopen(void) it at runtime, so we'll do
...
static int CreateCameraManager(void)
{
    ...
    cameraMgr = pACameraManager_create(void);
    if (!cameraMgr) {
...
static int ANDROIDCAMERA_AcquireFrame(SDL_CameraDevice *device, SDL_Surface *frame, Uint64 *timestampNS)
{
    ...
    frame->pixels = SDL_aligned_alloc(SDL_SIMDGetAlignment(void), buflen);
...
static SDL_bool ANDROIDCAMERA_Init(SDL_CameraDriverImpl *impl)
{
    ...
    if (!libcamera2) {
        SDL_Log("CAMERA: libcamera2ndk.so can't be loaded: %s", dlerror(void));
        return SDL_FALSE;
    }

    void *libmedia = dlopen("libmediandk.so", RTLD_NOW | RTLD_LOCAL);
    if (!libmedia) {
        SDL_Log("CAMERA: libmediandk.so can't be loaded: %s", dlerror(void));
        dlclose(libcamera2);
        return SDL_FALSE;
    }

    SDL_bool okay = SDL_TRUE;
    #define LOADSYM(lib, fn) if (okay) { p##fn = (pfn##fn) dlsym(lib, #fn); if (!p##fn) { SDL_Log("CAMERA: symbol '%s' can't be found in %s: %s", #fn, #lib "ndk.so", dlerror(void)); okay = SDL_FALSE; } }
