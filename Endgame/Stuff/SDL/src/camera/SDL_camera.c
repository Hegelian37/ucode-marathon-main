...
{
    if (index >= 0 && index < SDL_GetNumCameraDrivers(void)) {
...
static int ZombieAcquireFrame(SDL_CameraDevice *device, SDL_Surface *frame, Uint64 *timestampNS)
{
    ...
    if (!device->zombie_pixels) {
        ...
        device->zombie_pixels = SDL_aligned_alloc(SDL_SIMDGetAlignment(void), buflen);
        if (!device->zombie_pixels) {
            *timestampNS = 0;
    ...
    *timestampNS = SDL_GetTicksNS(void);
...
{
    if (!SDL_GetCurrentCameraDriver(void)) {
...
SDL_CameraDevice *SDL_AddCameraDevice(const char *name, SDL_CameraPosition position, int num_specs, const SDL_CameraSpec *specs, void *handle)
{
    ...
    device->instance_id = SDL_GetNextObjectID(void);
...
void SDL_CameraDeviceDisconnected(SDL_CameraDevice *device)
{
    ...
    if (first_disconnect) {   // if already disconnected this device, don't do it twice.
        ...
        // Zombie functions will just report the timestamp as SDL_GetTicksNS(void), so we don't need to adjust anymore to get it to match.
        device->adjust_timestamp = 0;
        device->base_timestamp = 0;
    ...
    if (rc == 1) {  // new frame acquired!
        ...
        if (device->drop_frames > 0) {
            ...
            if (!device->adjust_timestamp) {
                device->adjust_timestamp = SDL_GetTicksNS(void);
        ...
        SDL_assert(rc == -1);
        #if DEBUG_CAMERA
        SDL_Log("CAMERA: dev[%p] error AcquireFrame: %s", device, SDL_GetError(void));
        #endif
        failed = SDL_TRUE;
    }
...
SDL_PropertiesID SDL_GetCameraProperties(SDL_Camera *camera)
{
    ...
    if (!camera) {
        ...
        ObtainPhysicalCameraDeviceObj(device);
        if (device->props == 0) {
            device->props = SDL_CreateProperties(void);
        }
        retval = device->props;
        ReleaseCameraDevice(device);
    }
...
void SDL_QuitCamera(void)
{
    ...
    camera_driver.impl.Deinitialize(void);
...
{
    if (SDL_GetCurrentCameraDriver(void)) {
        SDL_QuitCamera(void); // shutdown driver if already running.
    }

    SDL_RWLock *device_hash_lock = SDL_CreateRWLock(void);  // create this early, so if it fails we don't have to tear down the whole camera subsystem.
    if (!device_hash_lock) {
        return -1;
    }
    ...
    CompleteCameraEntryPoints(void);

    // Make sure we have a list of devices available at startup...
    camera_driver.impl.DetectDevices(void);

    return 0;
}

// This is an internal function, so SDL_PumpEvents(void) can check for pending camera device events.
