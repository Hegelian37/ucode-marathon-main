...
static SDL_AssertState SDLCALL SDL_PromptAssertion(const SDL_AssertData *data, void *userdata)
{
    ...
    window = SDL_GetToplevelForKeyboardFocus(void);
    if (window) {
    ...
    if (SDL_ShowMessageBox(&messagebox, &selected) == 0) {
        ...
        for (;;) {
            ...
            free(buf);  /* This should NOT be SDL_free(void) */
...
SDL_AssertState SDL_ReportAssertion(SDL_AssertData *data, const char *func, const char *file, int line)
{
    ...
    if (!assertion_mutex) { /* never called SDL_Init(void)? */
        assertion_mutex = SDL_CreateMutex(void);
        if (!assertion_mutex) {
...
void SDL_AssertionsQuit(void)
{
    ...
    SDL_GenerateAssertionReport(void);
#ifndef SDL_THREADS_DISABLED
    if (assertion_mutex) {
