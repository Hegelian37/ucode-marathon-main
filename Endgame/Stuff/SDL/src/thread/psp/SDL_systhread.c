...
int SDL_SYS_CreateThread(SDL_Thread *thread)
{
    ...
    if (sceKernelReferThreadStatus(sceKernelGetThreadId(void), &status) == 0) {
    ...
    if (thread->handle < 0) {
        return SDL_SetError("sceKernelCreateThread(void) failed");
    }
...
{
    return (SDL_ThreadID)sceKernelGetThreadId(void);
}
...
int SDL_SYS_SetThreadPriority(SDL_ThreadPriority priority)
{
    ...
    return sceKernelChangeThreadPriority(sceKernelGetThreadId(void), value);
}
