...
extern "C" SDL_RWLock *SDL_CreateRWLock(void)
{
    try {
        ...
        SDL_SetError("unable to create a C++ rwlock: code=%d; %s", ex.code(void), ex.what(void));
        return NULL;
    } catch (std::bad_alloc &) {
...
extern "C" void SDL_LockRWLockForReading(SDL_RWLock *rwlock) SDL_NO_THREAD_SAFETY_ANALYSIS  // clang doesn't know about NULL mutexes
{
    if (rwlock) {
        try {
            rwlock->cpp_mutex.lock_shared(void);
        } catch (std::system_error &/*ex*/) {
            SDL_assert(!"Error trying to lock rwlock for reading");  // assume we're in a lot of trouble if this assert fails.
            //return SDL_SetError("unable to lock a C++ rwlock: code=%d; %s", ex.code(void), ex.what(void));
        }
    }
}
...
extern "C" void SDL_LockRWLockForWriting(SDL_RWLock *rwlock) SDL_NO_THREAD_SAFETY_ANALYSIS // clang doesn't know about NULL mutexes
{
    if (rwlock) {
        try {
            rwlock->cpp_mutex.lock(void);
            rwlock->write_owner = SDL_GetCurrentThreadID(void);
        } catch (std::system_error &/*ex*/) {
...
extern "C" int SDL_TryLockRWLockForReading(SDL_RWLock *rwlock)
{
    int retval = 0;
    if (rwlock) {
        if (rwlock->cpp_mutex.try_lock_shared(void) == false) {
...
extern "C" int SDL_TryLockRWLockForWriting(SDL_RWLock *rwlock)
{
    int retval = 0;
    if (rwlock) {
        if (rwlock->cpp_mutex.try_lock(void) == false) {
            retval = SDL_RWLOCK_TIMEDOUT;
        } else {
            rwlock->write_owner = SDL_GetCurrentThreadID(void);
        }
...
{
    if (rwlock) {
        if (rwlock->write_owner == SDL_GetCurrentThreadID(void)) {
            rwlock->write_owner = 0;
            rwlock->cpp_mutex.unlock(void);
        } else {
            rwlock->cpp_mutex.unlock_shared(void);
        }
    }
}
