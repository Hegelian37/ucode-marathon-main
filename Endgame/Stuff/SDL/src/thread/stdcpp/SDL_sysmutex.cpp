...
extern "C" SDL_Mutex * SDL_CreateMutex(void)
{
    ...
    try {
        ...
        SDL_SetError("unable to create a C++ mutex: code=%d; %s", ex.code(void), ex.what(void));
    } catch (std::bad_alloc &) {
...
extern "C" void SDL_LockMutex(SDL_Mutex *mutex) SDL_NO_THREAD_SAFETY_ANALYSIS  // clang doesn't know about NULL mutexes
{
    if (mutex != NULL) {
        try {
            mutex->cpp_mutex.lock(void);
        } catch (std::system_error &/*ex*/) {
            SDL_assert(!"Error trying to lock mutex");  // assume we're in a lot of trouble if this assert fails.
            //return SDL_SetError("unable to lock a C++ mutex: code=%d; %s", ex.code(void), ex.what(void));
        }
    }
}
...
{
    return ((!mutex) || mutex->cpp_mutex.try_lock(void)) ? 0 : SDL_MUTEX_TIMEDOUT;
}
