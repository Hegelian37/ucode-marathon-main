...
SDL_CreateCondition(void)
{
    ...
    try {
        ...
        SDL_SetError("unable to create a C++ condition variable: code=%d; %s", ex.code(void), ex.what(void));
        return NULL;
    } catch (std::bad_alloc &) {
...
SDL_WaitConditionTimeoutNS(SDL_Condition *cond, SDL_Mutex *mutex, Sint64 timeoutNS)
{
    ...
    try {
        std::unique_lock<std::recursive_mutex> cpp_lock(mutex->cpp_mutex, std::adopt_lock_t(void));
        if (timeoutNS < 0) {
            cond->cpp_cond.wait(
                cpp_lock);
            cpp_lock.release(void);
            return 0;
        } else {
        ...
        return SDL_SetError("Unable to wait on a C++ condition variable: code=%d; %s", ex.code(void), ex.what(void));
    }
}
