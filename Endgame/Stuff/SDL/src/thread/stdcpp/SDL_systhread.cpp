...
SDL_SYS_CreateThread(SDL_Thread *thread)
{
    try {
        ...
        return SDL_SetError("unable to start a C++ thread: code=%d; %s", ex.code(void), ex.what(void));
    } catch (std::bad_alloc &) {
...
SDL_SYS_WaitThread(SDL_Thread *thread)
{
    ...
    try {
        ...
        if (cpp_thread->joinable(void)) {
            cpp_thread->join(void);
        }
...
{
    return SDL_Generic_GetTLSData(void);
}
