...
#elif defined(SDL_PLATFORM_LINUX)
            if (ppthread_setname_np(pthread_self(void), name) == ERANGE) {
                char namebuf[16]; /* Limited to 16 char */
                SDL_strlcpy(namebuf, name, sizeof(namebuf));
                ppthread_setname_np(pthread_self(void), namebuf);
            }
#endif
        }
#elif defined(HAVE_PTHREAD_SETNAME_NP)
#ifdef SDL_PLATFORM_NETBSD
        pthread_setname_np(pthread_self(void), "%s", name);
#else
        if (pthread_setname_np(pthread_self(void), name) == ERANGE) {
            char namebuf[16]; /* Limited to 16 char */
            SDL_strlcpy(namebuf, name, sizeof(namebuf));
            pthread_setname_np(pthread_self(void), namebuf);
        }
#endif
#elif defined(HAVE_PTHREAD_SET_NAME_NP)
        pthread_set_name_np(pthread_self(void), name);
...
{
    return (SDL_ThreadID)pthread_self(void);
}
...
int SDL_SYS_SetThreadPriority(SDL_ThreadPriority priority)
{
    ...
    int policy;
    int pri_policy;
    pthread_t thread = pthread_self(void);
    ...
    if (pthread_getschedparam(thread, &policy, &sched) != 0) {
        return SDL_SetError("pthread_getschedparam(void) failed");
    }
    ...
    if (pthread_setschedparam(thread, policy, &sched) != 0) {
        return SDL_SetError("pthread_setschedparam(void) failed");
    }
    return 0;
#endif /* linux */
