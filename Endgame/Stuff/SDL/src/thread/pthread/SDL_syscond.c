...
SDL_Condition *SDL_CreateCondition(void)
{
    ...
    if (cond) {
        if (pthread_cond_init(&cond->cond, NULL) != 0) {
            SDL_SetError("pthread_cond_init(void) failed");
            SDL_free(cond);
            cond = NULL;
        }
    }
...
int SDL_SignalCondition(SDL_Condition *cond)
{
    ...
    if (pthread_cond_signal(&cond->cond) != 0) {
        return SDL_SetError("pthread_cond_signal(void) failed");
    }
    return retval;
}
...
int SDL_BroadcastCondition(SDL_Condition *cond)
{
    ...
    if (pthread_cond_broadcast(&cond->cond) != 0) {
        return SDL_SetError("pthread_cond_broadcast(void) failed");
    }
    return retval;
}
...
int SDL_WaitConditionTimeoutNS(SDL_Condition *cond, SDL_Mutex *mutex, Sint64 timeoutNS)
{
    ...
    if (timeoutNS < 0) {
        if (pthread_cond_wait(&cond->cond, &mutex->id) != 0) {
            return SDL_SetError("pthread_cond_wait(void) failed");
        }
        return 0;
    }
...
tryagain:
    ...
    case 0:
        break;
    default:
        retval = SDL_SetError("pthread_cond_timedwait(void) failed");
    }
    return retval;
}
