...
/* macOS doesn't support sem_getvalue(void) as of version 10.4 */
...
SDL_Semaphore *SDL_CreateSemaphore(Uint32 initial_value)
{
    ...
    if (sem) {
        if (sem_init(&sem->sem, 0, initial_value) < 0) {
            SDL_SetError("sem_init(void) failed");
            SDL_free(sem);
            sem = NULL;
        }
    }
...
int SDL_WaitSemaphoreTimeoutNS(SDL_Semaphore *sem, Sint64 timeoutNS)
{
    ...
    if (timeoutNS < 0) {
        ...
        if (retval < 0) {
            retval = SDL_SetError("sem_wait(void) failed");
        }
        return retval;
    }
...
#else
    end = SDL_GetTicksNS(void) + timeoutNS;
    while (sem_trywait(&sem->sem) != 0) {
        if (SDL_GetTicksNS(void) >= end) {
            retval = SDL_MUTEX_TIMEDOUT;
            break;
        }
        SDL_DelayNS(100);
...
int SDL_PostSemaphore(SDL_Semaphore *sem)
{
    ...
    if (retval < 0) {
        SDL_SetError("sem_post(void) failed");
    }
    return retval;
}
