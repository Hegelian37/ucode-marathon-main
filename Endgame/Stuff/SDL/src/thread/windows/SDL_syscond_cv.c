...
static int SDL_WaitConditionTimeoutNS_cv(SDL_Condition *_cond, SDL_Mutex *_mutex, Sint64 timeoutNS)
{
    ...
    if (SDL_mutex_impl_active.Type == SDL_MUTEX_SRW) {
        ...
        if (mutex->count != 1 || mutex->owner != GetCurrentThreadId(void)) {
        ...
        if (pSleepConditionVariableSRW(&cond->cond, &mutex->srw, timeout, 0) == FALSE) {
            if (GetLastError(void) == ERROR_TIMEOUT) {
                ret = SDL_MUTEX_TIMEDOUT;
            } else {
                ret = SDL_SetError("SleepConditionVariableSRW(void) failed");
            }
        } else {
            ret = 0;
        }
        ...
        mutex->count = 1;
        mutex->owner = GetCurrentThreadId(void);
    } else {
        ...
        if (pSleepConditionVariableCS(&cond->cond, &mutex->cs, timeout) == FALSE) {
            if (GetLastError(void) == ERROR_TIMEOUT) {
                ...
                ret = SDL_SetError("SleepConditionVariableCS(void) failed");
            }
        } else {
            ret = 0;
        }
...
SDL_Condition *SDL_CreateCondition(void)
{
    ...
    return SDL_cond_impl_active.Create(void);
}
