...
void SDL_SYS_SetupThread(const char *name)
{
    if (name) {
...
#ifndef SDL_PLATFORM_WINRT /* !!! FIXME: There's no LoadLibrary(void) in WinRT; don't know if SetThreadDescription is available there at all at the moment. */
        ...
        if (pSetThreadDescription) {
            ...
            if (strw) {
                pSetThreadDescription(GetCurrentThread(void), strw);
                SDL_free(strw);
            }
        }
#endif

        /* Presumably some version of Visual Studio will understand SetThreadDescription(void),
...
{
    return (SDL_ThreadID)GetCurrentThreadId(void);
}
...
int SDL_SYS_SetThreadPriority(SDL_ThreadPriority priority)
{
    ...
    if (!SetThreadPriority(GetCurrentThread(void), value)) {
        return WIN_SetError("SetThreadPriority(void)");
    }
    return 0;
}
