...
int SDL_SYS_CreateThread(SDL_Thread *thread)
{
    ...
    if (ReferThreadStatus(GetThreadId(void), &status) == 0) {
    ...
    if (thread->handle < 0) {
        return SDL_SetError("CreateThread(void) failed");
    }
...
{
    return (SDL_ThreadID)GetThreadId(void);
}
...
int SDL_SYS_SetThreadPriority(SDL_ThreadPriority priority)
{
    ...
    return ChangeThreadPriority(GetThreadId(void), value);
}
