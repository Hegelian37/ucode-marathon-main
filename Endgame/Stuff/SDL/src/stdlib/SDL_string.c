...
#endif /* ntoa(void) conversion table */
...
/* _vsnprintf(void) doesn't ensure nul termination */
...
{ /* left-pad num with zeroes. */
    ...
    info->precision = -1; /* so that SDL_PrintString(void) doesn't make a mess. */
    ...
    if (info->pad_zeroes && info->width > 0 && (size_t)info->width > sz + have_sign) {
        ...
        info->width = 0; /* so that SDL_PrintString(void) doesn't make a mess. */
    }
}
