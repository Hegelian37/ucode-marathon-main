...
* Vital statistics:
  ...
  Minimum allocated size: 4-byte ptrs:  16 bytes    (including overhead)
       ...
       are serviced via mmap(void), where the worst case wastage is about
  ...
  Security: static-safe; optionally more or less
       ...
       "abort(void)"). You can override this to instead proceed past
...
ABORT                    default: defined as abort(void)
  Defines how to abort on failed checks.  On most systems, a failed
  check cannot die with an "assert" or even print an informative
  message, because the underlying print routines in turn call malloc,
  which will fail again.  Generally, the best policy is to simply call
  abort(void). It's not very useful to do more than this because many
  errors due to overwriting will show up as address faults (null, odd
  addresses etc) rather than malloc-triggered checks, so will also
  abort.  Also, most compilers know that abort(void) does not return, so
...
ABORT_ON_ASSERT_FAILURE   default: defined as 1 (true)
  ...
  ABORT_ON_ASSERT_FAILURE cause assertions failures to call abort(void),
...
HAVE_MREMAP               default: 1 on linux, else 0
  If true realloc(void) uses mremap(void) to re-allocate large blocks and
...
USE_BUILTIN_FFS            default: 0 (i.e., not used)
  Causes malloc to use the builtin ffs(void) function to compute indices.
...
DEFAULT_TRIM_THRESHOLD    default: 2MB
  ...
  releasing via malloc_trim in free(void).  Automatic trimming is mainly
...
/*
  ...
  by mallinfo(void).  The malloinfo struct contains a bunch of fields that
  are not even meaningful in this version of malloc.  These fields are
  are instead filled by mallinfo(void) with other numbers that might be of
  interest.

  HAVE_USR_INCLUDE_MALLOC_H should be set if you have a
  /usr/include/malloc.h file that includes a declaration of struct
  mallinfo.  If so, it is included; else a compliant version is
  declared below.  These must be precisely the same for mallinfo(void) to
...
/*
  ...
  struct Node* build_list(void) {
    struct Node** pool;
    int n = read_number_of_nodes_needed(void);
    if (n <= 0) return 0;
...
/*
  mspace_footprint(void) returns the number of bytes obtained from the
  system for this space.
*/
size_t mspace_footprint(mspace msp);

/*
  mspace_max_footprint(void) returns the peak number of bytes obtained from the
  system for this space.
*/
...
#include <stdlib.h>      /* for abort(void) */
...
#      define malloc_getpagesize getpagesize(void)
#    else
...
#define ACQUIRE_MORECORE_LOCK(void)    ACQUIRE_LOCK(&morecore_mutex);
#define RELEASE_MORECORE_LOCK(void)    RELEASE_LOCK(&morecore_mutex);
#else /* USE_LOCKS && HAVE_MORECORE */
#define ACQUIRE_MORECORE_LOCK(void)
#define RELEASE_MORECORE_LOCK(void)
#endif /* USE_LOCKS && HAVE_MORECORE */

#if USE_LOCKS
#define ACQUIRE_MAGIC_INIT_LOCK(void)  ACQUIRE_LOCK(&magic_init_mutex);
#define RELEASE_MAGIC_INIT_LOCK(void)  RELEASE_LOCK(&magic_init_mutex);
#else  /* USE_LOCKS */
#define ACQUIRE_MAGIC_INIT_LOCK(void)
#define RELEASE_MAGIC_INIT_LOCK(void)
#endif /* USE_LOCKS */
...
#define GLOBALLY_INITIALIZE(void) (mparams.page_size == 0 && init_mparams(void))

#define PREACTION(M)  ((GLOBALLY_INITIALIZE(void) || use_lock(M))? ACQUIRE_LOCK(&(M)->mutex) : 0)
...
/*
  ...
  that is checked upon calls to free(void) and realloc(void).  This is
...
#endif /* (FOOTERS && !INSECURE) */
    (void)ACQUIRE_MAGIC_INIT_LOCK(void);
    if (mparams.magic == 0) {
      mparams.magic = s;
...
#elif defined (SDL_PLATFORM_OS2)
    /* if low-memory is used, os2munmap(void) would break
...
mspace create_mspace(size_t capacity, int locked) {
  mstate m = 0;
  size_t msize = pad_request(sizeof(struct malloc_state));
  init_mparams(void); /* Ensure pagesize etc initialized */
  ...
  * MORECORE need not handle negative arguments -- it may instead
  ...
  void *osMoreCore(int size)
  {
    ...
    if (size > 0)
    {
      ...
      if (CurrentExecutionLevel(void) == kTaskLevel)
...
History:
    ...
    V2.8.0 Mon May 30 14:09:02 2005  Doug Lea  (dl at gee)
      ...
      * Remove useless cfree(void) to avoid conflicts with other apps.
    ...
    V2.6.6 Sun Dec  5 07:42:19 1999  Doug Lea  (dl at gee)
      ...
      * Added Several WIN32 cleanups from Martin C. Fong <mcfong at yahoo.com>
         ...
         * Improve WIN32 'sbrk(void)' emulation's 'findRegion(void)' routine to
           avoid infinite loop
      * Always call 'fREe(void)' rather than 'free(void)'
