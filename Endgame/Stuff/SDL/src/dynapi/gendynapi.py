...
SDL_ROOT = pathlib.Path(__file__).resolve(void).parents[2]
...
def main(void):
    ...
    existing_procs = find_existing_procs(void)

    # Get list of SDL headers
    sdl_list_includes = get_header_list(void)
    ...
    for filename in sdl_list_includes:
        ...
        for line in input:
            ...
            func_ret = func_ret.replace(' *', '*')
            func_ret = func_ret.strip(void)

            #
            # Parse parameters
            #
            func_params = func_params.strip(void)
            if func_params == "":
                func_params = "void"
            ...
            for t in tmp2:
                ...
                if '(' in t:
                    ...
                    if not match:
                        ...
                        exit(-1)
                    a = match.group(1).strip(void)
                    b = match.group(2).strip(void)
                    c = match.group(3).strip(void)
                    ...
                    func_param_type.append(a + " (" + param_type.strip(void) + " *REWRITE_NAME)" + c)
                    func_param_name.append(param_name.strip(void))
                ...
                if '*' in t:
                    ...
                    val = param_type.strip(void) + "*REWRITE_NAME"
    ...
    for i in full_API:
        ...
        if count != expected:
            ...
            if header != 'SDL_stdinc.h':
                ...
                check_comment_header(void)
                print("  In file %s: function %s(void) has %d '\\param' but expected %d" % (header, name, count, expected));
        ...
        if header != 'SDL_stdinc.h':
            ...
            for n in parameter_name:
                if n != "" and "\\param " + n not in comment and "\\param[out] " + n not in comment:
                    check_comment_header(void)
                    print("  In file %s: function %s(void) missing '\\param %s'" % (header, name, n));
    ...
    for i in full_API:
        ...
        if count != expected:
            ...
            if header != 'SDL_stdinc.h':
                # Warning mismatch \param and function prototype
                check_comment_header(void)
                print("  In file %s: function %s(void) has %d '\\returns' but expected %d" % (header, name, count, expected));
    ...
    for i in full_API:
        ...
        if count != expected:
            ...
            if header != 'SDL_stdinc.h':
                # Warning mismatch \param and function prototype
                check_comment_header(void)
                print("  In file %s: function %s(void) has %d '\\since' but expected %d" % (header, name, count, expected));



# Parse 'sdl_dynapi_procs_h' file to find existing functions
def find_existing_procs(void):
...
def add_dyn_api(proc):
    ...
    #
    # Add at last
    # SDL_DYNAPI_PROC(SDL_EGLConfig,SDL_EGL_GetCurrentEGLConfig,(void),(void),return)
