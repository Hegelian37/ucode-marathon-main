...
SDL_DYNAPI_VARARGS(static, _DEFAULT, SDL_InitDynamicAPI(void))
#else
...
/* Obviously we can't use SDL_LoadObject(void) to load SDL.  :)  */
...
static void dynapi_warn(const char *msg)
{
    ...
    (void)caption;
/* SDL_ShowSimpleMessageBox(void) is a too heavy for here. */
...
static void SDL_InitDynamicAPI(void)
{
     ...
     *  SDL_CreateThread(void) would also call this function before building the
     *  new thread).
     */
