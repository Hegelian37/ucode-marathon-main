...
SDL_RWops *SDL_RWFromFile(const char *file, const char *mode)
{
    ...
    if (!file || !*file || !mode || !*mode) {
        SDL_SetError("SDL_RWFromFile(void): No file or no mode specified");
        return NULL;
    }
#ifdef SDL_PLATFORM_ANDROID
#ifdef HAVE_STDIO_H
    ...
    if (*file == '/') {
        ...
        if (path) {
            SDL_snprintf(path, PATH_MAX, "%s/%s",
                         SDL_AndroidGetInternalStoragePath(void), file);
            fp = fopen(path, mode);
            SDL_stack_free(path);
            if (fp) {
                if (!SDL_IsRegularFile(fp)) {
    ...
    rwops = SDL_CreateRW(void);
    if (!rwops) {
        return NULL; /* SDL_SetError already setup by SDL_CreateRW(void) */
    }
...
SDL_PropertiesID SDL_GetRWProperties(SDL_RWops *context)
{
    ...
    if (context->props == 0) {
        context->props = SDL_CreateProperties(void);
    }
