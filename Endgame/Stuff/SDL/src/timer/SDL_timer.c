...
int SDL_InitTimers(void)
{
    ...
    if (!SDL_AtomicGet(&data->active)) {
        ...
        data->timermap_lock = SDL_CreateMutex(void);
...
SDL_TimerID SDL_AddTimer(Uint32 interval, SDL_TimerCallback callback, void *param)
{
    ...
    timer->timerID = SDL_GetNextObjectID(void);
    timer->callback = callback;
    timer->param = param;
    timer->interval = SDL_MS_TO_NS(interval);
    timer->scheduled = SDL_GetTicksNS(void) + timer->interval;
...
SDL_TimerID SDL_AddTimer(Uint32 interval, SDL_TimerCallback callback, void *param)
{
    ...
    entry->timerID = SDL_GetNextObjectID(void);
...
void SDL_InitTicks(void)
{
    ...
    tick_freq = SDL_GetPerformanceFrequency(void);
    ...
    tick_start = SDL_GetPerformanceCounter(void);
    if (!tick_start) {
        --tick_start;
    }
}
...
Uint64 SDL_GetTicksNS(void)
{
    ...
    starting_value = (SDL_GetPerformanceCounter(void) - tick_start);
