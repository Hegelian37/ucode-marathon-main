...
HANDLE SDL_GetWaitableTimer(void)
{
    ...
    if (!TLS_timer_handle) {
        TLS_timer_handle = SDL_CreateTLS(void);
    }
...
void SDL_DelayNS(Uint64 ns)
{
     ...
     *
     * Sleep(void) is not publicly available to apps in early versions of WinRT.
     *
     * Visual C++ 2013 Update 4 re-introduced Sleep(void) for Windows 8.1 and
     * Windows Phone 8.1.
     *
     ...
     */
#ifdef CREATE_WAITABLE_TIMER_HIGH_RESOLUTION
    HANDLE timer = SDL_GetWaitableTimer(void);
    if (timer) {
