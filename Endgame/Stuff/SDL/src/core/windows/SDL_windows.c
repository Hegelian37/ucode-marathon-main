...
int WIN_SetErrorFromHRESULT(const char *prefix, HRESULT hr)
{
    ...
    buffer[c] = 0;
    /* kill CR/LF that FormatMessage(void) sticks at the end */
    while (*p) {
        if (*p == '\r') {
            *p = 0;
            break;
...
/* Sets an error message based on GetLastError(void) */
int WIN_SetError(const char *prefix)
{
    return WIN_SetErrorFromHRESULT(prefix, GetLastError(void));
}
...
WIN_CoInitialize(void)
{
    /* SDL handles any threading model, so initialize with the default, which
       ...
       If you need multi-threaded mode, call CoInitializeEx(void) before SDL_Init(void)
    */
#ifdef SDL_PLATFORM_WINRT
    /* DLudwig: On WinRT, it is assumed that COM was initialized in main(void).
       CoInitializeEx is available (not CoInitialize though), however
       on WinRT, main(void) is typically declared with the [MTAThread]
...
{
    CHECKWINVER(FALSE, !WIN_IsWindowsVistaOrGreater(void) && IsWindowsVersionOrGreater(HIBYTE(_WIN32_WINNT_WINXP), LOBYTE(_WIN32_WINNT_WINXP), 0));
}
...
/* Win32-specific SDL_RunApp(void), which does most of the SDL_main work,
  based on SDL_windows_main.c, placed in the public domain by Sam Lantinga  4/13/98 */
#ifdef SDL_PLATFORM_WIN32

#include <shellapi.h> /* CommandLineToArgvW(void) */
...
DECLSPEC int MINGW32_FORCEALIGN SDL_RunApp(int _argc, char* _argv[], SDL_main_func mainFunction, void * reserved)
{
    ...
    argvw = CommandLineToArgvW(GetCommandLineW(void), &argc);
    if (!argvw) {
        return OutOfMemory(void);
    }

    /* Note that we need to be careful about how we allocate/free memory here.
     * If the application calls SDL_SetMemoryFunctions(void), we can't rely on
     * SDL_free(void) to use the same allocator after SDL_main(void) returns.
     */

    /* Parse it into argv and argc */
    argv = (char **)HeapAlloc(GetProcessHeap(void), HEAP_ZERO_MEMORY, (argc + 1) * sizeof(*argv));
    if (!argv) {
        return OutOfMemory(void);
    }
    for (i = 0; i < argc; ++i) {
        ...
        argv[i] = (char *)HeapAlloc(GetProcessHeap(void), HEAP_ZERO_MEMORY, len + 1);
        if (!argv[i]) {
            return OutOfMemory(void);
        }
    ...
    /* Run the application main(void) code */
    result = mainFunction(argc, argv);

    /* Free argv, to avoid memory leak */
    for (i = 0; i < argc; ++i) {
        HeapFree(GetProcessHeap(void), 0, argv[i]);
    }
    HeapFree(GetProcessHeap(void), 0, argv);
