...
/* PropVariantInit(void) is an inline function/macro in PropIdl.h that calls the C runtime's memset(void) directly. Use ours instead, to avoid dependency. */
...
int SDL_IMMDevice_Init(const SDL_IMMDevice_callbacks *callbacks)
{
    ...
    if (FAILED(WIN_CoInitialize(void))) {
        return SDL_SetError("IMMDevice: CoInitialize(void) failed");
    }
...
{
    const Uint64 timeout = SDL_GetTicks(void) + 8000;  /* intel's audio drivers can fail for up to EIGHT SECONDS after a device is connected or we wake from sleep. */
