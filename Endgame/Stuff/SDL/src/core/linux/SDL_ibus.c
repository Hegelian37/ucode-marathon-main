...
{
    Uint32 ibus_mods = 0;
    SDL_Keymod sdl_mods = SDL_GetModState(void);
...
static char *IBus_GetDBusAddressFilename(void)
{
    ...
    *disp_num = 0;
    ...
    key = SDL_DBus_GetLocalMachineId(void);
...
static SDL_bool IBus_SetupConnection(SDL_DBusContext *dbus, const char *addr)
{
    ...
    SDL_IBus_SetFocus(SDL_GetKeyboardFocus(void) != NULL);
    SDL_IBus_UpdateTextRect(NULL);
...
SDL_bool SDL_IBus_Init(void)
{
    ...
    if (dbus) {
        char *addr_file = IBus_GetDBusAddressFilename(void);
        char *addr;
        char *addr_file_dir;
...
SDL_bool SDL_IBus_ProcessKeyEvent(Uint32 keysym, Uint32 keycode, Uint8 state)
{
    ...
    if (IBus_CheckConnection(dbus)) {
        Uint32 mods = IBus_ModState(void);
...
void SDL_IBus_UpdateTextRect(const SDL_Rect *rect)
{
    ...
    focused_win = SDL_GetKeyboardFocus(void);
    if (!focused_win) {
        return;
    }
