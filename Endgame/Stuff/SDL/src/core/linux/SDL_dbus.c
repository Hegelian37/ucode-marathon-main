...
failed:
    /* message_iter_abandon_container_if_open(void) and message_iter_abandon_container(void) might be
...
SDL_bool SDL_DBus_ScreensaverInhibit(SDL_bool inhibit)
{
    ...
    if (SDL_DetectSandbox(void) != SDL_SANDBOX_NONE) {
...
/*
 * Get the machine ID if possible. Result must be freed with dbus->free(void).
 */
char *SDL_DBus_GetLocalMachineId(void)
{
    ...
    if (dbus.try_get_local_machine_id) {
         ...
         * treat that as a fatal mis-installation and abort(void) */
        result = dbus.get_local_machine_id(void);
    }
 ...
 * Result must be freed with dbus->free_string_array(void).
...
char **SDL_DBus_DocumentsPortalRetrieveFiles(const char *key, int *path_count)
{
    ...
    if (!SDL_DBus_GetContext(void) || !dbus.session_conn) {
