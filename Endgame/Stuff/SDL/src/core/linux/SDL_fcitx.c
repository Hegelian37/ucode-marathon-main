...
static char *GetAppName(void)
{
    ...
    (void)SDL_snprintf(procfile, sizeof(procfile), "/proc/%d/exe", getpid(void));
#elif defined(SDL_PLATFORM_FREEBSD)
    (void)SDL_snprintf(procfile, sizeof(procfile), "/proc/%d/file", getpid(void));
#endif
...
{
    fcitx_client.dbus = SDL_DBus_GetContext(void);
...
{
    Uint32 mod_state = Fcitx_ModState(void);
