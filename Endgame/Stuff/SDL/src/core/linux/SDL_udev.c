...
int SDL_UDEV_Init(void)
{
    ...
    if (!_this) {
        ...
        retval = SDL_UDEV_LoadLibrary(void);
        if (retval < 0) {
            SDL_UDEV_Quit(void);
            return retval;
        }
        ...
        _this->udev = _this->syms.udev_new(void);
        if (!_this->udev) {
            SDL_UDEV_Quit(void);
            return SDL_SetError("udev_new(void) failed");
        }

        _this->udev_mon = _this->syms.udev_monitor_new_from_netlink(_this->udev, "udev");
        if (!_this->udev_mon) {
            SDL_UDEV_Quit(void);
            return SDL_SetError("udev_monitor_new_from_netlink(void) failed");
        }
...
int SDL_UDEV_Scan(void)
{
    ...
    if (!enumerate) {
        SDL_UDEV_Quit(void);
        return SDL_SetError("udev_enumerate_new(void) failed");
    }
...
SDL_bool SDL_UDEV_GetProductInfo(const char *device_path, Uint16 *vendor, Uint16 *product, Uint16 *version, int *class)
{
    ...
    enumerate = _this->syms.udev_enumerate_new(_this->udev);
    if (!enumerate) {
        SDL_SetError("udev_enumerate_new(void) failed");
        return SDL_FALSE;
    }
...
int SDL_UDEV_LoadLibrary(void)
{
    ...
    if (!_this->udev_handle) {
        ...
        if (_this->udev_handle) {
            retval = SDL_UDEV_load_syms(void);
            if (retval < 0) {
                SDL_UDEV_UnloadLibrary(void);
            }
        }
...
void SDL_UDEV_Poll(void)
{
    ...
    while (SDL_UDEV_hotplug_update_available(void)) {
