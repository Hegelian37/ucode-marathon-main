...
IFrameworkView ^ SDLApplicationSource::CreateView(void)
{
    // TODO, WinRT: see if this function (CreateView) can ever get called
    // more than once.  For now, just prevent it from ever assigning
    // SDL_WinRTGlobalApp more than once.
    SDL_assert(!SDL_WinRTGlobalApp);
    SDL_WinRTApp ^ app = ref new SDL_WinRTApp(void);
...
int SDL_WinRTInitNonXAMLApp(int (*mainFunction)(int, char **))
{
    ...
    auto direct3DApplicationSource = ref new SDLApplicationSource(void);
    CoreApplication::Run(direct3DApplicationSource);
    return 0;
}

static void WINRT_ProcessWindowSizeChange(void) // TODO: Pass an SDL_Window-identifying thing into WINRT_ProcessWindowSizeChange(void)
{
    CoreWindow ^ coreWindow = CoreWindow::GetForCurrentThread(void);
    if (coreWindow) {
...
SDL_WinRTApp::SDL_WinRTApp(void) : m_windowClosed(false),
...
#endif
{
    ...
    {
        CoreWindow ^ window = CoreWindow::GetForCurrentThread(void);
        if (window) {
    ...
    WINRT_ProcessWindowSizeChange(void);
...
void SDL_WinRTApp::SetWindow(CoreWindow ^ window)
{
    ...
    Windows::Devices::Input::MouseDevice::GetForCurrentView(void)->MouseMoved +=
...
#if NTDDI_VERSION >= NTDDI_WIN10
    Windows::UI::Core::SystemNavigationManager::GetForCurrentView(void)->BackRequested +=
...
#if NTDDI_VERSION > NTDDI_WIN8
    DisplayInformation::GetForCurrentView(void)->OrientationChanged +=
    ...
    SettingsPane::GetForCurrentView(void)->CommandsRequested +=
...
void SDL_WinRTApp::Run(void)
{
    SDL_SetMainReady(void);
    if (WINRT_SDLAppEntryPoint) {
        // TODO, WinRT: pass the C-style main(void) a reasonably realistic
...
bool SDL_WinRTApp::ShouldWaitForAppResumeEvents(void)
{
     ...
     * own event pump (aka ProcessEvents(void)) will pause regardless
...
void SDL_WinRTApp::PumpEvents(void)
{
    if (!m_windowClosed) {
        if (!ShouldWaitForAppResumeEvents(void)) {
            /* This is the normal way in which events should be pumped.
             * 'ProcessAllIfPresent' will make ProcessEvents(void) process anywhere
             * from zero to N events, and will then return.
             */
            CoreWindow::GetForCurrentThread(void)->Dispatcher->ProcessEvents(CoreProcessEventsOption::ProcessAllIfPresent);
        } else {
             ...
             */
            CoreWindow::GetForCurrentThread(void)->Dispatcher->ProcessEvents(CoreProcessEventsOption::ProcessOneAndAllPending);
        }
    }
}

void SDL_WinRTApp::Uninitialize(void)
{
}
...
{
    CoreWindow::GetForCurrentThread(void)->Activate(void);
}
...
void SDL_WinRTApp::OnSuspending(Platform::Object ^ sender, SuspendingEventArgs ^ args)
{
    ...
    SuspendingDeferral ^ deferral = args->SuspendingOperation->GetDeferral(void);
    create_task([this, deferral](void) {
        // Send an app did-enter-background event immediately to observers.
        // CoreDispatcher::ProcessEvents, which is the backbone on which
        // SDL_WinRTApp::PumpEvents is built, will not return to its caller
        // once it sends out a suspend event.  Any events posted to SDL's
        // event queue won't get received until the WinRT app is resumed.
        // SDL_AddEventWatch(void) may be used to receive app-suspend events on
        // WinRT.
