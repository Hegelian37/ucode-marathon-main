 ...
 * Output 'pitch' that can be used with SDL_ConvertPixels(void)
 *
                        ...
                        const Uint8 **y, const Uint8 **u, const Uint8 **v, Uint32 *y_stride, Uint32 *uv_stride)
{
    ...
    case SDL_PIXELFORMAT_NV21:
        ...
        break;
    default:
        return SDL_SetError("GetYUVPlanes(void): Unsupported YUV format: %s", SDL_GetPixelFormatName(format));
    }
...
#define PACKED4_TO_PACKED4_ROW_SSE2(shuffle)                      \
    while (x >= 4) {                                              \
        ...
        __m128i lo = _mm_unpacklo_epi8(yuv, _mm_setzero_si128(void)); \
        __m128i hi = _mm_unpackhi_epi8(yuv, _mm_setzero_si128(void)); \
