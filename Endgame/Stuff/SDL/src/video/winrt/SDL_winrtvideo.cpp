...
static void WINRT_DeleteDevice(SDL_VideoDevice *device)
{
    if (device->driverdata) {
        ...
        if (video_data->winrtEglWindow) {
            video_data->winrtEglWindow->Release(void);
        }
        SDL_free(video_data);
    }
...
static int WINRT_AddDisplaysForAdapter(SDL_VideoDevice *_this, IDXGIFactory2 *dxgiFactory2, int adapterIndex)
{
    ...
    if (FAILED(hr)) {
        if (hr != DXGI_ERROR_NOT_FOUND) {
            WIN_SetErrorFromHRESULT(__FUNCTION__ ", IDXGIFactory1::EnumAdapters1(void) failed", hr);
        }
        return -1;
    }
        ...
        if (WINRT_AddDisplaysForOutput(_this, dxgiAdapter1, outputIndex) < 0) {
            ...
            */
            if (adapterIndex == 0 && outputIndex == 0) {
                ...
                ApplicationView ^ appView = ApplicationView::GetForCurrentView(void);
#endif
                CoreWindow ^ coreWin = CoreWindow::GetForCurrentThread(void);
                SDL_zero(display);
                SDL_zero(mode);
...
int WINRT_InitModes(SDL_VideoDevice *_this)
{
    ...
    */
    ...
    if (FAILED(hr)) {
        return WIN_SetErrorFromHRESULT(__FUNCTION__ ", CreateDXGIFactory1(void) failed", hr);
    }
...
void WINRT_VideoQuit(SDL_VideoDevice *_this)
{
    ...
    if (driverdata && driverdata->displayRequest) {
        driverdata->displayRequest->Release(void);
...
int WINRT_CreateWindow(SDL_VideoDevice *_this, SDL_Window *window, SDL_PropertiesID create_props)
{
    ...
    */
    if (!WINRT_XAMLWasEnabled) {
        data->coreWindow = CoreWindow::GetForCurrentThread(void);
#if SDL_WINRT_USE_APPLICATIONVIEW
        data->appView = ApplicationView::GetForCurrentView(void);
#endif
    }
    SDL_SetProperty(SDL_GetWindowProperties(window), SDL_PROP_WINDOW_WINRT_WINDOW_POINTER, reinterpret_cast<IInspectable *>(data->coreWindow.Get(void)));
    ...
    if (!(window->flags & SDL_WINDOW_OPENGL)) {
        ...
        if (SDL_EGL_ChooseConfig(_this) != 0) {
            /* SDL_EGL_ChooseConfig failed, SDL_GetError(void) should have info */
            return -1;
        }
        ...
        } else if (data->coreWindow.Get(void) != nullptr) {
            /* Attempt to create a window surface using newer versions of
             * ANGLE/WinRT:
             */
            IInspectable *coreWindowAsIInspectable = reinterpret_cast<IInspectable *>(data->coreWindow.Get(void));
    ...
    if (WINRT_XAMLWasEnabled) {
        ...
        */
        ...
        bool isWindowActive = WINRT_IsCoreWindowActive(data->coreWindow.Get(void));
        if (isWindowActive) {
    ...
    if (isWindowActive) {
        if (fullscreen) {
            if (!data->appView->IsFullScreenMode) {
                return data->appView->TryEnterFullScreenMode(void) ? 0 : -1;
            }
        } else {
            if (data->appView->IsFullScreenMode) {
                data->appView->ExitFullScreenMode(void);
            }
        }
    }
#endif
...
static ABI::Windows::System::Display::IDisplayRequest *WINRT_CreateDisplayRequest(SDL_VideoDevice *_this)
{
    ...
    if (pDisplayRequestRaw) {
        pDisplayRequestRaw->Release(void);
    }
    if (pActivationFactory) {
        pActivationFactory->Release(void);
    }
    if (hClassName) {
...
int WINRT_SuspendScreenSaver(SDL_VideoDevice *_this)
{
    ...
    if (driverdata && driverdata->displayRequest) {
        ...
        if (_this->suspend_screensaver) {
            displayRequest->RequestActive(void);
        } else {
            displayRequest->RequestRelease(void);
        }
    }
    return 0;
}
