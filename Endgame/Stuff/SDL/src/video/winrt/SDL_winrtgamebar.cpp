...
/* Retrieves a pointer to the game bar, or NULL if it is not available.
   If a pointer is returned, it's ->Release(void) method must be called
   after the caller has finished using it.
*/
static IGameBarStatics_ *WINRT_GetGameBar(void)
{
...
static void WINRT_HandleGameBarIsInputRedirected_MainThread(void)
{
    ...
    if (SUCCEEDED(gameBar->get_IsInputRedirected(&isInputRedirected))) {
        if (!isInputRedirected) {
            ...
            */
            SDL_Cursor *cursor = SDL_GetCursor(void);
            SDL_SetCursor(cursor);
        }
    }
    gameBar->Release(void);
}

static void WINRT_HandleGameBarIsInputRedirected_NonMainThread(Platform::Object ^ o1, Platform::Object ^ o2)
{
    Windows::UI::Core::CoreDispatcher ^ dispatcher = WINRT_MainThreadDispatcher.Resolve<Windows::UI::Core::CoreDispatcher>(void);
    if (dispatcher) {
        dispatcher->RunAsync(
...
void WINRT_InitGameBar(SDL_VideoDevice *_this)
{
    ...
    IGameBarStatics_ *gameBar = WINRT_GetGameBar(void);
    if (gameBar) {
        /* GameBar.IsInputRedirected events can come in via something other than
           ...
           SDL thread.
        */
        WINRT_MainThreadDispatcher = Windows::UI::Core::CoreWindow::GetForCurrentThread(void)->Dispatcher;
