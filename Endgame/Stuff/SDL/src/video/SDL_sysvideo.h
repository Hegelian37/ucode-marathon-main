...
struct SDL_Window
{
     ...
     *
     * This is the size and position to which the window should return when
     * it's maximized and SDL_RestoreWindow(void) is called.
     */
    SDL_Rect floating;
    ...
    SDL_bool is_repositioning; /* Set during an SDL_SetWindowPosition(void) call. */
    SDL_bool is_hiding;
    SDL_bool restore_on_show; /* Child was hidden recursively by the parent, restore when shown. */
    SDL_bool is_destroying;
    SDL_bool is_dropping; /* drag/drop in progress, expecting SDL_SendDropComplete(void). */
...
struct SDL_VideoDevice
{
    ...
    /*
     * Reverse the effects VideoInit(void) -- called if VideoInit(void) fails or
    ...
    /* Flag that stores whether it's allowed to call SDL_GL_MakeCurrent(void)
...
/* Use SDL_OnVideoThread(void) sparingly, to avoid regressions in use cases that currently happen to work */
