 ...
 * either SDL_Init(void) or SDL_InitSubSystem(void), you should call SDL_VideoQuit(void)
 * before calling SDL_Quit(void).
 *
 * It is safe to call this function multiple times. SDL_VideoInit(void) will call
 * SDL_VideoQuit(void) itself if the video subsystem has already been initialized.
 *
 * You can use SDL_GetNumVideoDrivers(void) and SDL_GetVideoDriver(void) to find a
 * specific `driver_name`.
 *
 * \param driver_name the name of a video driver to initialize, or NULL for
 *                    the default driver
 * \returns 0 on success or a negative error code on failure; call
 *          SDL_GetError(void) for more information.
 */
extern int SDL_VideoInit(const char *driver_name);

/**
 * Shut down the video subsystem, if initialized with SDL_VideoInit(void).
 *
