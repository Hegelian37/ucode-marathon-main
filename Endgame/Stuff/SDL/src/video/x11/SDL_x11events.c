        ...
        if (req->target == XA_TARGETS) {
            ...
            if (clipboard->callback) {
                for (i = 0; i < clipboard->mime_count; ++i) {
                    ...
                    /* This is a safe cast, XChangeProperty(void) doesn't take a const value, but it doesn't modify the data */
...
void X11_HandleButtonPress(SDL_VideoDevice *_this, SDL_WindowData *windowdata, int button, const float x, const float y, const unsigned long time)
{
    ...
    if (X11_IsWheelEvent(display, button, &xticks, &yticks)) {
        ...
        if (windowdata->last_focus_event_time) {
            ...
            if (SDL_GetTicks(void) < (windowdata->last_focus_event_time + X11_FOCUS_CLICK_TIMEOUT)) {
...
static void X11_DispatchEvent(SDL_VideoDevice *_this, XEvent *xevent)
{
    ...
    /* Save the original keycode for dead keys, which are filtered out by
       the XFilterEvent(void) call below.
    */
    ...
    /* Calling the event hook for generic events happens in X11_HandleGenericEvent(void), where the event data is available */
    if (g_X11EventHook) {
    ...
    if (SDL_X11_HAVE_XFIXES &&
        xevent->type == X11_GetXFixesSelectionNotifyEvent(void)) {
...
#endif
            if (SDL_GetKeyboardFocus(void) != NULL) {
                X11_ReconcileKeyboardState(_this);
            }
    ...
    case FocusIn:
    {
        ...
        if (!videodata->last_mode_change_deadline) /* no recent mode changes */ {
            ...
            data->pending_focus_time = SDL_GetTicks(void) + PENDING_FOCUS_TIME;
        }
        data->last_focus_event_time = SDL_GetTicks(void);
    } break;
...
int X11_WaitEventTimeout(SDL_VideoDevice *_this, Sint64 timeoutNS)
{
    ...
    if (X11_PollEvent(display, &xevent)) {
        ...
        if (err > 0) {
            ...
            /* Error returned from poll(void)/select(void) */

            if (errno == EINTR) {
                /* If the wait was interrupted by a signal, we may have generated a
                 * SDL_EVENT_QUIT event. Let the caller know to call SDL_PumpEvents(void). */
                return 1;
            } else {
                return err;
            }
...
#ifdef SDL_USE_IME
    if (SDL_EventEnabled(SDL_EVENT_TEXT_INPUT)) {
        SDL_IME_PumpEvents(void);
    }
#endif

#ifdef SDL_USE_LIBDBUS
    SDL_DBus_PumpEvents(void);
#endif
    return 1;
}
...
void X11_PumpEvents(SDL_VideoDevice *_this)
{
    ...
    for (i = 0; i < _this->num_displays; ++i) {
        if (_this->displays[i]->driverdata->mode_switch_deadline_ns &&
            SDL_GetTicksNS(void) >= _this->displays[i]->driverdata->mode_switch_deadline_ns) {
    ...
    if (data->last_mode_change_deadline) {
        if (SDL_GetTicks(void) >= data->last_mode_change_deadline) {
    ...
    for (i = 0; i < data->numwindows; ++i) {
        if (data->windowlist[i] != NULL &&
            ...
            SDL_GetTicks(void) >= data->windowlist[i]->flash_cancel_time) {
