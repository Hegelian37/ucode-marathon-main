...
static int X11_MessageBoxCreateWindow(SDL_MessageBoxDataX11 *data)
{
    ...
    if (windowdata) {
        ...
        const SDL_VideoDevice *dev = SDL_GetVideoDevice(void);
...
static int X11_MessageBoxLoop(SDL_MessageBoxDataX11 *data)
{
    ...
    while (!close_dialog) {
        ...
        /* can't use XWindowEvent(void) because it can't handle ClientMessage events. */
        /* can't use XNextEvent(void) because we only want events for this window. */
...
{
#if SDL_FORK_MESSAGEBOX
    /* Use a child process to protect against setlocale(void). Annoying. */
    pid_t pid;
    int fds[2];
    int status = 0;
    ...
    if (pid == -1) { /* failed */
        ...
        close(fds[1]);
        _exit(exitcode); /* don't run atexit(void) stuff, static destructors, etc. */
