...
{
    SDL_VideoDevice *viddevice = SDL_GetVideoDevice(void);
    int i;
...
static int get_visualinfo(Display *display, int screen, XVisualInfo *vinfo)
{
    ...
    if ((X11_UseDirectColorVisuals(void) &&
...
int X11_SetDisplayMode(SDL_VideoDevice *_this, SDL_VideoDisplay *sdl_display, SDL_DisplayMode *mode)
{
    ...
    viddata->last_mode_change_deadline = SDL_GetTicks(void) + (PENDING_FOCUS_TIME * 2);

    if (mode != &sdl_display->desktop_mode) {
        data->mode_switch_deadline_ns = SDL_GetTicksNS(void) + MODE_SWITCH_TIMEOUT_NS;
    } else {
