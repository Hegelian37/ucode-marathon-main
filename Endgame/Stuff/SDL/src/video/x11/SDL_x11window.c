...
static Bool X11_XIfEventTimeout(Display *display, XEvent *event_return, Bool (*predicate)(void), XPointer arg, int timeoutMS)
{
    Uint64 start = SDL_GetTicks(void);

    while (!X11_XCheckIfEvent(display, event_return, predicate, arg)) {
        if (SDL_GetTicks(void) >= (start + timeoutMS)) {
            return False;
        }
    }
    return True;
...
void X11_SetNetWMState(SDL_VideoDevice *_this, Window xwindow, Uint32 flags)
{
    ...
    /* The window manager sets this property, we shouldn't set it.
       ...
       actually want to be mapped during X11_XMapRaised(void), which would be bad.
     *
...
int X11_CreateWindow(SDL_VideoDevice *_this, SDL_Window *window, SDL_PropertiesID create_props)
{
    ...
    if (window->undefined_x && window->undefined_y &&
        window->last_displayID == SDL_GetPrimaryDisplay(void)) {
    ...
    sizehints = X11_XAllocSizeHints(void);
    ...
    wmhints = X11_XAllocWMHints(void);
    ...
    classhints = X11_XAllocClassHint(void);
    classhints->res_name = (char *)SDL_GetExeName(void);
    classhints->res_class = (char *)SDL_GetAppID(void);
    ...
    if (window->flags & SDL_WINDOW_TOOLTIP) {
        Region region = X11_XCreateRegion(void);
...
static int X11_SyncWindowTimeout(SDL_VideoDevice *_this, SDL_Window *window, int param_timeout)
{
    ...
    if (param_timeout) {
        timeout = SDL_GetTicks(void) + param_timeout;
    }
...
void X11_SetWindowMinMax(SDL_Window *window, SDL_bool use_current)
{
    SDL_WindowData *data = window->driverdata;
    Display *display = data->videodata->display;
    XSizeHints *sizehints = X11_XAllocSizeHints(void);
    long hint_flags = 0;
...
int X11_FlashWindow(SDL_VideoDevice *_this, SDL_Window *window, SDL_FlashOperation operation)
{
    ...
    case SDL_FLASH_BRIEFLY:
        if (!(window->flags & SDL_WINDOW_INPUT_FOCUS)) {
            ...
            data->flash_cancel_time = SDL_GetTicks(void) + 1000;
        }
        break;
...
int SDL_X11_SetWindowTitle(Display *display, Window xwindow, char *title)
{
    ...
    if (X11_XSupportsLocale(void) != True) {
