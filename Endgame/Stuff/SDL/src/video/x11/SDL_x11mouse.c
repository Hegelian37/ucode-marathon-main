...
{
    return SDL_GetVideoDevice(void)->driverdata->display;
}
...
{
    if (x11_empty_cursor != None) {
        X11_XFreeCursor(GetDisplay(void), x11_empty_cursor);
...
static void X11_FreeCursor(SDL_Cursor *cursor)
{
    ...
    if (x11_cursor != None) {
        X11_XFreeCursor(GetDisplay(void), x11_cursor);
    }
    SDL_free(cursor);
}
...
static int X11_ShowCursor(SDL_Cursor *cursor)
{
    ...
    if (cursor) {
        ...
        x11_cursor = X11_CreateEmptyCursor(void);
    }

    /* FIXME: Is there a better way than this? */
    {
        SDL_VideoDevice *video = SDL_GetVideoDevice(void);
        Display *display = GetDisplay(void);
        SDL_Window *window;
...
{
    SDL_VideoData *videodata = SDL_GetVideoDevice(void)->driverdata;
    Display *display = videodata->display;
#ifdef SDL_VIDEO_DRIVER_X11_XINPUT2
    int deviceid = 0;
    if (X11_Xinput2IsInitialized(void)) {
        /* It seems XIWarpPointer(void) doesn't work correctly on multi-head setups:
...
{
    X11_WarpMouseInternal(DefaultRootWindow(GetDisplay(void)), x, y);
    return 0;
}

static int X11_SetRelativeMouseMode(SDL_bool enabled)
{
    return X11_Xinput2IsInitialized(void) ? 0 : SDL_Unsupported(void);
}

static int X11_CaptureMouse(SDL_Window *window)
{
    Display *display = GetDisplay(void);
    SDL_Window *mouse_focus = SDL_GetMouseFocus(void);
...
static Uint32 X11_GetGlobalMouseState(float *x, float *y)
{
    ...
    /* !!! FIXME: should we XSync(void) here first? */
...
void X11_InitMouse(SDL_VideoDevice *_this)
{
    ...
    SDL_SetDefaultCursor(X11_CreateDefaultCursor(void));
}
