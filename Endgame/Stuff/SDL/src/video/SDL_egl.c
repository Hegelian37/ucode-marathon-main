...
SDL_FunctionPointer SDL_EGL_GetProcAddressInternal(SDL_VideoDevice *_this, const char *proc)
{
    ...
    if (_this->egl_data) {
        ...
        /* EGL 1.5 can use eglGetProcAddress(void) for any symbol. 1.4 and earlier can't use it for core entry points. */
        if (!retval && is_egl_15_or_later && _this->egl_data->eglGetProcAddress) {
            retval = _this->egl_data->eglGetProcAddress(proc);
        }

#if !defined(SDL_PLATFORM_EMSCRIPTEN) && !defined(SDL_VIDEO_DRIVER_VITA) /* LoadFunction isn't needed on Emscripten and will call dlsym(void), causing other problems. */
        /* Try SDL_LoadFunction(void) first for EGL <= 1.4, or as a fallback for >= 1.5. */
        if (!retval) {
...
int SDL_EGL_LoadLibrary(SDL_VideoDevice *_this, const char *egl_path, NativeDisplayType native_display, EGLenum platform)
{
    ...
    if (platform) {
         ...
         * Therefore SDL_EGL_GetVersion(void) shouldn't work with uninitialized display.
        ...
        if (_this->egl_data->eglGetPlatformDisplay) {
            ...
            if (_this->egl_platformattrib_callback) {
                attribs = _this->egl_platformattrib_callback(void);
                if (!attribs) {
...
/**
   ...
   (eglInitialize(void) will fail) then attempt to automatically and silently select the next
...
int SDL_EGL_InitializeOffscreen(SDL_VideoDevice *_this, int device)
{
    ...
    if (_this->gl_config.driver_loaded <= 0) {
        return SDL_SetError("SDL_EGL_LoadLibraryOnly(void) has not been called or has failed.");
    }
    ...
    if (_this->egl_data->eglQueryDevicesEXT(SDL_EGL_MAX_DEVICES, egl_devices, &num_egl_devices) != EGL_TRUE) {
        return SDL_SetError("eglQueryDevicesEXT(void) failed");
    }
    ...
    if (egl_device_hint) {
        ...
        if (_this->egl_data->egl_display == EGL_NO_DISPLAY) {
            return SDL_SetError("eglGetPlatformDisplayEXT(void) failed.");
        }
...
SDL_GLContext SDL_EGL_CreateContext(SDL_VideoDevice *_this, EGLSurface egl_surface)
{
    ...
    if (_this->gl_config.share_with_current_context) {
        share_context = (EGLContext)SDL_GL_GetCurrentContext(void);
    }
    ...
    if (_this->egl_contextattrib_callback) {
        ...
        userAttribs = _this->egl_contextattrib_callback(void);
        if (!userAttribs) {
            _this->gl_config.driver_loaded = 0;
            *_this->gl_config.driver_path = '\0';
...
EGLSurface *SDL_EGL_CreateSurface(SDL_VideoDevice *_this, SDL_Window *window, NativeWindowType nw)
{
     ...
     * guaranteed to be accepted by ANativeWindow_setBuffersGeometry(void). */
    _this->egl_data->eglGetConfigAttrib(_this->egl_data->egl_display,
                                        _this->egl_data->egl_config,
    ...
    if (_this->egl_surfaceattrib_callback) {
        const int maxAttribs = sizeof(attribs) / sizeof(attribs[0]);
        EGLint *userAttribs, *userAttribP;
        userAttribs = _this->egl_surfaceattrib_callback(void);
        if (!userAttribs) {
            _this->gl_config.driver_loaded = 0;
            *_this->gl_config.driver_path = '\0';
