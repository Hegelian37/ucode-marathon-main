...
{
    /* Just pretend you do this here, but don't do it until KMSDRM_CreateWindow(void),
       where we do the same library load we would normally do here.
       because this gets called by SDL_CreateWindow(void) before KMSDR_CreateWindow(void),
       so gbm dev isn't yet created when this is called, AND we can't alter the
       call order in SDL_CreateWindow(void). */
#if 0
...
{
    /* As with KMSDRM_GLES_LoadLibrary(void), we define our own "dummy" unloading function
...
int KMSDRM_GLES_SwapWindow(SDL_VideoDevice *_this, SDL_Window *window)
{
    ...
    if (!windata->bo) {
        ...
        /* On subsequent swaps, queue the new front buffer to be flipped during
           ...
           Remember: drmModePageFlip(void) never blocks, it just issues the flip,
           which will be done during the next vblank, or immediately if
           we pass the DRM_MODE_PAGE_FLIP_ASYNC flag.
           Since calling drmModePageFlip(void) will return EBUSY if we call it
