        ...
        if (SDL_memcmp(res->d_name, kmsdrm_dri_devname,
            ...
            if (drm_fd >= 0) {
                devindex = SDL_atoi(device + kmsdrm_dri_pathsize +
                                    kmsdrm_dri_devnamesize);
                if (SDL_KMSDRM_LoadSymbols(void)) {
...
static SDL_VideoDevice *KMSDRM_CreateDevice(void)
{
    ...
    if (!KMSDRM_Available(void)) {
        return NULL;
    }

    devindex = get_driindex(void);
    if (devindex < 0) {
...
SDL_bool KMSDRM_WaitPageflip(SDL_VideoDevice *_this, SDL_WindowData *windata)
{
    ...
    /* Stay on the while loop until we get the desired event.
       ...
       -The event is of POLLIN type, but even then, if the event is not a pageflip,
        drmHandleEvent(void) won't unset wait_for_pageflip, so we have to iterate
      ...
      we are waiting for, so the drmEventHandle(void) doesn't run the flip handler, and
    ...
    */
    while (windata->waiting_for_flip) {
        ...
        /* poll(void) waits for events arriving on the FD, and returns < 0 if timeout passes
        ...
        if (ret < 0) {
            if (errno == EINTR) {
                /* poll(void) returning < 0 and setting errno = EINTR means there was a signal before
        ...
        if (pfd.revents & POLLIN) {
            /* There is data to read on the FD!
               ...
               event we are passing in &ev. If it does, drmHandleEvent(void) will unset
        ...
        /* If we got to this point in the loop, we may iterate or exit the loop:
           -A legit (non-error) event arrived, and it was a POLLING event, and it was consumed
            by drmHandleEvent(void).
              -If it was a PAGEFLIP event, waiting_for_flip will be unset by drmHandleEvent(void)
               and we will exit the loop.
              -If it wasn't a PAGEFLIP, drmHandleEvent(void) won't unset waiting_for_flip, so we
               iterare back to polling.
           -A legit (non-error) event arrived, but it's not a POLLIN event, so it hasn't to be
            consumed by drmHandleEvent(void), so waiting_for_flip isn't set and we iterate back
            to polling. */
    }
...
static void KMSDRM_AddDisplay(SDL_VideoDevice *_this, drmModeConnector *connector, drmModeRes *resources)
{
    ...
    /* Since we create and show the default cursor on KMSDRM_InitMouse(void),
       and we call KMSDRM_InitMouse(void) when we create a window, we have to know
...
/* Initializes the list of SDL displays: we build a new display for each
   ...
   This is to be called early, in VideoInit(void), because it gets us
   the videomode information, which SDL needs immediately after VideoInit(void). */
static int KMSDRM_InitDisplays(SDL_VideoDevice *_this)
{
    ...
    for (i = 0; i < resources->count_connectors; i++) {
        ...
        if (connector->connection == DRM_MODE_CONNECTED && connector->count_modes) {
            /* If it's a connected connector with available videomodes, try to add
               an SDL Display representing it. KMSDRM_AddDisplay(void) is purposely void,
    ...
    if (SDL_GetPrimaryDisplay(void) == 0) {
...
/* Init the Vulkan-INCOMPATIBLE stuff:
   ...
   This is to be called late, in WindowCreate(void), and ONLY if this is not
   a Vulkan window.
...
static void KMSDRM_GBMDeinit(SDL_VideoDevice *_this, SDL_DisplayData *dispdata)
{
    ...
    /* Destroy GBM device. GBM surface is destroyed by DestroySurfaces(void),
...
int KMSDRM_CreateSurfaces(SDL_VideoDevice *_this, SDL_Window *window)
{
    ...
    */
    ...
    /* Current context passing to EGL is now done here. If something fails,
       go back to delayed SDL_EGL_MakeCurrent(void) call in SwapWindow. */
    egl_context = (EGLContext)SDL_GL_GetCurrentContext(void);
...
int KMSDRM_VideoInit(SDL_VideoDevice *_this)
{
    ...
    SDL_VideoData *viddata = _this->driverdata;
    SDL_LogDebug(SDL_LOG_CATEGORY_VIDEO, "KMSDRM_VideoInit(void)");
    ...
    /* Get KMSDRM resources info and store what we need. Getting and storing
       ...
       For VK-incompatible initializations we have KMSDRM_GBMInit(void), which is
...
int KMSDRM_SetDisplayMode(SDL_VideoDevice *_this, SDL_VideoDisplay *display, SDL_DisplayMode *mode)
{
    /* Set the dispdata->mode to the new mode and leave actual modesetting
       pending to be done on SwapWindow(void) via drmModeSetCrtc(void) */
...
void KMSDRM_DestroyWindow(SDL_VideoDevice *_this, SDL_Window *window)
{
    ...
    if (!is_vulkan && viddata->gbm_init) {
        ...
        /* Unload library and deinit GBM, but only if this is the last window.
           ...
           if there is a complete window, or 0 if we got here from SDL_CreateWindow(void)
           because KMSDRM_CreateWindow(void) returned an error so the window wasn't
...
/* reflect it: if it's fullscreen, KMSDRM_SetWindwoFullscreen(void) will  */
...
int KMSDRM_CreateWindow(SDL_VideoDevice *_this, SDL_Window *window, SDL_PropertiesID create_props)
{
    ...
    if (!is_vulkan && !vulkan_mode) { /* NON-Vulkan block. */
        ...
        if (!(viddata->gbm_init)) {
            ...
            /* After SDL_CreateWindow, most SDL programs will do SDL_CreateRenderer(void),
               which will in turn call GL_CreateRenderer(void) or GLES2_CreateRenderer(void).
               In order for the GL_CreateRenderer(void) or GLES2_CreateRenderer(void) call to
               ...
               -Load the GL library (which can't be done until the GBM device has been
                created, so we have to do it here instead of doing it on VideoInit(void))
                and mark it as loaded by setting gl_config.driver_loaded to 1.
               So if you ever see KMSDRM_CreateWindow(void) to be called two times in tests,
               don't be shy to debug GL_CreateRenderer(void) or GLES2_CreateRenderer(void)
               to find out why!
             */
        ...
        /* Manually load the GL library. KMSDRM_EGL_LoadLibrary(void) has already
           been called by SDL_CreateWindow(void) but we don't do anything there,
           our KMSDRM_EGL_LoadLibrary(void) is a dummy precisely to be able to load it here.
           If we let SDL_CreateWindow(void) load the lib, it would be loaded
           before we call KMSDRM_GBMInit(void), causing all GLES programs to fail. */
        if (!_this->egl_data) {
        ...
        /* The FULLSCREEN flags are cut out from window->flags at this point,
           ...
           KMSDRM_SetWindowFullscreen(void) to reconfigure it if necessary. */
    ...
    /* Focus on the newly created window.
       SDL_SetMouseFocus(void) also takes care of calling KMSDRM_ShowCursor(void) if necessary. */
    ...
    /* Allocated windata will be freed in KMSDRM_DestroyWindow,
       and KMSDRM_DestroyWindow(void) will be called by SDL_CreateWindow(void)
