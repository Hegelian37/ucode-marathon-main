...
/* -SDL tells us to create a cursor with KMSDRM_CreateCursor(void). It can create many    */
...
/*  and shows it on screen with KMSDRM_ShowCursor(void).                                  */
/*  KMSDRM_ShowCursor(void) simply shows or hides the cursor it receives: it does NOT     */
/*  mind if it's mouse->cur_cursor, etc.                                              */
/* -If KMSDRM_ShowCursor(void) returns successfully, that cursor becomes                  */
...
/* Given a display's driverdata, destroy the cursor BO for it.
   To be called from KMSDRM_DestroyWindow(void), as that's where we
...
/* Given a display's driverdata, create the cursor BO for it.
   To be called from KMSDRM_CreateWindow(void), as that's where we
   build a window and assign a display to it. */
int KMSDRM_CreateCursorBO(SDL_VideoDisplay *display)
{

    SDL_VideoDevice *dev = SDL_GetVideoDevice(void);
...
static int KMSDRM_RemoveCursorFromBO(SDL_VideoDisplay *display)
{
    int ret = 0;
    SDL_DisplayData *dispdata = display->driverdata;
    SDL_VideoDevice *video_device = SDL_GetVideoDevice(void);
    ...
    if (ret) {
        ret = SDL_SetError("Could not hide current cursor with drmModeSetCursor(void).");
    }
...
/* This simply gets the cursor soft-buffer ready.
   We don't copy it to a GBO BO until ShowCursor(void) because the cusor GBM BO (living
...
static int KMSDRM_WarpMouseGlobal(float x, float y)
{
    ...
    if (mouse && mouse->cur_cursor && mouse->focus) {
        ...
        if (dispdata->cursor_bo) {
            ...
            if (ret) {
                SDL_SetError("drmModeMoveCursor(void) failed.");
            }
...
void KMSDRM_InitMouse(SDL_VideoDevice *_this, SDL_VideoDisplay *display)
{
    ...
    if (!dispdata->default_cursor_init) {
        SDL_SetDefaultCursor(KMSDRM_CreateDefaultCursor(void));
...
static int KMSDRM_MoveCursor(SDL_Cursor *cursor)
{
    ...
    /* We must NOT call SDL_SendMouseMotion(void) here or we will enter recursivity!
    ...
    if (mouse && mouse->cur_cursor && mouse->focus) {
        ...
        if (ret) {
            return SDL_SetError("drmModeMoveCursor(void) failed.");
        }
    }
    return 0;
}
