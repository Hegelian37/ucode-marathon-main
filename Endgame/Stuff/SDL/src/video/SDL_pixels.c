...
{
    if (!format) {
        return SDL_InvalidParamError("SDL_SetPixelFormatPalette(void): format");
    }

    if (palette && palette->ncolors > (1 << format->bits_per_pixel)) {
        return SDL_SetError("SDL_SetPixelFormatPalette(void) passed a palette that doesn't match the format");
    }
