...
int VITA_ShowMessageBox(const SDL_MessageBoxData *messageboxdata, int *buttonID)
{
    ...
    if (init_result == SCE_COMMON_DIALOG_ERROR_GXM_IS_UNINITIALIZED) {
        gxm_minimal_init_for_common_dialog(void);
        init_result = sceMsgDialogInit(&param);
        setup_minimal_gxm = SDL_TRUE;
    }

    gxm_init_for_common_dialog(void);

    if (init_result >= 0) {
        while (sceMsgDialogGetStatus(void) == SCE_COMMON_DIALOG_STATUS_RUNNING) {
            gxm_swap_for_common_dialog(void);
        }
        ...
        sceMsgDialogTerm(void);
    } else {
        return -1;
    }

    gxm_term_for_common_dialog(void);

    if (setup_minimal_gxm) {
        gxm_minimal_term_for_common_dialog(void);
    }
