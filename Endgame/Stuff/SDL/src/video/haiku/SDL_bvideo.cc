...
static SDL_Cursor * HAIKU_CreateDefaultCursor(void)
{
...
{
    SDL_Window *window = SDL_GetMouseFocus(void);
    if (!window) {
      return 0;
    }

	SDL_BWin *bewin = _ToBeWin(window);
	BGLView *_SDL_GLView = bewin->GetGLView(void);
...
static void HAIKU_MouseInit(SDL_VideoDevice *_this)
{
	...
	SDL_SetDefaultCursor(HAIKU_CreateDefaultCursor(void));
}

int HAIKU_VideoInit(SDL_VideoDevice *_this)
{
    /* Initialize the Be Application for appserver interaction */
    if (SDL_InitBeApp(void) < 0) {
        return -1;
    }
