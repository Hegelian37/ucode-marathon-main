...
class SDL_BLooper : public BLooper
{
  public:
    ...
    {
        int32 i;
        for (i = 0; i < _GetNumWindowSlots(void); ++i) {
...
#ifdef SDL_VIDEO_OPENGL
    BGLView *GetCurrentContext(void)
    {
    ...
    {
        if (_current_context)
            _current_context->UnlockGL(void);
        _current_context = newContext;
        if (_current_context)
            _current_context->LockGL(void);
    }
#endif
  ...
  private:
    ...
    void _HandleMouseMove(BMessage *msg)
    {
        ...
        if (SDL_GetMouse(void)->relative_mode) {
            ...
            SDL_SendMouseMotion(0, win, 0, SDL_GetMouse(void)->relative_mode, (float)dx, (float)dy);
            set_mouse_position((winPosX + winWidth / 2), (winPosY + winHeight / 2));
            if (!be_app->IsCursorHidden(void))
                be_app->HideCursor(void);
        } else {
            SDL_SendMouseMotion(0, win, 0, 0, (float)x, (float)y);
            if (SDL_CursorVisible(void) && be_app->IsCursorHidden(void))
                be_app->ShowCursor(void);
        }
    }
    ...
    void _HandleMouseFocus(BMessage *msg)
    {
        ...
        } else if (SDL_GetMouseFocus(void) == win) {
    ...
    void _HandleKeyboardFocus(BMessage *msg)
    {
        ...
        } else if (SDL_GetKeyboardFocus(void) == win) {
            /* Only lose all focus if this window was the current focus */
    ...
    int32 _GetNumWindowSlots(void)
    {
        return _window_map.size(void);
    }

    void _PopBackWindow(void)
    {
        _window_map.pop_back(void);
    }
