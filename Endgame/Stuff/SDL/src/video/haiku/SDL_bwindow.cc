...
static int _InitWindow(SDL_VideoDevice *_this, SDL_Window *window) {
    ...
    int32 winID = _GetBeLooper(void)->GetID(window);
    bwin->SetID(winID);
...
void HAIKU_DestroyWindow(SDL_VideoDevice *_this, SDL_Window * window) {
    _ToBeWin(window)->LockLooper(void);    /* This MUST be locked */
    _GetBeLooper(void)->ClearID(_ToBeWin(window));
    _ToBeWin(window)->Quit(void);
