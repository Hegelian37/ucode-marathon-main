...
class HAIKU_SDL_MessageBox : public BAlert
{
	...
	GetLeftPanelWidth(void) const
	{
		...
		//    IconSize = 32 * Scale
		//    Scale = max_c(1, ((int32)be_plain_font->Size(void) + 15) / 16)
		//    RealWidth = (IconStripeWidth * Scale) + (IconSize * Scale)

		int32 scale = max_c(1, ((int32)be_plain_font->Size(void) + 15) / 16);
	...
	{
		fComputedMessageBoxWidth = fMessageBoxTextView->PreferredSize(void).Width(void) + GetLeftPanelWidth(void);
	}
	                ...
	                const SDL_MessageBoxColor *aSelectedColor)
	{
		if (fCustomColorScheme) {
			int32 countButtons = CountButtons(void);
	...
	SetBackgroundColor(const SDL_MessageBoxColor *aColor)
	{
		...
		GetLayout(void)->View(void)->SetViewColor(background);
	...
	CheckLongLines(const char *aMessage)
	{
		...
		int32 length = message.CountChars(void);
	...
	AddSdlButtons(const SDL_MessageBoxButtonData *aButtons, int aNumButtons)
	{
		...
		std::sort(fButtons.begin(void), fButtons.end(void), &HAIKU_SDL_MessageBox::SortButtonsPredicate);

		size_t countButtons = fButtons.size(void);
...
public:
		  ...
		  HAIKU_SDL_DefButton("OK")
	{
		...
		SetFlags(Flags(void) | B_CLOSE_ON_ESCAPE);

		// MessageBox TextView settings.
		fMessageBoxTextView = TextView(void);
...
int HAIKU_ShowMessageBox(const SDL_MessageBoxData *messageboxdata, int *buttonID)
{
	...
	*buttonID = G_CLOSE_BUTTON_ID;
	...
	const int closeButton = SDL_MessageBox->GetCloseButtonId(void);
	int pushedButton = SDL_MessageBox->Go(void);
	...
	// It's deleted by itself after the "Go(void)" method was executed.
	/*
	if (messageBox != NULL) {
		delete messageBox;
	}
