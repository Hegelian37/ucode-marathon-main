...
int HAIKU_GL_SwapWindow(SDL_VideoDevice *_this, SDL_Window * window) {
    _ToBeWin(window)->SwapBuffers(void);
    return 0;
}
...
int HAIKU_GL_MakeCurrent(SDL_VideoDevice *_this, SDL_Window * window, SDL_GLContext context) {
    ...
    if (glView) {
        if ((glView->Window(void) == NULL) || (!window) || (_ToBeWin(window)->GetGLView(void) != glView)) {
            return SDL_SetError("MakeCurrent failed");
        }
    }
    _GetBeLooper(void)->SetCurrentContext(glView);
    return 0;
}
...
SDL_GLContext HAIKU_GL_CreateContext(SDL_VideoDevice *_this, SDL_Window * window) {
    ...
    _GetBeLooper(void)->SetCurrentContext(bwin->GetGLView(void));
    return (SDL_GLContext)(bwin->GetGLView(void));
}
...
int HAIKU_GL_DeleteContext(SDL_VideoDevice *_this, SDL_GLContext context) {
    ...
    BGLView* glView = (BGLView*)context;
    SDL_BWin *bwin = (SDL_BWin*)glView->Window(void);
    if (!bwin) {
        delete glView;
    } else {
        bwin->RemoveGLView(void);
    }
    return 0;
}
...
void HAIKU_GL_RebootContexts(SDL_VideoDevice *_this) {
    ...
    while (window) {
        SDL_BWin *bwin = _ToBeWin(window);
        if (bwin->GetGLView(void)) {
            bwin->LockLooper(void);
            bwin->RemoveGLView(void);
            bwin->CreateGLView(bwin->GetGLType(void));
            bwin->UnlockLooper(void);
        }
        window = window->next;
    }
}
