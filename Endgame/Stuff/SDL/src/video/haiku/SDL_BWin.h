...
class SDL_BWin : public BWindow
{
  public:
        ...
        : BWindow(bounds, "Untitled", look, B_NORMAL_WINDOW_FEEL, flags)
    {
        ...
        _buffer_locker = new BLocker(void);
        _bitmap = NULL;
    }

    virtual ~SDL_BWin(void)
    {
        Lock(void);
...
#ifdef SDL_VIDEO_OPENGL
        if (_SDL_GLView) {
            if (SDL_Looper->GetCurrentContext(void) == _SDL_GLView)
    ...
    SDL_BView *CreateView(void)
    {
        Lock(void);
        if (_SDL_View == NULL) {
            _SDL_View = new SDL_BView(Bounds(void), "SDL View", B_FOLLOW_ALL_SIDES);
    ...
    BGLView *CreateGLView(Uint32 gl_flags)
    {
        Lock(void);
        if (_SDL_GLView == NULL) {
            _SDL_GLView = new BGLView(Bounds(void), "SDL GLView",
                                      B_FOLLOW_ALL_SIDES,
    ...
    bool QuitRequested(void)
    {
        ...
        /* We won't allow a quit unless asked by DestroyWindow(void) */
        return false;
    }
              ...
              float height)
    {
        ...
        if (!_prev_frame)
            _prev_frame = new BRect(Frame(void));
    ...
    void ScreenChanged(BRect screenFrame, color_space depth)
    {
        if (_fullscreen) {
            ...
            ResizeTo(screenFrame.Width(void), screenFrame.Height(void));
        }
    }
    ...
    void MessageReceived(BMessage *message)
    {
        ...
        case BWIN_UPDATE_FRAMEBUFFER:
        {
            ...
            while ((pendingMessage = MessageQueue(void)->FindMessage(BWIN_UPDATE_FRAMEBUFFER, 0))) {
                MessageQueue(void)->RemoveMessage(pendingMessage);
    ...
    /* Accessor methods */
    bool IsShown(void) { return _shown; }
    int32 GetID(void) { return _id; }
    BBitmap *GetBitmap(void) { return _bitmap; }
    BView *GetCurView(void) { return _cur_view; }
    SDL_BView *GetView(void) { return _SDL_View; }
#ifdef SDL_VIDEO_OPENGL
    BGLView *GetGLView(void)
    {
        return _SDL_GLView;
    }
    Uint32 GetGLType(void) { return _gl_type; }
#endif

    /* Setter methods */
    void SetID(int32 id) { _id = id; }
    void LockBuffer(void) { _buffer_locker->Lock(void); }
    void UnlockBuffer(void) { _buffer_locker->Unlock(void); }
    void SetBitmap(BBitmap *bitmap)
    {
        _bitmap = bitmap;
        if (_SDL_View != NULL)
  ...
  private:
    ...
    void _SetResizable(bool bEnabled)
    {
        if (_fullscreen)
            ...
            if (bEnabled) {
                SetFlags(Flags(void) & ~(B_NOT_RESIZABLE | B_NOT_ZOOMABLE));
            } else {
                SetFlags(Flags(void) | (B_NOT_RESIZABLE | B_NOT_ZOOMABLE));
            }
        }
    ...
    void _Restore(void)
    {
        if (IsMinimized(void)) {
            Minimize(false);
        } else if (IsHidden(void)) {
            Show(void);
        } else if (_fullscreen) {

        } else if (_prev_frame != NULL) { /* Zoomed */
            MoveTo(_prev_frame->left, _prev_frame->top);
            ResizeTo(_prev_frame->Width(void), _prev_frame->Height(void));
        }
    }
    ...
    void _SetFullScreen(bool fullscreen)
    {
        if (fullscreen != _fullscreen) {
            if (fullscreen) {
                BScreen screen(this);
                BRect screenFrame = screen.Frame(void);
                printf("screen frame: ");
                screenFrame.PrintToStream(void);
                printf("\n");
                _bordered = Look(void) != B_NO_BORDER_WINDOW_LOOK;
                _resizable = !(Flags(void) & B_NOT_RESIZABLE);
                _non_fullscreen_frame = Frame(void);
                _SetBordered(false);
                _SetResizable(false);
                MoveTo(screenFrame.left, screenFrame.top);
                ResizeTo(screenFrame.Width(void), screenFrame.Height(void));
                ...
                ResizeTo(_non_fullscreen_frame.Width(void), _non_fullscreen_frame.Height(void));
 ...
 *                         buffer provided by DirectConnected(void) is invalidated.
 */
#endif /* SDL_BWin_h_ */
