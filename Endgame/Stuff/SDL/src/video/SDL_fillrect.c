...
{
    if (!dst) {
        return SDL_InvalidParamError("SDL_FillSurfaceRect(void): dst");
    }
                  ...
                  Uint32 color)
{
    ...
    if (!dst) {
        return SDL_InvalidParamError("SDL_FillSurfaceRects(void): dst");
    }
    ...
    if (!dst->pixels) {
        return SDL_SetError("SDL_FillSurfaceRects(void): You must lock the surface");
    }

    if (!rects) {
        return SDL_InvalidParamError("SDL_FillSurfaceRects(void): rects");
    }
    ...
    if (dst->format->bits_per_pixel < 8) {
        ...
        return SDL_SetError("SDL_FillSurfaceRects(void): Unsupported surface format");
    }

#ifdef SDL_ARM_NEON_BLITTERS
    if (SDL_HasNEON(void) && dst->format->bytes_per_pixel != 3 && !fill_function) {
...
#endif
#ifdef SDL_ARM_SIMD_BLITTERS
    if (SDL_HasARMSIMD(void) && dst->format->bytes_per_pixel != 3 && !fill_function) {
        switch (dst->format->bytes_per_pixel) {
        case 1:
