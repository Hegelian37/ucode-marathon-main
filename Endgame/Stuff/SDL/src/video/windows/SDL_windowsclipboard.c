...
static int WIN_SetClipboardImage(SDL_VideoDevice *_this)
{
    ...
    if (hMem) {
        ...
        /* WIN_ConvertBMPtoDIB(void) set the error */
        result = -1;
    }
    return result;
}
...
int WIN_SetClipboardData(SDL_VideoDevice *_this)
{
    ...
    if (WIN_OpenClipboard(_this)) {
        ...
        data->clipboard_count = GetClipboardSequenceNumber(void);
        WIN_CloseClipboard(void);
    } else {
...
{
    const DWORD count = GetClipboardSequenceNumber(void);
