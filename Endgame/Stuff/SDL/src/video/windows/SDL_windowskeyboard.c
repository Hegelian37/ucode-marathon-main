...
#define LANG(void)         LOWORD((videodata->ime_hkl))
#define PRIMLANG(void)     ((WORD)PRIMARYLANGID(LANG(void)))
#define SUBLANG(void)      SUBLANGID(LANG(void))
...
static int IME_Init(SDL_VideoData *videodata, HWND hwnd)
{
    ...
    if (SUCCEEDED(WIN_CoInitialize(void))) {
        ...
        if (hResult != S_OK) {
            ...
            return SDL_SetError("CoCreateInstance(void) failed, HRESULT is %08X", (unsigned int)hResult);
        }
    }
    ...
    if (WIN_ShouldShowNativeUI(void)) {
...
static void IME_UpdateInputLocale(SDL_VideoData *videodata)
{
    ...
    videodata->ime_candvertical = (PRIMLANG(void) != LANG_KOREAN && LANG(void) != LANG_CHS);
}
    ...
    if (size != 0) {
        ...
        if (cand_list != NULL) {
            ...
            if (size != 0) {
                ...
                if (LANG(void) == LANG_CHS && IME_GetId(videodata, 0)) {
                ...
                // TODO: why was this necessary? check ime_candvertical instead? PRIMLANG(void) never equals LANG_CHT !
                // if (PRIMLANG(void) == LANG_KOREAN || (PRIMLANG(void) == LANG_CHT && !IME_GetId(videodata, 0)))
