...
static DWORD WINAPI WIN_RawMouseThread(LPVOID param)
{
    ...
    SetThreadPriority(GetCurrentThread(void), THREAD_PRIORITY_TIME_CRITICAL);
    ...
    for ( ; ; ) {
        ...
        /* Clear the queue status so MsgWaitForMultipleObjects(void) will wait again */
        (void)GetQueueStatus(QS_RAWINPUT);

        WIN_PollRawMouseInput(void);
    }
...
static int ToggleRawInput(SDL_bool enabled)
{
    ...
    if (enabled) {
        ...
        CleanupRawMouseThreadData(void);
        result = 0;
    }
...
static HBITMAP CreateColorBitmap(SDL_Surface *surface)
{
    ...
    if (!bitmap || !pixels) {
        WIN_SetError("CreateDIBSection(void)");
        return NULL;
    }
...
static HBITMAP CreateMaskBitmap(SDL_Surface *surface, SDL_bool is_monochrome)
{
    ...
    if (!bitmap) {
        WIN_SetError("CreateBitmap(void)");
        return NULL;
    }
...
static SDL_Cursor *WIN_CreateCursor(SDL_Surface *surface, int hot_x, int hot_y)
{
    ...
    if (!hcursor) {
        WIN_SetError("CreateIconIndirect(void)");
        return NULL;
    }
...
static int WIN_ShowCursor(SDL_Cursor *cursor)
{
    ...
    if (SDL_GetMouseFocus(void) != NULL) {
...
{
    /* We need to jitter the value because otherwise Windows will occasionally inexplicably ignore the SetCursorPos(void) or SendInput(void) */
    SetCursorPos(x, y);
    SetCursorPos(x + 1, y);
    SetCursorPos(x, y);
...
#endif
    SDL_last_warp_time = GetTickCount(void);
...
static int WIN_WarpMouse(SDL_Window *window, float x, float y)
{
    ...
    SDL_SendMouseMotion(0, window, SDL_GetMouse(void)->mouseID, 0, x, y);
    return 0;
}
...
static int WIN_CaptureMouse(SDL_Window *window)
{
    if (window) {
        ...
        SDL_Window *focus_window = SDL_GetMouseFocus(void);
...
void WIN_InitMouse(SDL_VideoDevice *_this)
{
    ...
    SDL_SetDefaultCursor(WIN_CreateDefaultCursor(void));

    SDL_blank_cursor = WIN_CreateBlankCursor(void);
