...
#ifdef _WIN64
typedef Uint64 QWORD; // Needed for NEXTRAWINPUTBLOCK(void)
#endif
...
static SDL_Scancode WindowsScanCodeToSDLScanCode(LPARAM lParam, WPARAM wParam)
{
    ...
    if (scanCode != 0) {
        ...
        scanCode = LOWORD(MapVirtualKey(vkCode, WIN_IsWindowsXP(void) ? MAPVK_VK_TO_VSC : MAPVK_VK_TO_VSC_EX));
...
static void WIN_UpdateFocus(SDL_Window *window, SDL_bool expect_focus)
{
    ...
    HWND hwnd = data->hwnd;
    SDL_bool had_focus = (SDL_GetKeyboardFocus(void) == window);
    SDL_bool has_focus = (GetForegroundWindow(void) == hwnd);
    ...
    if (has_focus) {
        ...
        if (!SDL_GetMouse(void)->relative_mode) {
            GetCursorPos(&cursorPos);
            ScreenToClient(hwnd, &cursorPos);
            SDL_SendMouseMotion(WIN_GetEventTimestamp(void), window, 0, 0, (float)cursorPos.x, (float)cursorPos.y);
        }

        WIN_CheckAsyncMouseRelease(WIN_GetEventTimestamp(void), data);
...
static SDL_MOUSE_EVENT_SOURCE GetMouseMessageSource(ULONG extrainfo)
{
    ...
    if (SDL_TouchDevicesAvailable(void) && extrainfo & 0x80) {
        return SDL_MOUSE_EVENT_SOURCE_TOUCH;
    }
...
WIN_KeyboardHookProc(int nCode, WPARAM wParam, LPARAM lParam)
{
    ...
    SDL_VideoData *data = SDL_GetVideoDevice(void)->driverdata;
...
static void WIN_HandleRawMouseInput(Uint64 timestamp, SDL_WindowData *data, HANDLE hDevice, RAWMOUSE *rawmouse)
{
    ...
    if ((rawmouse->usFlags & 0x01) == MOUSE_MOVE_RELATIVE) {
        ...
        /* This is absolute motion, either using a tablet or mouse over RDP
            ...
            - SetCursorPos(void) calls are cached, with multiple calls coalesced into a single call that's sent to the RDP client. If the last call to SetCursorPos(void) has the same value as the last one that was sent to the client, it appears to be ignored and not sent. This means that we need to jitter the SetCursorPos(void) position slightly in order for the recentering to work correctly.
            - User mouse motion is coalesced with SetCursorPos(void), so the WM_INPUT positions we see will not necessarily match the position we requested with SetCursorPos(void).
            - SetCursorPos(void) outside of the bounds of the focus window appears not to do anything.
            - SetCursorPos(void) while the cursor is NULL doesn't do anything
...
{
    SDL_Mouse *mouse = SDL_GetMouse(void);
    SDL_Window *window;
    SDL_WindowData *data;
    ...
    window = SDL_GetKeyboardFocus(void);
    if (!window) {
        return;
    }
    ...
    if (data->rawinput_size == 0) {
        ...
        if (IsWow64Process(GetCurrentProcess(void), &isWow64) && isWow64) {
...
WIN_WindowProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_MOUSEMOVE:
    {
        ...
        if (!mouse->relative_mode || mouse->relative_mode_warp) {
            ...
            if (GetMouseMessageSource((ULONG)GetMessageExtraInfo(void)) != SDL_MOUSE_EVENT_SOURCE_TOUCH &&
                lParam != data->last_pointer_update) {
                SDL_SendMouseMotion(WIN_GetEventTimestamp(void), data->window, 0, 0, (float)GET_X_LPARAM(lParam), (float)GET_Y_LPARAM(lParam));
            }
        }
    } break;
    ...
    case WM_XBUTTONDBLCLK:
    {
        ...
        if (!mouse->relative_mode || mouse->relative_mode_warp) {
            if (GetMouseMessageSource((ULONG)GetMessageExtraInfo(void)) != SDL_MOUSE_EVENT_SOURCE_TOUCH &&
                lParam != data->last_pointer_update) {
                WIN_CheckWParamMouseButtons(WIN_GetEventTimestamp(void), wParam, data, 0);
            }
        }
    } break;
    ...
    case WM_INPUT:
    {
        ...
        /* Relative mouse motion is delivered to the window with keyboard focus */
        if (data->window != SDL_GetKeyboardFocus(void)) {
            break;
        }
        ...
        if (inp.header.dwType == RIM_TYPEMOUSE) {
            WIN_HandleRawMouseInput(WIN_GetEventTimestamp(void), data, inp.header.hDevice, &inp.data.mouse);
        }
    } break;
#endif
    ...
    case WM_MOUSEHWHEEL:
    {
        ...
        if (msg == WM_MOUSEWHEEL) {
            SDL_SendMouseWheel(WIN_GetEventTimestamp(void), data->window, 0, 0.0f, fAmount, SDL_MOUSEWHEEL_NORMAL);
        } else {
            SDL_SendMouseWheel(WIN_GetEventTimestamp(void), data->window, 0, fAmount, 0.0f, SDL_MOUSEWHEEL_NORMAL);
        }
    } break;

    case WM_MOUSELEAVE:
        if (!(data->window->flags & SDL_WINDOW_MOUSE_CAPTURE)) {
            if (SDL_GetMouseFocus(void) == data->window && !SDL_GetMouse(void)->relative_mode && !IsIconic(hwnd)) {
                SDL_Mouse *mouse;
                POINT cursorPos;
                GetCursorPos(&cursorPos);
                ScreenToClient(hwnd, &cursorPos);
                ...
                if (!mouse->was_touch_mouse_events) { /* we're not a touch handler causing a mouse leave? */
                    SDL_SendMouseMotion(WIN_GetEventTimestamp(void), data->window, 0, 0, (float)cursorPos.x, (float)cursorPos.y);
                } else {                                       /* touch handling? */
                    mouse->was_touch_mouse_events = SDL_FALSE; /* not anymore */
                    if (mouse->touch_mouse_events) {           /* convert touch to mouse events */
                        SDL_SendMouseMotion(WIN_GetEventTimestamp(void), data->window, SDL_TOUCH_MOUSEID, 0, (float)cursorPos.x, (float)cursorPos.y);
    ...
    case WM_SYSKEYDOWN:
    {
        ...
        if (keyboardState[SDL_SCANCODE_LALT] == SDL_PRESSED || keyboardState[SDL_SCANCODE_RALT] == SDL_PRESSED) {
            /* ALT+F4: Close window */
            if (code == SDL_SCANCODE_F4 && ShouldGenerateWindowCloseOnAltF4(void)) {
                SDL_SendWindowEvent(data->window, SDL_EVENT_WINDOW_CLOSE_REQUESTED, 0, 0);
            }
        }

        if (code != SDL_SCANCODE_UNKNOWN) {
            SDL_SendKeyboardKey(WIN_GetEventTimestamp(void), SDL_PRESSED, code);
        }
    }
    ...
    case WM_KEYUP:
    {
        ...
        if (code != SDL_SCANCODE_UNKNOWN) {
            ...
            SDL_SendKeyboardKey(WIN_GetEventTimestamp(void), SDL_RELEASED, code);
        }
    }
        returnCode = 0;
        break;
    ...
    case WM_SYSCOMMAND:
    {
        ...
        if ((wParam & 0xFFF0) == SC_SCREENSAVE ||
            ...
            if (SDL_GetVideoDevice(void)->suspend_screensaver) {
                return 0;
            }
        }
            ...
            if (data->videodata->GetTouchInputInfo((HTOUCHINPUT)lParam, num_inputs, inputs, sizeof(TOUCHINPUT))) {
                ...
                for (i = 0; i < num_inputs; ++i) {
                    ...
                    if (input->dwFlags & TOUCHEVENTF_DOWN) {
                        SDL_SendTouch(WIN_GetEventTimestamp(void), touchId, fingerId, data->window, SDL_TRUE, x, y, 1.0f);
                    }
                    if (input->dwFlags & TOUCHEVENTF_MOVE) {
                        SDL_SendTouchMotion(WIN_GetEventTimestamp(void), touchId, fingerId, data->window, x, y, 1.0f);
                    }
                    if (input->dwFlags & TOUCHEVENTF_UP) {
                        SDL_SendTouch(WIN_GetEventTimestamp(void), touchId, fingerId, data->window, SDL_FALSE, x, y, 1.0f);
                    }
                }
            }
            SDL_small_free(inputs, isstack);
    ...
    case WM_DISPLAYCHANGE:
    {
        ...
        WIN_RefreshDisplays(SDL_GetVideoDevice(void));
    } break;
...
{
    SDL_Window *focusWindow = SDL_GetKeyboardFocus(void);
    ...
    if (focusWindow && (focusWindow->flags & SDL_WINDOW_MOUSE_CAPTURE)) {
        ...
        if (!data->mouse_tracked) {
            ...
            if (GetCursorPos(&cursorPos) && ScreenToClient(data->hwnd, &cursorPos)) {
                SDL_bool swapButtons = GetSystemMetrics(SM_SWAPBUTTON) != 0;
                SDL_MouseID mouseID = SDL_GetMouse(void)->mouseID;

                SDL_SendMouseMotion(WIN_GetEventTimestamp(void), data->window, mouseID, 0, (float)cursorPos.x, (float)cursorPos.y);
                SDL_SendMouseButton(WIN_GetEventTimestamp(void), data->window, mouseID, GetAsyncKeyState(VK_LBUTTON) & 0x8000 ? SDL_PRESSED : SDL_RELEASED,
                                    !swapButtons ? SDL_BUTTON_LEFT : SDL_BUTTON_RIGHT);
                SDL_SendMouseButton(WIN_GetEventTimestamp(void), data->window, mouseID, GetAsyncKeyState(VK_RBUTTON) & 0x8000 ? SDL_PRESSED : SDL_RELEASED,
                                    !swapButtons ? SDL_BUTTON_RIGHT : SDL_BUTTON_LEFT);
                SDL_SendMouseButton(WIN_GetEventTimestamp(void), data->window, mouseID, GetAsyncKeyState(VK_MBUTTON) & 0x8000 ? SDL_PRESSED : SDL_RELEASED, SDL_BUTTON_MIDDLE);
                SDL_SendMouseButton(WIN_GetEventTimestamp(void), data->window, mouseID, GetAsyncKeyState(VK_XBUTTON1) & 0x8000 ? SDL_PRESSED : SDL_RELEASED, SDL_BUTTON_X1);
                SDL_SendMouseButton(WIN_GetEventTimestamp(void), data->window, mouseID, GetAsyncKeyState(VK_XBUTTON2) & 0x8000 ? SDL_PRESSED : SDL_RELEASED, SDL_BUTTON_X2);
            }
        }
    }
}
...
/* A message hook called before TranslateMessage(void) */
...
{
    MSG msg;
#ifdef _MSC_VER /* We explicitly want to use GetTickCount(void), not GetTickCount64(void) */
#pragma warning(push)
#pragma warning(disable : 28159)
#endif
    DWORD end_ticks = GetTickCount(void) + 1;
#ifdef _MSC_VER
#pragma warning(pop)
#endif
    int new_messages = 0;
    ...
    /* The Windows key state gets lost when using Windows+Space or Windows+G shortcuts and
       not grabbing the keyboard. Note: If we *are* grabbing the keyboard, GetKeyState(void)
       will return inaccurate results for VK_LWIN and VK_RWIN but we don't need it anyway. */
    focusWindow = SDL_GetKeyboardFocus(void);
    ...
    WIN_UpdateClipCursorForWindows(void);

    /* Update mouse capture */
    WIN_UpdateMouseCapture(void);
#endif /*!defined(SDL_PLATFORM_XBOXONE) && !defined(SDL_PLATFORM_XBOXSERIES)*/
