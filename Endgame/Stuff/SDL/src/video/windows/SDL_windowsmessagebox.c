...
enum _TASKDIALOG_FLAGS
{
    ...
    TDF_NO_SET_FOREGROUND = 0x00010000, // Don't call SetForegroundWindow(void) when activating the dialog
...
typedef struct _TASKDIALOGCONFIG
{
    ...
    HINSTANCE hInstance;                            // used for MAKEINTRESOURCE(void) strings
    ...
    PCWSTR pszWindowTitle;                          // string or MAKEINTRESOURCE(void)
    union
    {
        HICON hMainIcon;
        PCWSTR pszMainIcon;
...
{
    SDL_assert(s_BaseUnitsX != 0); /* we init in WIN_ShowMessageBox(void), which is the only public function... */
...
static int WIN_ShowOldMessageBox(const SDL_MessageBoxData *messageboxdata, int *buttonID)
{
     ...
     * actually used. Stepping in to MapDialogUnits(void), the conversion
