...
{
    if (!cursor) {
        cursor = Android_CreateEmptyCursor(void);
    }
    if (cursor) {
        SDL_AndroidCursorData *data = (SDL_AndroidCursorData *)cursor->driverdata;
        ...
        /* SDL error set inside Android_CreateEmptyCursor(void) */
        return -1;
    }
}
...
{
    if (!Android_JNI_SupportsRelativeMouse(void)) {
        return SDL_Unsupported(void);
    }
...
void Android_InitMouse(void)
{
    ...
    SDL_SetDefaultCursor(Android_CreateDefaultCursor(void));
...
{
    Android_DestroyEmptyCursor(void);
}
