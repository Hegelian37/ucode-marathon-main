...
static void android_egl_context_backup(SDL_Window *window)
{
    if (window) {
        ...
        SDL_WindowData *data = window->driverdata;
        data->egl_context = SDL_GL_GetCurrentContext(void);
...
void Android_PumpEvents_Blocking(SDL_VideoDevice *_this)
{
    ...
    if (videodata->isPaused) {
        ...
        ANDROIDAUDIO_PauseDevices(void);
        OPENSLES_PauseDevices(void);
        AAUDIO_PauseDevices(void);
        ...
        if (SDL_WaitSemaphore(Android_ResumeSem) == 0) {
            ...
            ANDROIDAUDIO_ResumeDevices(void);
            OPENSLES_ResumeDevices(void);
            AAUDIO_ResumeDevices(void);
