...
/* These are filled in with real values in Android_SetScreenResolution on init (before SDL_main(void)) */
...
int Android_VideoInit(SDL_VideoDevice *_this)
{
    ...
    display->natural_orientation = Android_JNI_GetDisplayNaturalOrientation(void);
    display->current_orientation = Android_JNI_GetDisplayCurrentOrientation(void);
    display->content_scale = Android_ScreenDensity;

    Android_InitTouch(void);
...
void Android_SendResize(SDL_Window *window)
{
    /*
      ...
      which can happen after VideoInit(void).
    */
