    ...
    if (features == (enum blit_features) - 1) {
        ...
        if (override) {
            ...
            features = (0
                        ...
                        | ((SDL_HasMMX(void)) ? BLIT_FEATURE_HAS_MMX : 0)
                        /* Feature 2 is has-AltiVec */
                        | ((SDL_HasAltiVec(void)) ? BLIT_FEATURE_HAS_ALTIVEC : 0)
                        /* Feature 4 is dont-use-prefetch */
                        /* !!!! FIXME: Check for G5 or later, not the cache size! Always prefetch on a G4. */
                        | ((GetL3CacheSize(void) == 0) ? BLIT_FEATURE_ALTIVEC_DONT_USE_PREFETCH : 0));
        }
    }
    return features;
}
...
#else
/* Feature 1 is has-MMX */
#define GetBlitFeatures(void) ((SDL_HasMMX(void) ? BLIT_FEATURE_HAS_MMX : 0) | (SDL_HasARMSIMD(void) ? BLIT_FEATURE_HAS_ARM_SIMD : 0))
#endif
            ...
            if (srcfmt->bytes_per_pixel > 0 &&
                ...
                for (which = 0; table[which].dstbpp; ++which) {
                    if (MASKOK(srcfmt->Rmask, table[which].srcR) &&
                        ...
                        ((table[which].blit_features & GetBlitFeatures(void)) ==
...
#ifdef SDL_ALTIVEC_BLITTERS
            if ((srcfmt->bytes_per_pixel == 4) && (dstfmt->bytes_per_pixel == 4) && SDL_HasAltiVec(void)) {
