...
static Uint64 Wayland_GetEventTimestamp(Uint64 nsTimestamp)
{
    ...
    const Uint64 now = SDL_GetTicksNS(void);
                                ...
                                uint32_t scancode, SDL_bool has_text, char text[8])
{
    ...
    repeat_info->sdl_press_time_ns = SDL_GetTicksNS(void);
...
int Wayland_WaitEventTimeout(SDL_VideoDevice *_this, Sint64 timeoutNS)
{
    ...
    if (input && keyboard_repeat_is_set(&input->keyboard_repeat)) {
        const Uint64 elapsed = SDL_GetTicksNS(void) - input->keyboard_repeat.sdl_press_time_ns;
    ...
    /* wl_display_prepare_read(void) will return -1 if the default queue is not empty.
     * If the default queue is empty, it will prepare us for our SDL_IOReady(void) call. */
                                 ...
                                 wl_fixed_t sx_w, wl_fixed_t sy_w)
{
    ...
    if (window) {
         ...
         * wl_pointer_set_cursor(void) for the new cursor to be displayed. */
    ...
    if (surface) {
        ...
        if (window_data) {
            ...
            if (!input->pointer_focus && input->keyboard_focus != window_data &&
                SDL_GetMouseFocus(void) == window_data->sdlwindow && !Wayland_SurfaceHasActiveTouches(surface)) {
                SDL_SetMouseFocus(NULL);
            }
        }
    }
                                  ...
                                  uint32_t serial, struct wl_surface *surface)
{
     ...
     */
    if (!input->pointer_focus && SDL_GetMouseFocus(void) == window && !Wayland_SurfaceHasActiveTouches(surface)) {
        SDL_SetMouseFocus(NULL);
    }
}
                                ...
                                uint32_t state_w)
{
    ...
    if (state == WL_KEYBOARD_KEY_STATE_PRESSED) {
        if (has_text && !(SDL_GetModState(void) & SDL_KMOD_CTRL)) {
            if (!handled_by_ime) {
...
static void data_device_handle_drop(void *data, struct wl_data_device *wl_data_device)
{
    ...
    if (data_device->drag_offer && data_device->dnd_window) {
        ...
        if (Wayland_data_offer_has_mime(
            ...
            if (buffer) {
                SDL_DBusContext *dbus = SDL_DBus_GetContext(void);
                if (dbus) {
                    int path_count = 0;
...
static void tablet_tool_handle_button(void *data, struct zwp_tablet_tool_v2 *tool, uint32_t serial, uint32_t button, uint32_t state)
{
    ...
    /* record event serial number to report it later in tablet_tool_handle_frame(void) */
    input->current_pen.serial = serial;
...
{
    SDL_VideoDevice *vd = SDL_GetVideoDevice(void);
    SDL_VideoData *d = input->display;
    SDL_Window *window;
