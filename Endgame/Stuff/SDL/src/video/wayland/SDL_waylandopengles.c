...
int Wayland_GLES_SwapWindow(SDL_VideoDevice *_this, SDL_Window *window)
{
    ...
    if (swap_interval != 0 && data->surface_status == WAYLAND_SURFACE_STATUS_SHOWN) {
        ...
        const Uint64 max_wait = SDL_GetTicksNS(void) + (SDL_NS_PER_SECOND / 20);
        while (SDL_AtomicGet(&data->swap_interval_ready) == 0) {
            Uint64 now;

            WAYLAND_wl_display_flush(display);

            /* wl_display_prepare_read_queue(void) will return -1 if the event queue is not empty.
             * If the event queue is empty, it will prepare us for our SDL_IOReady(void) call. */
            ...
            /* Beyond this point, we must either call wl_display_cancel_read(void) or wl_display_read_events(void) */
