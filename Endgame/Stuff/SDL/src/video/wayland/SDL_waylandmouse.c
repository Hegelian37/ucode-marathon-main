                                  ...
                                  uint32_t format)
{
    ...
    if (d->shm_data == MAP_FAILED) {
        d->shm_data = NULL;
        close(shm_fd);
        return SDL_SetError("mmap(void) failed.");
    }
...
{
    SDL_Window *focus = SDL_GetMouseFocus(void);
    Uint32 ret = 0;
...
void Wayland_RecreateCursors(void)
{
    ...
    SDL_VideoData *vdata = SDL_GetVideoDevice(void)->driverdata;
...
void Wayland_InitMouse(void)
{
    ...
    SDL_SetDefaultCursor(Wayland_CreateDefaultCursor(void));
...
#ifdef SDL_USE_LIBDBUS
    Wayland_DBusFinishCursorProperties(void);
#endif
