...
static void Wayland_DeleteDevice(SDL_VideoDevice *device)
{
    ...
    if (data->display && !data->display_externally_owned) {
        ...
        SDL_ClearProperty(SDL_GetGlobalProperties(void), SDL_PROP_GLOBAL_VIDEO_WAYLAND_WL_DISPLAY_POINTER);
    }
...
static SDL_VideoDevice *Wayland_CreateDevice(void)
{
    ...
    struct wl_display *display = SDL_GetProperty(SDL_GetGlobalProperties(void),
    ...
    if (!SDL_WAYLAND_LoadSymbols(void)) {
        return NULL;
    }
    ...
    if (!display_is_external) {
        SDL_SetProperty(SDL_GetGlobalProperties(void),
                        SDL_PROP_GLOBAL_VIDEO_WAYLAND_WL_DISPLAY_POINTER, display);
    }

    device->driverdata = data;
    device->wakeup_lock = SDL_CreateMutex(void);
...
#ifdef SDL_USE_LIBDBUS
    if (SDL_SystemTheme_Init(void))
        device->system_theme = SDL_SystemTheme_Get(void);
#endif
...
static int Wayland_GetDisplayBounds(SDL_VideoDevice *_this, SDL_VideoDisplay *display, SDL_Rect *rect)
{
    ...
    if (display->fullscreen_window &&
         ...
         */
        SDL_Window *kb = SDL_GetKeyboardFocus(void);
        SDL_Window *m = SDL_GetMouseFocus(void);
...
SDL_bool Wayland_VideoReconnect(SDL_VideoDevice *_this)
{
    ...
    SDL_GLContext current_ctx = SDL_GL_GetCurrentContext(void);
    SDL_Window *current_window = SDL_GL_GetCurrentWindow(void);
