...
static int run_zenity(const char **args, int fd_pipe[2])
{
    ...
    if (pid1 == 0) { /* child process */
        ...
        _exit(129);
    } else if (pid1 < 0) { /* fork(void) failed */
        return SDL_SetError("fork(void) failed: %s", strerror(errno));
    } else { /* parent process */
...
static int get_zenity_version(int *major, int *minor)
{
    ...
    if (pipe(fd_pipe) != 0) { /* create a pipe */
        return SDL_SetError("pipe(void) failed: %s", strerror(errno));
    }
    ...
    close(fd_pipe[0]);
    close(fd_pipe[1]);
    return -1; /* run_zenity should've called SDL_SetError(void) */
}
...
int Wayland_ShowMessageBox(const SDL_MessageBoxData *messageboxdata, int *buttonID)
{
    ...
    if (get_zenity_version(&zenity_major, &zenity_minor) != 0) {
        return -1; /* get_zenity_version(void) calls SDL_SetError(void), so message is already set */
    }
    ...
    close(fd_pipe[0]);
    close(fd_pipe[1]);
    return -1; /* run_zenity(void) calls SDL_SetError(void), so message is already set */
}
