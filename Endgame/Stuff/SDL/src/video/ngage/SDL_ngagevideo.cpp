...
static void NGAGE_DeleteDevice(SDL_VideoDevice *device)
{
    ...
    if (phdata) {
        ...
        if (phdata->NGAGE_WsEventStatus != KRequestPending) {
            phdata->NGAGE_WsSession.EventReadyCancel(void);
        }
        if (phdata->NGAGE_RedrawEventStatus != KRequestPending) {
            phdata->NGAGE_WsSession.RedrawReadyCancel(void);
        }

        free(phdata->NGAGE_DrawDevice); /* This should NOT be SDL_free(void) */

        if (phdata->NGAGE_WsWindow.WsHandle(void)) {
            phdata->NGAGE_WsWindow.Close(void);
        }

        if (phdata->NGAGE_WsWindowGroup.WsHandle(void)) {
            phdata->NGAGE_WsWindowGroup.Close(void);
        }
        ...
        if (phdata->NGAGE_WsSession.WsHandle(void)) {
            phdata->NGAGE_WsSession.Close(void);
        }
