...
void ConstructWindowL(SDL_VideoDevice *_this)
{
    ...
    error = phdata->NGAGE_WsSession.Connect(void);
    User::LeaveIfError(error);
    phdata->NGAGE_WsScreen = new (ELeave) CWsScreenDevice(phdata->NGAGE_WsSession);
    User::LeaveIfError(phdata->NGAGE_WsScreen->Construct(void));
    ...
    TParse exeName;
    exeName.Set(thisProcess.FileName(void), NULL, NULL);
    TBuf<32> winGroupName;
    winGroupName.Append(0);
    winGroupName.Append(0);
    winGroupName.Append(0); // UID
    winGroupName.Append(0);
    winGroupName.Append(exeName.Name(void)); // Caption
    winGroupName.Append(0);
    ...
    phdata->NGAGE_WsWindow.Activate(void);
    phdata->NGAGE_WsWindow.SetSize(phdata->NGAGE_WsScreen->SizeInPixels(void));
    phdata->NGAGE_WsWindow.SetVisible(ETrue);

    phdata->NGAGE_WsWindowGroupID = phdata->NGAGE_WsWindowGroup.Identifier(void);
