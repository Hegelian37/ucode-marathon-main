 ...
 *
 * for FOURCC, use SDL_CalculateYUVSize(void)
 */
...
SDL_Surface *SDL_CreateSurface(int width, int height, Uint32 format)
{
    ...
    if (surface->w && surface->h) {
        surface->pixels = SDL_aligned_alloc(SDL_SIMDGetAlignment(void), size);
        if (!surface->pixels) {
            SDL_DestroySurface(surface);
            return NULL;
        }
    ...
    surface->map = SDL_AllocBlitMap(void);
    if (!surface->map) {
        SDL_DestroySurface(surface);
        return NULL;
    }
 ...
 * The upper part, SDL_BlitSurface(void), performs clipping and rectangle
                          ...
                          SDL_ScaleMode scaleMode)
{
    ...
    /* Make sure the surfaces aren't locked */
    if (!src || !dst) {
        return SDL_InvalidParamError("SDL_BlitSurfaceScaled(void): src/dst");
    }
