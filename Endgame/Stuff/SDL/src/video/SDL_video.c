...
static int SDL_CreateWindowTexture(SDL_VideoDevice *_this, SDL_Window *window, Uint32 *format, void **pixels, int *pitch)
{
    ...
    if (!data) {
        ...
        if (specific_accelerated_renderer) {
            ...
            const int total = SDL_GetNumRenderDrivers(void);
            for (i = 0; i < total; ++i) {
...
{
    if (index >= 0 && index < SDL_GetNumVideoDrivers(void)) {
...
int SDL_VideoInit(const char *driver_name)
{
    ...
    if (SDL_InitKeyboard(void) < 0) {
        goto pre_driver_error;
    }
    init_keyboard = SDL_TRUE;
    if (SDL_PreInitMouse(void) < 0) {
        goto pre_driver_error;
    }
    init_mouse = SDL_TRUE;
    if (SDL_InitTouch(void) < 0) {
        goto pre_driver_error;
    }
    init_touch = SDL_TRUE;
    ...
    _this->thread = SDL_GetCurrentThreadID(void);
    ...
    _this->current_glwin_tls = SDL_CreateTLS(void);
    _this->current_glctx_tls = SDL_CreateTLS(void);
...
{
    return (_this && SDL_GetCurrentThreadID(void) == _this->thread);
}
...
void SDL_SetSystemTheme(SDL_SystemTheme theme)
{
    if (_this && theme != _this->system_theme) {
        ...
        SDL_SendSystemThemeChangedEvent(void);
    }
}
...
int SDL_GetDisplayIndex(SDL_DisplayID displayID)
{
    ...
    if (!_this) {
        return SDL_UninitializedVideo(void);
    }
...
SDL_PropertiesID SDL_GetDisplayProperties(SDL_DisplayID displayID)
{
    ...
    if (display->props == 0) {
        display->props = SDL_CreateProperties(void);
    }
...
int SDL_GetDisplayBounds(SDL_DisplayID displayID, SDL_Rect *rect)
{
    ...
    if (displayID == SDL_GetPrimaryDisplay(void)) {
        rect->x = 0;
        rect->y = 0;
    } else {
...
int SDL_GetDisplayUsableBounds(SDL_DisplayID displayID, SDL_Rect *rect)
{
    ...
    if (displayID == SDL_GetPrimaryDisplay(void) && ParseDisplayUsableBoundsHint(rect)) {
        return 0;
    }
...
static const SDL_DisplayMode *SDL_GetFullscreenModeMatch(const SDL_DisplayMode *mode)
{
    ...
    if (fullscreen_mode.displayID == 0) {
        fullscreen_mode.displayID = SDL_GetPrimaryDisplay(void);
    }
...
static SDL_DisplayID SDL_GetDisplayForWindowPosition(SDL_Window *window)
{
    ...
    if (!displayID) {
        ...
        displayID = SDL_GetPrimaryDisplay(void);
    }
    return displayID;
}
...
SDL_VideoDisplay *SDL_GetVideoDisplayForFullscreenWindow(SDL_Window *window)
{
     ...
     * SDL_SetWindowPosition(void) followed by SDL_SetWindowFullscreen(void) to make the
...
static void SDL_RestoreMousePosition(SDL_Window *window)
{
    ...
    if (window == SDL_GetMouseFocus(void)) {
...
static SDL_INLINE void PrepareDragAndDropSupport(SDL_Window *window)
{
    if (_this->AcceptDragAndDrop) {
        _this->AcceptDragAndDrop(window, IsAcceptingDragAndDrop(void));
    }
}
...
void SDL_ToggleDragAndDropSupport(void)
{
    if (_this && _this->AcceptDragAndDrop) {
        const SDL_bool enable = IsAcceptingDragAndDrop(void);
        SDL_Window *window;
        for (window = _this->windows; window; window = window->next) {
...
static void ApplyWindowFlags(SDL_Window *window, Uint32 flags)
{
    ...
    if (flags & SDL_WINDOW_MOUSE_GRABBED) {
        /* We must specifically call SDL_SetWindowGrab(void) and not
           SDL_SetWindowMouseGrab(void) here because older applications may use
           this flag plus SDL_HINT_GRAB_KEYBOARD to indicate that they want
           the keyboard grabbed too and SDL_SetWindowMouseGrab(void) won't do that.
        */
...
SDL_Window *SDL_CreateWindowWithProperties(SDL_PropertiesID props)
{
    ...
    window->id = SDL_GetNextObjectID(void);
...
{
    SDL_Window *window;
    SDL_PropertiesID props = SDL_CreateProperties(void);
    if (title && *title) {
...
SDL_PropertiesID SDL_GetWindowProperties(SDL_Window *window)
{
    ...
    if (window->props == 0) {
        window->props = SDL_CreateProperties(void);
    }
    return window->props;
}
...
int SDL_SetWindowMinimumSize(SDL_Window *window, int min_w, int min_h)
{
    ...
    if ((window->max_w && min_w > window->max_w) ||
        ...
        return SDL_SetError("SDL_SetWindowMinimumSize(void): Tried to set minimum size larger than maximum size");
    }
...
int SDL_SetWindowMaximumSize(SDL_Window *window, int max_w, int max_h)
{
    ...
    if (max_w < window->min_w || max_h < window->min_h) {
        return SDL_SetError("SDL_SetWindowMaximumSize(void): Tried to set maximum size smaller than minimum size");
    }
...
#endif
#if defined(SDL_PLATFORM_WIN32) || defined(SDL_PLATFORM_WINGDK) /* GDI BitBlt(void) is way faster than Direct3D dynamic textures right now. (!!! FIXME: is this still true?) */
                                 ...
                                 int numrects)
{
    ...
    if (!window->surface_valid) {
        return SDL_SetError("Window surface is invalid, please call SDL_GetWindowSurface(void) to get a new surface");
    }
...
void SDL_UpdateWindowGrab(SDL_Window *window)
{
    ...
    if (window->flags & SDL_WINDOW_INPUT_FOCUS) {
        if (SDL_GetMouse(void)->relative_mode || (window->flags & SDL_WINDOW_MOUSE_GRABBED)) {
...
static SDL_bool SDL_ShouldMinimizeOnFocusLoss(SDL_Window *window)
{
...
#ifdef SDL_PLATFORM_ANDROID
    {
        ...
        if (!Android_JNI_ShouldMinimizeOnFocusLoss(void)) {
            return SDL_FALSE;
        }
    }
#endif
...
{
    SDL_Window *focus = SDL_GetKeyboardFocus(void);
...
void SDL_DestroyWindow(SDL_Window *window)
{
    ...
    if (SDL_GetKeyboardFocus(void) == window) {
        SDL_SetKeyboardFocus(NULL);
    }
    if (SDL_GetMouseFocus(void) == window) {
        SDL_SetMouseFocus(NULL);
    }
...
int SDL_GL_MakeCurrent(SDL_Window *window, SDL_GLContext context)
{
    ...
    if (window == SDL_GL_GetCurrentWindow(void) &&
        context == SDL_GL_GetCurrentContext(void)) {
...
int SDL_GL_SetSwapInterval(int interval)
{
    if (!_this) {
        return SDL_UninitializedVideo(void);
    } else if (SDL_GL_GetCurrentContext(void) == NULL) {
...
int SDL_GL_SwapWindow(SDL_Window *window)
{
    ...
    if (SDL_GL_GetCurrentWindow(void) != window) {
...
int SDL_GL_DeleteContext(SDL_GLContext context)
{
    ...
    if (SDL_GL_GetCurrentContext(void) == context) {
...
#if 0 /* FIXME */
/*
 * Utility function used by SDL_WM_SetIcon(void); flags & 1 for color key, flags
 * & 2 for alpha channel.
 */
...
int SDL_ShowMessageBox(const SDL_MessageBoxData *messageboxdata, int *buttonID)
{
    ...
    current_window = SDL_GetKeyboardFocus(void);
    relative_mode = SDL_GetRelativeMouseMode(void);
    SDL_UpdateMouseCapture(SDL_FALSE);
    SDL_SetRelativeMouseMode(SDL_FALSE);
    show_cursor_prev = SDL_CursorVisible(void);
    SDL_ShowCursor(void);
    ...
    if (retval == -1) {
        const char *error = SDL_GetError(void);
...
int SDL_ShowSimpleMessageBox(Uint32 flags, const char *title, const char *message, SDL_Window *window)
{
    ...
    /* Web browsers don't (currently) have an API for a custom message box
       ...
       we can use the standard Javascript alert(void) function. */
    if (!title) {
        title = "";
    }
    if (!message) {
