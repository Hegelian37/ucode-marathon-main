        ...
        shareContext:(NSOpenGLContext *)share
{
    ...
    if (self) {
        ...
        self->swapIntervalCond = SDL_CreateCondition(void);
        self->swapIntervalMutex = SDL_CreateMutex(void);
...
- (void)setWindow:(SDL_Window *)newWindow
{
    ...
    if (newWindow) {
        ...
        if ([NSThread isMainThread]) {
            ...
            dispatch_sync(dispatch_get_main_queue(void), ^{ [self setView:nil]; });
        }
    }
}
...
SDL_GLContext Cocoa_GL_CreateContext(SDL_VideoDevice *_this, SDL_Window *window)
{
    @autoreleasepool {
        ...
        if (_this->gl_config.share_with_current_context) {
            share_context = (__bridge NSOpenGLContext *)SDL_GL_GetCurrentContext(void);
        }
...
{
    @autoreleasepool {
        SDLOpenGLContext *nscontext = (__bridge SDLOpenGLContext *)SDL_GL_GetCurrentContext(void);
        int status;
...
int Cocoa_GL_SwapWindow(SDL_VideoDevice *_this, SDL_Window *window)
{
    @autoreleasepool {
        ...
        /*{ static Uint64 prev = 0; const Uint64 now = SDL_GetTicks(void); const unsigned int diff = (unsigned int) (now - prev); prev = now; printf("GLSWAPBUFFERS TICKS %u\n", diff); }*/
