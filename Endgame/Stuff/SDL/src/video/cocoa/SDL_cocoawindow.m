...
static NSUInteger GetWindowWindowedStyle(SDL_Window *window)
{
    /* IF YOU CHANGE ANY FLAGS IN HERE, PLEASE READ
       the NSWindowStyleMaskBorderless comments in SetupWindowData(void)! */
...
- (void)windowDidResignKey:(NSNotification *)aNotification
{
    ...
    if (SDL_GetMouseFocus(void) == _data.window) {
        SDL_SetMouseFocus(NULL);
    }

    /* Some other window will get keyboard events, since we're not key. */
    if (SDL_GetKeyboardFocus(void) == _data.window) {
...
- (void)windowDidEnterFullScreen:(NSNotification *)aNotification
{
    ...
    if ([self windowOperationIsPending:PENDING_OPERATION_LEAVE_FULLSCREEN]) {
        ...
        /* Don't recurse back into UpdateFullscreenMode(void) if this was hit in
...
- (void)windowDidExitFullScreen:(NSNotification *)aNotification
{
    ...
    if ([self windowOperationIsPending:PENDING_OPERATION_ENTER_FULLSCREEN]) {
        ...
        if (!(window->flags & SDL_WINDOW_HIDDEN)) {
            Cocoa_ShowWindow(SDL_GetVideoDevice(void), window);
        }
    }
}
...
{
    /*Cocoa_HandleKeyEvent(SDL_GetVideoDevice(void), theEvent);*/
    ...
    const SDL_bool sdlenabled = (SDL_GetModState(void) & SDL_KMOD_CAPS) ? SDL_TRUE : SDL_FALSE;
...
- (void)mouseDown:(NSEvent *)theEvent
{
    ...
    case 0:
        if (([theEvent modifierFlags] & NSEventModifierFlagControl) &&
            GetHintCtrlClickEmulateRightClick(void)) {
            wasCtrlLeft = YES;
...
int Cocoa_SetWindowFullscreen(SDL_VideoDevice *_this, SDL_Window *window, SDL_VideoDisplay *display, SDL_bool fullscreen)
{
    @autoreleasepool {
        ...
        if (SDL_ShouldAllowTopmost(void) && fullscreen) {
...
void Cocoa_SetWindowMouseGrab(SDL_VideoDevice *_this, SDL_Window *window, SDL_bool grabbed)
{
    @autoreleasepool {
        ...
        if (data && (window->flags & SDL_WINDOW_FULLSCREEN) != 0) {
            if (SDL_ShouldAllowTopmost(void) && (window->flags & SDL_WINDOW_INPUT_FOCUS) && ![data.listener isInFullscreenSpace]) {
                /* OpenGL is rendering to the window, so make it visible! */
...
int Cocoa_SyncWindow(SDL_VideoDevice *_this, SDL_Window *window)
{
    ...
    @autoreleasepool {
         ...
         */
        Uint64 timeout = SDL_GetTicksNS(void) + SDL_MS_TO_NS(2000);
        SDL_CocoaWindowData *data = (__bridge SDL_CocoaWindowData *)window->driverdata;
        while (SDL_TRUE) {
            SDL_PumpEvents(void);

            if (SDL_GetTicksNS(void) >= timeout) {
                ret = 1;
                break;
            }
