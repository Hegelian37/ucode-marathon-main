...
static SDL_Window *SDL_FindWindowAtPoint(const float x, const float y)
{
    ...
    SDL_Window *i;
    for (i = SDL_GetVideoDevice(void)->windows; i; i = i->next) {
...
static int Cocoa_SetRelativeMouseMode(SDL_bool enabled)
{
    ...
    if (result != kCGErrorSuccess) {
        return SDL_SetError("CGAssociateMouseAndMouseCursorPosition(void) failed");
    }
...
int Cocoa_InitMouse(SDL_VideoDevice *_this)
{
    ...
    SDL_SetDefaultCursor(Cocoa_CreateDefaultCursor(void));
...
void Cocoa_HandleMouseWarp(CGFloat x, CGFloat y)
{
     ...
     */
    SDL_MouseData *driverdata = (SDL_MouseData *)SDL_GetMouse(void)->driverdata;
