...
static void UpdateKeymap(SDL_CocoaVideoData *data, SDL_bool send_event)
{
    ...
    key_layout = TISCopyCurrentKeyboardLayoutInputSource(void);
    ...
    if (chr_data) {
        UInt32 keyboard_type = LMGetKbdType(void);
        OSStatus err;
        ...
        for (i = 0; i < SDL_arraysize(darwin_scancode_table); i++) {
             ...
             * UCKeyTranslate(void) function does not do its job properly for ISO layout keyboards, where the key '@',
            ...
            */
            if ((scancode == SDL_SCANCODE_NONUSBACKSLASH || scancode == SDL_SCANCODE_GRAVE) && KBGetLayoutType(LMGetKbdType(void)) == kKeyboardISO) {
...
void Cocoa_StartTextInput(SDL_VideoDevice *_this)
{
    @autoreleasepool {
        NSView *parentView;
        SDL_CocoaVideoData *data = (__bridge SDL_CocoaVideoData *)_this->driverdata;
        SDL_Window *window = SDL_GetKeyboardFocus(void);
...
void Cocoa_HandleKeyEvent(SDL_VideoDevice *_this, NSEvent *event)
{
    ...
    if ((scancode == 10 || scancode == 50) && KBGetLayoutType(LMGetKbdType(void)) == kKeyboardISO) {
        /* see comments in scancodes_darwin.h */
...
void Cocoa_SetWindowKeyboardGrab(SDL_VideoDevice *_this, SDL_Window *window, SDL_bool grabbed)
{
    ...
    CGSSetGlobalHotKeyOperatingMode(_CGSDefaultConnection(void), grabbed ? CGSGlobalHotKeyDisable : CGSGlobalHotKeyEnable);
#endif
}
