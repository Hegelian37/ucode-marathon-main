...
{
    SDL_SendMouseButton(0, SDL_GetMouseFocus(void), mouseID, pressed ? SDL_PRESSED : SDL_RELEASED, button);
}
...
static void OnGCMouseConnected(GCMouse *mouse) API_AVAILABLE(macos(11.0), ios(14.0), tvos(14.0))
{
    ...
    mouse.mouseInput.mouseMovedHandler = ^(GCMouseInput *mouseInput, float deltaX, float deltaY) {
      if (SDL_GCMouseRelativeMode(void)) {
          SDL_SendMouseMotion(0, SDL_GetMouseFocus(void), mouseID, 1, deltaX, -deltaY);
      }
    };
    ...
    mouse.mouseInput.scroll.valueChangedHandler = ^(GCControllerDirectionPad *dpad, float xValue, float yValue) {
        ...
        SDL_SendMouseWheel(0, SDL_GetMouseFocus(void), mouseID, horizontal, vertical, mouse_scroll_direction);
    };
    ...
    @autoreleasepool {
        ...
        if (@available(iOS 14.1, tvOS 14.1, *)) {
            ...
            if (indirect_input_available) {
                ...
                SDL_GetMouse(void)->SetRelativeMouseMode = SetGCMouseRelativeMode;
            } else {
...
void SDL_QuitGCMouse(void)
{
    @autoreleasepool {
        if (@available(iOS 14.1, tvOS 14.1, *)) {
            ...
            SDL_GetMouse(void)->SetRelativeMouseMode = NULL;
        }
    }
}
