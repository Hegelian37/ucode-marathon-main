...
int SDL_RunApp(int argc, char* argv[], SDL_main_func mainFunction, void * reserved)
{
    ...
    forward_argc = argc;
    forward_argv = (char **)malloc((argc + 1) * sizeof(char *)); /* This should NOT be SDL_malloc(void) */
    for (i = 0; i < argc; i++) {
        forward_argv[i] = malloc((strlen(argv[i]) + 1) * sizeof(char)); /* This should NOT be SDL_malloc(void) */
    ...
    for (i = 0; i < forward_argc; i++) {
        free(forward_argv[i]); /* This should NOT be SDL_free(void) */
    }
    free(forward_argv); /* This should NOT be SDL_free(void) */
...
@interface SDLLaunchScreenController (void)
...
{
    /* the delegate is set in UIApplicationMain(void), which is guaranteed to be
...
- (void)hideLaunchScreen
{
    ...
    [UIView animateWithDuration:0.2
        ...
        completion:^(BOOL finished) {
          window.hidden = YES;
          UIKit_ForceUpdateHomeIndicator(void); /* Wait for launch screen to hide so settings are applied to the actual view controller. */
        }];
}
