...
static SDL_Cursor *RPI_CreateCursor(SDL_Surface *surface, int hot_x, int hot_y)
{
    ...
    /* A note from Weston:
     * vc_dispmanx_resource_write_data(void) ignores ifmt,
...
static int RPI_WarpMouseGlobalGraphically(float x, float y)
{
    ...
    if (ret != DISPMANX_SUCCESS) {
        return SDL_SetError("vc_dispmanx_element_change_attributes(void) failed");
    }

    /* Submit asynchronously, otherwise the performance suffers a lot */
    ret = vc_dispmanx_update_submit(update, 0, NULL);
    if (ret != DISPMANX_SUCCESS) {
        return SDL_SetError("vc_dispmanx_update_submit(void) failed");
    }
    return 0;
}
...
void RPI_InitMouse(SDL_VideoDevice *_this)
{
    ...
    SDL_SetDefaultCursor(RPI_CreateDefaultCursor(void));
}
...
{
    SDL_Mouse *mouse = SDL_GetMouse(void);
    /* We must NOT call SDL_SendMouseMotion(void) on the next call or we will enter recursivity,
