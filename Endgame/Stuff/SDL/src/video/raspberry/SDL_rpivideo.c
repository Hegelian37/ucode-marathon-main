...
static float RPI_GetRefreshRate(void)
{
...
static void AddDispManXDisplay(const int display_id)
{
    ...
    /* RPI_GetRefreshRate(void) doesn't distinguish between displays. I'm not sure the hardware distinguishes either */
    SDL_zero(mode);
    mode.w = modeinfo.width;
    mode.h = modeinfo.height;
    mode.refresh_rate = RPI_GetRefreshRate(void);
...
int RPI_CreateWindow(SDL_VideoDevice *_this, SDL_Window *window, SDL_PropertiesID create_props)
{
    ...
    if (SDL_GetHintBoolean(SDL_HINT_VIDEO_DOUBLE_BUFFER, SDL_FALSE)) {
        wdata->vsync_cond = SDL_CreateCondition(void);
        wdata->vsync_cond_mutex = SDL_CreateMutex(void);
