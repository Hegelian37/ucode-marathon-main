...
static int SDLCALL SDL_MainCallbackEventWatcher(void *userdata, SDL_Event *event)
{
    if (ShouldDispatchImmediately(event)) {
        ...
        SDL_DispatchMainCallbackEvents(void);
        SDL_DispatchMainCallbackEvent(event);
    } else {
...
int SDL_IterateMainCallbacks(SDL_bool pump_events)
{
    ...
    if (rc == 0) {
        rc = SDL_main_iteration_callback(void);
...
void SDL_QuitMainCallbacks(void)
{
    ...
    SDL_main_quit_callback(void);

    // for symmetry, you should explicitly Quit what you Init, but we might come through here uninitialized and SDL_Quit(void) will clear everything anyhow.
