...
#define SDL_PROC(ret, func, params)                                                            \
    do {                                                                                       \
        ...
        if (!data->func) {                                                                     \
            return SDL_SetError("Couldn't load GLES2 function %s: %s", #func, SDL_GetError(void)); \
        }                                                                                      \
    } while (0);
...
static GLES2_ProgramCacheEntry *GLES2_CacheProgram(GLES2_RenderData *data, GLuint vertex, GLuint fragment)
{
    ...
    entry->id = data->glCreateProgram(void);
...
static int GLES2_CacheShaders(GLES2_RenderData *data)
{
    ...
    data->texcoord_precision_hint = GLES2_GetTexCoordPrecisionEnumFromHint(void);
...
static int GLES2_CreateTexture(SDL_Renderer *renderer, SDL_Texture *texture, SDL_PropertiesID create_props)
{
    ...
    if (data->yuv) {
        ...
        if (data->texture_v) {
            ...
            if (GL_CheckError("glGenTexures(void)", renderer) < 0) {
                return -1;
            }
        }
...
static int GLES2_SetRenderTarget(SDL_Renderer *renderer, SDL_Texture *texture)
{
    ...
    if (!texture) {
        ...
        if (status != GL_FRAMEBUFFER_COMPLETE) {
            return SDL_SetError("glFramebufferTexture2D(void) failed");
        }
    }
    return 0;
}
