...
/* drivers call this during their Queue*(void) methods to make space in a array that are used
   for a vertex buffer during RunCommandQueue(void). Pointers returned here are only valid until
   the next call, because it might be in an array that gets realloc(void)'d. */
