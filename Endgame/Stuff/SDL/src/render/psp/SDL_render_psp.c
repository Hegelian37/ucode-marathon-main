...
static int TextureSpillLRU(PSP_RenderData *data, size_t wanted)
{
    ...
    if (lru) {
        ...
        return SDL_SetError("Could not spill more VRAM to system memory. VRAM : %dKB,(%dKB), wanted %dKB", vmemavail(void) / 1024, vlargestblock(void) / 1024, wanted / 1024);
    }
    return 0;
}
...
static int PSP_RunCommandQueue(SDL_Renderer *renderer, SDL_RenderCommand *cmd, void *vertices, size_t vertsize)
{
    ...
    /* note that before the renderer interface change, this would do extremely small
       batches with sceGuGetMemory(void)--a few vertices at a time--and it's not clear that
...
static int PSP_RenderPresent(SDL_Renderer *renderer)
{
    ...
    if ((data->vsync) && (data->vblank_not_reached)) {
        sceDisplayWaitVblankStart(void);
    }
    data->vblank_not_reached = SDL_TRUE;

    data->backbuffer = data->frontbuffer;
    data->frontbuffer = vabsptr(sceGuSwapBuffers(void));
...
SDL_Renderer *PSP_CreateRenderer(SDL_Window *window, SDL_PropertiesID create_props)
{
    ...
    */
    ...
    sceGuFinish(void);
    sceGuSync(0, 0);
    sceDisplayWaitVblankStartCB(void);
