...
static void D3D12_ReleaseAll(SDL_Renderer *renderer)
{
    ...
    if (data) {
         ...
         * to prevent IUnknown::Release(void) calls from crashing.
         */
        if (data->hD3D12Mod) {
...
static void D3D12_ResetCommandList(D3D12_RenderData *data)
{
    ...
    data->currentRenderTargetView.ptr = 0;
    /* FIXME should we also clear currentSampler.ptr and currentRenderTargetView.ptr ? (and use D3D12_InvalidateCachedState(void) instead) */
...
#else
    /* CreateEventEx(void) arrived in Vista, so we need to load it with GetProcAddress for XP. */
    {
...
static HRESULT D3D12_CreateSwapChain(SDL_Renderer *renderer, int w, int h)
{
    ...
    if (WIN_IsWindows8OrGreater(void)) {
...
static HRESULT D3D12_CreateWindowSizeDependentResources(SDL_Renderer *renderer)
{
    ...
    data->rotation = D3D12_GetCurrentRotation(void);
...

{
    ...
    if (!currentPipelineState ||
        ...
        */
        ...
        if (!currentPipelineState) {
            /* The error has been set inside D3D12_CreatePipelineState(void) */
            return -1;
        }
...
SDL_Renderer *D3D12_CreateRenderer(SDL_Window *window, SDL_PropertiesID create_props)
{
    ...
    data->identity = MatrixIdentity(void);
