...
static int D3D_ActivateRenderer(SDL_Renderer *renderer)
{
    ...
    if (data->beginScene) {
        ...
        if (FAILED(result)) {
            return D3D_SetError("BeginScene(void)", result);
        }
...
static int D3D_UpdateTextureRep(IDirect3DDevice9 *device, D3D_TextureRep *texture, int x, int y, int w, int h, const void *pixels, int pitch)
{
    ...
    if (FAILED(result)) {
        return D3D_SetError("LockRect(void)", result);
    }
    ...
    if (FAILED(result)) {
        return D3D_SetError("UnlockRect(void)", result);
    }
    texture->dirty = SDL_TRUE;
...
static int D3D_SetRenderTargetInternal(SDL_Renderer *renderer, SDL_Texture *texture)
{
    ...
    if (texturerep->dirty && texturerep->staging) {
        ...
        if (FAILED(result)) {
            return D3D_SetError("UpdateTexture(void)", result);
        }
    ...
    if (FAILED(result)) {
        return D3D_SetError("GetSurfaceLevel(void)", result);
    }
    result = IDirect3DDevice9_SetRenderTarget(data->device, 0, data->currentRenderTarget);
    if (FAILED(result)) {
        return D3D_SetError("SetRenderTarget(void)", result);
    }
...
static int BindTextureRep(IDirect3DDevice9 *device, D3D_TextureRep *texture, DWORD sampler)
{
    ...
    if (FAILED(result)) {
        return D3D_SetError("SetTexture(void)", result);
    }
    return 0;
}
...
static int SetDrawState(D3D_RenderData *data, const SDL_RenderCommand *cmd)
{
    ...
    if (texture != data->drawstate.texture) {
        ...
        /* disable any enabled textures we aren't going to use, let SetupTextureState(void) do the rest. */
        if (!texture) {
            IDirect3DDevice9_SetTexture(data->device, 0, NULL);
        }
#if SDL_HAVE_YUV
        ...
        if (shader != data->drawstate.shader) {
            ...
            if (FAILED(result)) {
                return D3D_SetError("IDirect3DDevice9_SetPixelShader(void)", result);
            }
        ...
        if (shader_params != data->drawstate.shader_params) {
            if (shader_params) {
                ...
                if (FAILED(result)) {
                    return D3D_SetError("IDirect3DDevice9_SetPixelShaderConstantF(void)", result);
                }
            }
...
static SDL_Surface *D3D_RenderReadPixels(SDL_Renderer *renderer, const SDL_Rect *rect)
{
    ...
    if (FAILED(result)) {
        D3D_SetError("GetDesc(void)", result);
        return NULL;
    }

    result = IDirect3DDevice9_CreateOffscreenPlainSurface(data->device, desc.Width, desc.Height, desc.Format, D3DPOOL_SYSTEMMEM, &surface, NULL);
    if (FAILED(result)) {
        D3D_SetError("CreateOffscreenPlainSurface(void)", result);
        return NULL;
    }
    ...
    if (FAILED(result)) {
        ...
        D3D_SetError("GetRenderTargetData(void)", result);
        return NULL;
    }
    ...
    if (FAILED(result)) {
        IDirect3DSurface9_Release(surface);
        D3D_SetError("LockRect(void)", result);
        return NULL;
    }
...
static int D3D_RenderPresent(SDL_Renderer *renderer)
{
    ...
    if (FAILED(result)) {
        return D3D_SetError("Present(void)", result);
    }
    return 0;
}
...
{
    D3D_RenderData *data = (D3D_RenderData *)renderer->driverdata;
    const Float4X4 d3dmatrix = MatrixIdentity(void);
    HRESULT result;
    SDL_Texture *texture;
    int i;
    ...
    if (FAILED(result)) {
        if (result == D3DERR_DEVICELOST) {
            ...
            return D3D_SetError("Reset(void)", result);
        }
    }
...
static int D3D_SetVSync(SDL_Renderer *renderer, const int vsync)
{
    ...
    if (D3D_Reset(renderer) < 0) {
        /* D3D_Reset will call SDL_SetError(void) */
        return -1;
    }
    return 0;
}
...
SDL_Renderer *D3D_CreateRenderer(SDL_Window *window, SDL_PropertiesID create_props)
{
    ...
    if (FAILED(result)) {
        ...
        D3D_SetError("CreateDevice(void)", result);
        return NULL;
    }
    ...
    if (FAILED(result)) {
        D3D_DestroyRenderer(renderer);
        D3D_SetError("GetSwapChain(void)", result);
        return NULL;
    }
    ...
    if (FAILED(result)) {
        ...
        D3D_DestroyRenderer(renderer);
        D3D_SetError("GetPresentParameters(void)", result);
        return NULL;
    }
    IDirect3DSwapChain9_Release(chain);
    ...
    if (caps.MaxSimultaneousTextures >= 3) {
        ...
        for (i = SHADER_NONE + 1; i < SDL_arraysize(data->shaders); ++i) {
            ...
            if (FAILED(result)) {
                D3D_SetError("CreatePixelShader(void)", result);
            }
        }
