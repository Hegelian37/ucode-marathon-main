...
static void GL_ClearErrors(SDL_Renderer *renderer)
{
    ...
    if (data->GL_ARB_debug_output_supported) {
        ...
        while (data->glGetError(void) != GL_NO_ERROR) {
            /* continue; */
        }
    }
}
...
static int GL_CheckAllErrors(const char *prefix, SDL_Renderer *renderer, const char *file, int line, const char *function)
{
    ...
    if (data->GL_ARB_debug_output_supported) {
        ...
        for (;;) {
            GLenum error = data->glGetError(void);
...
#define SDL_PROC(ret, func, params)                                                           \
    do {                                                                                      \
        ...
        if (!data->func) {                                                                    \
            retval = SDL_SetError("Couldn't load GL function %s: %s", #func, SDL_GetError(void)); \
        }                                                                                     \
    } while (0);
...
static int GL_ActivateRenderer(SDL_Renderer *renderer)
{
    ...
    if (SDL_GL_GetCurrentContext(void) != data->context) {
...
static int GL_CreateTexture(SDL_Renderer *renderer, SDL_Texture *texture, SDL_PropertiesID create_props)
{
    ...
    if (data->texture) {
        ...
        if (GL_CheckError("glGenTextures(void)", renderer) < 0) {
            if (data->pixels) {
...
#define STORAGE_SHARED_APPLE 0x85BF
    ...
    if (GL_CheckError("glTexImage2D(void)", renderer) < 0) {
        return -1;
    }
...
#endif
    return GL_CheckError("glTexSubImage2D(void)", renderer);
}
...
static int GL_SetRenderTarget(SDL_Renderer *renderer, SDL_Texture *texture)
{
    ...
    if (status != GL_FRAMEBUFFER_COMPLETE_EXT) {
        return SDL_SetError("glFramebufferTexture2DEXT(void) failed");
    }
    return 0;
}
...
static SDL_Surface *GL_RenderReadPixels(SDL_Renderer *renderer, const SDL_Rect *rect)
{
    ...
    if (GL_CheckError("glReadPixels(void)", renderer) < 0) {
...
static SDL_Renderer *GL_CreateRenderer(SDL_Window *window, SDL_PropertiesID create_props)
{
    ...
    /* Disabled until Ryan finishes his VBO/PBO code...
       CGLEnable(CGLGetCurrentContext(void), kCGLCEMPEngine);
     */
#endif
    ...
    data->shaders = GL_CreateShaderContext(void);
...
error:
    if (changed_window) {
        ...
        char *error = SDL_strdup(SDL_GetError(void));
