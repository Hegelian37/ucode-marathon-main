...
{
#ifndef SDL_RENDER_DISABLED
    if (index < 0 || index >= SDL_GetNumRenderDrivers(void)) {
        SDL_SetError("index must be in the range of 0 - %d",
                            SDL_GetNumRenderDrivers(void) - 1);
        return NULL;
    }
...
SDL_Renderer *SDL_CreateRendererWithProperties(SDL_PropertiesID props)
{
    ...
    const int n = SDL_GetNumRenderDrivers(void);
    const char *hint;
    int i, attempted = 0;
...
#ifdef SDL_PLATFORM_ANDROID
    Android_ActivityMutex_Lock_Running(void);
#endif
    ...
    renderer->target_mutex = SDL_CreateMutex(void);
    ...
    renderer->line_method = SDL_GetRenderLineMethod(void);
...
#ifdef SDL_PLATFORM_ANDROID
    Android_ActivityMutex_Unlock(void);
#endif
    return renderer;
...
SDL_PropertiesID SDL_GetRendererProperties(SDL_Renderer *renderer)
{
    ...
    if (renderer->props == 0) {
        renderer->props = SDL_CreateProperties(void);
    }
...
SDL_Texture *SDL_CreateTextureWithProperties(SDL_Renderer *renderer, SDL_PropertiesID props)
{
    ...
    if (!texture_is_fourcc_and_target && IsSupportedFormat(renderer, format)) {
        ...
        int closest_format;
        SDL_PropertiesID native_props = SDL_CreateProperties(void);
...
SDL_Texture *SDL_CreateTextureFromSurface(SDL_Renderer *renderer, SDL_Surface *surface)
{
    ...
    if (!surface) {
        SDL_InvalidParamError("SDL_CreateTextureFromSurface(void): surface");
        return NULL;
    }
    ...
    if (format == surface->format->format && texture_colorspace == surface_colorspace) {
        if (surface->format->Amask && SDL_SurfaceHasColorKey(surface)) {
             ...
             * Intermediate conversion is needed to convert color key to alpha (SDL_ConvertColorkeyToAlpha(void)). */
    ...
    props = SDL_CreateProperties(void);
...
SDL_PropertiesID SDL_GetTextureProperties(SDL_Texture *texture)
{
    ...
    if (texture->props == 0) {
        texture->props = SDL_CreateProperties(void);
    }
...
int SDL_LockTexture(SDL_Texture *texture, const SDL_Rect *rect, void **pixels, int *pitch)
{
    ...
    if (texture->access != SDL_TEXTUREACCESS_STREAMING) {
        return SDL_SetError("SDL_LockTexture(void): texture must be streaming");
    }
...
int SDL_RenderPoints(SDL_Renderer *renderer, const SDL_FPoint *points, int count)
{
    ...
    if (!points) {
        return SDL_InvalidParamError("SDL_RenderPoints(void): points");
    }
    if (count < 1) {
        return 0;
    }
...
int SDL_RenderLines(SDL_Renderer *renderer, const SDL_FPoint *points, int count)
{
    ...
    if (!points) {
        return SDL_InvalidParamError("SDL_RenderLines(void): points");
    }
    if (count < 2) {
        return 0;
    }
...
int SDL_RenderRects(SDL_Renderer *renderer, const SDL_FRect *rects, int count)
{
    ...
    if (!rects) {
        return SDL_InvalidParamError("SDL_RenderRects(void): rects");
    }
    if (count < 1) {
        return 0;
    }
...
int SDL_RenderFillRects(SDL_Renderer *renderer, const SDL_FRect *rects, int count)
{
    ...
    if (!rects) {
        return SDL_InvalidParamError("SDL_RenderFillRects(void): rects");
    }
    if (count < 1) {
        return 0;
    }
