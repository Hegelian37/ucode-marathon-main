...
int SDL_DrawLine(SDL_Surface *dst, int x1, int y1, int x2, int y2, Uint32 color)
{
    ...
    if (!dst) {
        return SDL_InvalidParamError("SDL_DrawLine(void): dst");
    }

    func = SDL_CalculateDrawLineFunc(dst->format);
    if (!func) {
        return SDL_SetError("SDL_DrawLine(void): Unsupported surface format");
    }
                  ...
                  Uint32 color)
{
    ...
    if (!dst) {
        return SDL_InvalidParamError("SDL_DrawLines(void): dst");
    }

    func = SDL_CalculateDrawLineFunc(dst->format);
    if (!func) {
        return SDL_SetError("SDL_DrawLines(void): Unsupported surface format");
    }
