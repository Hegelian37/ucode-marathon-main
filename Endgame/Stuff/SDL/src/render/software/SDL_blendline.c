                  ...
                  SDL_BlendMode blendMode, Uint8 r, Uint8 g, Uint8 b, Uint8 a)
{
    ...
    if (!dst) {
        return SDL_InvalidParamError("SDL_BlendLine(void): dst");
    }

    func = SDL_CalculateBlendLineFunc(dst->format);
    if (!func) {
        return SDL_SetError("SDL_BlendLine(void): Unsupported surface format");
    }
                   ...
                   SDL_BlendMode blendMode, Uint8 r, Uint8 g, Uint8 b, Uint8 a)
{
    ...
    if (!dst) {
        return SDL_SetError("SDL_BlendLines(void): Passed NULL destination surface");
    }

    func = SDL_CalculateBlendLineFunc(dst->format);
    if (!func) {
        return SDL_SetError("SDL_BlendLines(void): Unsupported surface format");
    }
