...
{
    if (!dst) {
        return SDL_InvalidParamError("SDL_DrawPoint(void): dst");
    }

    /* This function doesn't work on surfaces < 8 bpp */
    if (dst->format->bits_per_pixel < 8) {
        return SDL_SetError("SDL_DrawPoint(void): Unsupported surface format");
    }
                   ...
                   Uint32 color)
{
    ...
    if (!dst) {
        return SDL_InvalidParamError("SDL_DrawPoints(void): dst");
    }

    /* This function doesn't work on surfaces < 8 bpp */
    if (dst->format->bits_per_pixel < 8) {
        return SDL_SetError("SDL_DrawPoints(void): Unsupported surface format");
    }
