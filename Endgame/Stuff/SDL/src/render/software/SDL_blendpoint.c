...
{
    if (!dst) {
        return SDL_InvalidParamError("SDL_BlendPoint(void): dst");
    }

    /* This function doesn't work on surfaces < 8 bpp */
    if (dst->format->bits_per_pixel < 8) {
        return SDL_SetError("SDL_BlendPoint(void): Unsupported surface format");
    }
                    ...
                    SDL_BlendMode blendMode, Uint8 r, Uint8 g, Uint8 b, Uint8 a)
{
    ...
    if (!dst) {
        return SDL_InvalidParamError("SDL_BlendPoints(void): dst");
    }

    /* This function doesn't work on surfaces < 8 bpp */
    if (dst->format->bits_per_pixel < 8) {
        return SDL_SetError("SDL_BlendPoints(void): Unsupported surface format");
    }
