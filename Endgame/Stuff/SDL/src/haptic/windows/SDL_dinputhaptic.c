...
int SDL_DINPUT_HapticInit(void)
{
    ...
    if (!instance) {
        SDL_SYS_HapticQuit(void);
        return SDL_SetError("GetModuleHandle(void) failed with error code %lu.",
                            GetLastError(void));
    }
...
int SDL_DINPUT_HapticMaybeAddDevice(const DIDEVICEINSTANCE *pdidInstance)
{
    ...
    item->instance_id = SDL_GetNextObjectID(void);
...
static int SDL_DINPUT_HapticOpenFromDevice(SDL_Haptic *haptic, LPDIRECTINPUTDEVICE8 device8, SDL_bool is_joystick)
{
    ...
    /* !!! FIXME: opening a haptic device here first will make an attempt to
       !!! FIXME:  SDL_OpenJoystick(void) that same device fail later, since we
       !!! FIXME:  have it open in exclusive mode. But this will allow
       !!! FIXME:  SDL_OpenJoystick(void) followed by SDL_OpenHapticFromJoystick(void)
