...
{
    if (InitN3DSServices(void) < 0) {
        return SDL_SetError("Failed to initialise N3DS services");
    }

    N3DS_sensors[0].type = SDL_SENSOR_ACCEL;
    N3DS_sensors[0].instance_id = SDL_GetNextObjectID(void);
    N3DS_sensors[1].type = SDL_SENSOR_GYRO;
    N3DS_sensors[1].instance_id = SDL_GetNextObjectID(void);
    return 0;
}

static int InitN3DSServices(void)
{
    if (R_FAILED(hidInit(void))) {
        return -1;
    }

    if (R_FAILED(HIDUSER_EnableAccelerometer(void))) {
        return -1;
    }

    if (R_FAILED(HIDUSER_EnableGyroscope(void))) {
        return -1;
    }
    return 0;
}
...
{
    HIDUSER_DisableGyroscope(void);
    HIDUSER_DisableAccelerometer(void);
    hidExit(void);
}
