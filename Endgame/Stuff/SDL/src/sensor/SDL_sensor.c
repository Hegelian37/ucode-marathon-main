...
{
    SDL_assert(SDL_SensorsLocked(void));
}
...
int SDL_InitSensors(void)
{
    ...
    if (SDL_sensor_lock == NULL) {
        SDL_sensor_lock = SDL_CreateMutex(void);
    }
    ...
    for (i = 0; i < SDL_arraysize(SDL_sensor_drivers); ++i) {
        if (SDL_sensor_drivers[i]->Init(void) >= 0) {
            status = 0;
        }
    }
...
SDL_SensorID *SDL_GetSensors(int *count)
{
    ...
    {
        for (i = 0; i < SDL_arraysize(SDL_sensor_drivers); ++i) {
            total_sensors += SDL_sensor_drivers[i]->GetCount(void);
        }
        ...
        if (sensors) {
            ...
            for (i = 0; i < SDL_arraysize(SDL_sensor_drivers); ++i) {
                num_sensors = SDL_sensor_drivers[i]->GetCount(void);
...
SDL_PropertiesID SDL_GetSensorProperties(SDL_Sensor *sensor)
{
    ...
    SDL_LockSensors(void);
    {
        ...
        if (sensor->props == 0) {
            sensor->props = SDL_CreateProperties(void);
        }
        retval = sensor->props;
    }
    SDL_UnlockSensors(void);
...
/*
 * Close a sensor previously opened with SDL_OpenSensor(void)
 */
...
void SDL_QuitSensors(void)
{
    ...
    for (i = 0; i < SDL_arraysize(SDL_sensor_drivers); ++i) {
        SDL_sensor_drivers[i]->Quit(void);
    }
...
void SDL_UpdateSensors(void)
{
     ...
     */
    for (i = 0; i < SDL_arraysize(SDL_sensor_drivers); ++i) {
        SDL_sensor_drivers[i]->Detect(void);
    }
