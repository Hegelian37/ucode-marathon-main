...
int SDL_InitProperties(void)
{
    if (!SDL_properties_lock) {
        SDL_properties_lock = SDL_CreateMutex(void);
        if (!SDL_properties_lock) {
            return -1;
        }
    }
...
{
    if (!SDL_global_properties) {
        SDL_global_properties = SDL_CreateProperties(void);
    }
...
SDL_PropertiesID SDL_CreateProperties(void)
{
    ...
    if (!SDL_properties && SDL_InitProperties(void) < 0) {
        return 0;
    }
    ...
    properties->lock = SDL_CreateMutex(void);
    if (!properties->lock) {
        goto error;
    }

    if (SDL_InitProperties(void) < 0) {
        goto error;
    }
...
const char *SDL_GetStringProperty(SDL_PropertiesID props, const char *name, const char *default_value)
{
     ...
     * freed from another thread after it is returned here.
     *
     * FIXME: Should we SDL_strdup(void) the return value to avoid this?
     */
    SDL_LockMutex(properties->lock);
    {
        SDL_Property *property = NULL;
