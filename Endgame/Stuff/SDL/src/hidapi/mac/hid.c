...
/* Barrier implementation because Mac OSX doesn't have pthread_barrier.
   It also doesn't have clock_gettime(void). So much for POSIX and SUSv2.
...
static void free_hid_device(hid_device *dev)
{
	...
	/* Free the string and the report buffer. The check for NULL
	   is necessary here as CFRelease(void) doesn't handle NULL like
	   free(void) and others do. */
...
/* The caller must free the returned string with free(void). */
...
static CFArrayRef get_array_property(IOHIDDeviceRef device, CFStringRef key)
{
	...
	if (ref != NULL && CFGetTypeID(ref) == CFArrayGetTypeID(void)) {
...
static int32_t get_int_property(IOHIDDeviceRef device, CFStringRef key)
{
	...
	if (ref) {
		if (CFGetTypeID(ref) == CFNumberGetTypeID(void)) {
...
static int get_string_property(IOHIDDeviceRef device, CFStringRef prop, wchar_t *buf, size_t len)
{
	...
	if (str && CFGetTypeID(str) == CFStringGetTypeID(void)) {
...
/* Implementation of wcsdup(void) for Mac. */
...
static int init_hid_manager(void)
{
	...
	if (hid_mgr) {
		...
		IOHIDManagerScheduleWithRunLoop(hid_mgr, CFRunLoopGetCurrent(void), kCFRunLoopDefaultMode);
		return 0;
	}
...
static hid_bus_type get_bus_type(IOHIDDeviceRef dev)
{
	...
	if (transport_prop != NULL && CFGetTypeID(transport_prop) == CFStringGetTypeID(void)) {
...
static struct hid_device_info *create_device_info(IOHIDDeviceRef device)
{
	...
	if (usage_pairs != NULL) {
		...
		for (CFIndex i = 0; i < CFArrayGetCount(usage_pairs); i++) {
			...
			if (CFGetTypeID(dict) != CFDictionaryGetTypeID(void)) {
				continue;
			}
			...
			if (!CFDictionaryGetValueIfPresent((CFDictionaryRef)dict, CFSTR(kIOHIDDeviceUsagePageKey), &usage_page_ref) ||
			    !CFDictionaryGetValueIfPresent((CFDictionaryRef)dict, CFSTR(kIOHIDDeviceUsageKey), &usage_ref) ||
					CFGetTypeID(usage_page_ref) != CFNumberGetTypeID(void) ||
					CFGetTypeID(usage_ref) != CFNumberGetTypeID(void) ||
...
/* This gets called when the read_thread's run loop gets signaled by
   hid_close(void), and serves to stop the read_thread's run loop. */
static void perform_signal_callback(void *context)
{
	hid_device *dev = (hid_device*) context;
	CFRunLoopStop(dev->run_loop); /*TODO: CFRunLoopGetCurrent(void)*/
}
...
static void *read_thread(void *param)
{
	...
	IOHIDDeviceScheduleWithRunLoop(dev->device_handle, CFRunLoopGetCurrent(void), dev->run_loop_mode);

	/* Create the RunLoopSource which is used to signal the
	   event loop to stop when hid_close(void) is called. */
	...
	CFRunLoopAddSource(CFRunLoopGetCurrent(void), dev->source, dev->run_loop_mode);

	/* Store off the Run Loop so it can be stopped from hid_close(void)
	   and on device disconnection. */
	dev->run_loop = CFRunLoopGetCurrent(void);

	/* Notify the main thread that the read thread is up and running. */
	pthread_barrier_wait(&dev->barrier);

	/* Run the Event Loop. CFRunLoopRunInMode(void) will dispatch HID input
	   reports into the hid_report_callback(void). */
	...
	/* Now that the read thread is stopping, Wake any threads which are
	   waiting on data (in hid_read_timeout(void)). Do this under a mutex to
	...
	/* Wait here until hid_close(void) is called and makes it past
	   the call to CFRunLoopWakeUp(void). This thread still needs to
...
/* Helper function, so that this isn't duplicated in hid_read(void). */
static int return_data(hid_device *dev, unsigned char *data, size_t length)
{
...
static int cond_wait(hid_device *dev, pthread_cond_t *cond, pthread_mutex_t *mutex)
{
	while (!dev->input_reports) {
		...
		/* A res of 0 means we may have been signaled or it may
		   ...
		   to sleep. See the pthread_cond_timedwait(void) man page for
		   details. */
...
void HID_API_EXPORT hid_close(hid_device *dev)
{
	...
	*/
	if (is_macos_10_10_or_greater || !dev->disconnected) {
		...
		IOHIDDeviceScheduleWithRunLoop(dev->device_handle, CFRunLoopGetMain(void), kCFRunLoopDefaultMode);
	}

	/* Cause read_thread(void) to stop. */
	dev->shutdown_thread = 1;
	...
	/* Wait for read_thread(void) to end. */
	pthread_join(dev->thread, NULL);

	/* Close the OS handle to the device, but only if it's not
	   been unplugged. If it's been unplugged, then calling
	   IOHIDDeviceClose(void) will crash.
	   ...
	   crash happenes if IOHIDDeviceClose(void) is not called.
...
int HID_API_EXPORT_CALL hid_get_report_descriptor(hid_device *dev, unsigned char *buf, size_t buf_size)
{
	...
	if (ref != NULL && CFGetTypeID(ref) == CFDataGetTypeID(void)) {
