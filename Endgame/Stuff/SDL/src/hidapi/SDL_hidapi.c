...
static void HIDAPI_InitializeDiscovery(void)
{
...
#if defined(SDL_PLATFORM_WIN32) || defined(SDL_PLATFORM_WINGDK)
    SDL_HIDAPI_discovery.m_nThreadID = SDL_GetCurrentThreadID(void);
    ...
    if (linux_enumeration_method == ENUMERATION_LIBUDEV) {
        ...
        usyms = SDL_UDEV_GetUdevSyms(void);
        if (usyms != NULL) {
            SDL_HIDAPI_discovery.m_pUdev = usyms->udev_new(void);
...
static void HIDAPI_UpdateDiscovery(void)
{
    ...
    if (SDL_GetCurrentThreadID(void) == SDL_HIDAPI_discovery.m_nThreadID) {
        MSG msg;
...
#undef LOAD_LIBUSB_SYMBOL
            ...
            } else if (LIBUSB_hid_init(void) < 0) {
#ifdef SDL_LIBUSB_DYNAMIC
                SDL_UnloadObject(libusb_ctx.libhandle);
#endif
                libusb_ctx.libhandle = NULL;
    ...
    ++attempts;
#ifdef SDL_PLATFORM_LINUX
    udev_ctx = SDL_UDEV_GetUdevSyms(void);
#endif /* __LINUX __ */
    if (udev_ctx && PLATFORM_hid_init(void) == 0) {
        ++success;
    }
#endif /* HAVE_PLATFORM_BACKEND */
...
#ifndef SDL_HIDAPI_DISABLED
    if (SDL_hidapi_refcount == 0 && SDL_hid_init(void) != 0) {
        return 0;
    }
