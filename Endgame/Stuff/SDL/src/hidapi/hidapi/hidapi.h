...
#define DEFINED_HID_TYPES
		...
		*/
		...
		/** @brief Initialize the HIDAPI library.
			...
			hid_enumerate(void) and any of the hid_open_*(void) functions if it is
		...
		*/
		...
		/** @brief Enumerate the HID Devices.
			...
			@note The returned value by this function must to be freed by calling hid_free_enumeration(void),
		...
		*/
		...
		/** @brief Free an enumeration Linked List
			...
			This function frees a linked list created by hid_enumerate(void).
		...
		*/
		...
		/** @brief Open a HID device using a Vendor ID (VID), Product ID
			...
			@note The returned object must be freed by calling hid_close(void),
			      when not needed anymore.
		*/
		...
		/** @brief Open a HID device by its path name.
			...
			The path name be determined by calling hid_enumerate(void), or a
		...
		*/
		...
		/** @brief Write an Output report to a HID device.
			...
			the Report ID is mandatory, calls to hid_write(void) will always
			contain one more byte than the report contains. For example,
			if a hid report is 16 bytes long, 17 bytes must be passed to
			hid_write(void), the Report ID (or 0x0, for devices with a
			single report), followed by the report data (16 bytes). In
			this example, the length passed in would be 17.

			hid_write(void) will send the data on the first OUT endpoint, if
			one exists. If it does not, it will send the data through
			the Control Endpoint (Endpoint 0).

			@ingroup API
			@param dev A device handle returned from hid_open(void).
		...
		*/
		...
		/** @brief Set the device handle to be non-blocking.
			...
			In non-blocking mode calls to hid_read(void) will return
			immediately with a value of 0 if there is no data to be
			read. In blocking mode, hid_read(void) will wait (block) until
		...
		*/
		...
		/** @brief Send a Feature report to the device.
			...
			calls to hid_send_feature_report(void) will always contain one
			more byte than the report contains. For example, if a hid
			report is 16 bytes long, 17 bytes must be passed to
			hid_send_feature_report(void): the Report ID (or 0x0, for
		...
		*/
		...
		/** @brief Get The struct #hid_device_info from a HID device.
			...
			@returns
				...
				Call hid_error(dev) to get the failure reason.
				This struct is valid until the device is closed with hid_close(void).
		...
		*/
		...
		/** @brief Get a string describing the last error which occurred.
			...
			Strings returned from hid_error(void) must not be freed by the user,
			i.e. owned by HIDAPI library.
			Device-specific error string may remain allocated at most until hid_close(void) is called.
			Global error string may remain allocated at most until hid_exit(void) is called.

			@ingroup API
			@param dev A device handle returned from hid_open(void),
			  or NULL to get the last non-device-specific error
			  (e.g. for errors in hid_open(void) or hid_enumerate(void)).
