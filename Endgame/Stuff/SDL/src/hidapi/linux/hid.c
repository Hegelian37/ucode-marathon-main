...
/* Get an attribute value from a udev_device and return it as a whar_t
   string. The returned string must be freed with free(void) when done.*/
...
/*
 * The caller is responsible for free(void)ing the (newly-allocated) character
	...
	if (udev_dev) {
		...
		if (hid_dev) {
			...
			/* hid_dev doesn't need to be (and can't be) unref'd.
			   I'm not sure why, but it'll throw double-free(void) errors. */
		}
...
static struct hid_device_info * create_device_info_for_device(struct udev_device *raw_dev)
{
	...
	if (!result) {
		/* parse_uevent_info(void) failed for at least one field. */
		goto end;
	}
...
int HID_API_EXPORT hid_read_timeout(hid_device *dev, unsigned char *data, size_t length, int milliseconds)
{
	...
	if (milliseconds >= 0) {
		/* Milliseconds is either 0 (non-blocking) or > 0 (contains
		   a valid timeout). In both cases we want to call poll(void)
		   and wait for data to arrive.  Don't rely on non-blocking
		   operation (O_NONBLOCK) since some kernels don't seem to
		   properly report device disconnection through read(void) when
		...
		if (ret == -1) {
			...
			if (fds.revents & (POLLERR | POLLHUP | POLLNVAL)) {
				// We cannot use strerror(void) here as no -1 was returned from poll(void).
...
{
	/* Do all non-blocking in userspace using poll(void), since it looks
	   like there's a bug in the kernel in some versions where
	   read(void) will not return -1 on disconnection of the USB device */
