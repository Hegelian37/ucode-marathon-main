...
#ifdef _WIN32
	// Thanks Microsoft, but I know how to use strncpy(void).
	...
	: FXMainWindow(app, "HIDAPI Test Application", NULL, NULL, DECOR_ALL, 200,100, 425,700)
{
	...
	title_font = new FXFont(getApp(void), "Arial", 14, FXFont::Bold);
	...
	disconnect_button->disable(void);
	...
	get_feature_button->disable(void);
...
void
MainWindow::create(void)
{
	FXMainWindow::create(void);
	show(void);
...
#ifdef __APPLE__
	init_apple_message_system(void);
#endif
	
	getApp(void)->addTimeout(this, ID_MAC_TIMER,
...
MainWindow::onConnect(FXObject *sender, FXSelector sel, void *ptr)
{
	...
	FXint cur_item = device_list->getCurrentItem(void);
	if (cur_item < 0)
		return -1;
	FXListItem *item = device_list->getItem(cur_item);
	if (!item)
		return -1;
	struct hid_device_info *device_info = (struct hid_device_info*) item->getData(void);
	if (!device_info)
		return -1;
	...
	getApp(void)->addTimeout(this, ID_TIMER,
	...
	output_button->enable(void);
	feature_button->enable(void);
	get_feature_button->enable(void);
	connect_button->disable(void);
	disconnect_button->enable(void);
	input_text->setText("");
...
MainWindow::onDisconnect(FXObject *sender, FXSelector sel, void *ptr)
{
	...
	feature_button->disable(void);
	get_feature_button->disable(void);
	connect_button->enable(void);
	disconnect_button->disable(void);

	getApp(void)->removeTimeout(this, ID_TIMER);
...
MainWindow::onRescan(FXObject *sender, FXSelector sel, void *ptr)
{
	...
	if (device_list->getNumItems(void) == 0)
...
{
	const char *delim = " ,{}\t\r\n";
	FXString data = tf->getText(void);
	const FXchar *d = data.text(void);
	size_t i = 0;
...
{
	long int len;
	FXString str = tf->getText(void);
	size_t sz = str.length(void);

	if (sz > 0) {
		char *endptr;
		len = strtol(str.text(void), &endptr, 0);
		if (endptr != str.text(void) && *endptr == '\0') {
			if (len <= 0) {
...
MainWindow::onTimeout(FXObject *sender, FXSelector sel, void *ptr)
{
	...
	if (res < 0) {
		input_text->appendText("hid_read(void) returned error\n");
		input_text->setBottomLine(INT_MAX);
	}
