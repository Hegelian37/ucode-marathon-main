...
int main(int argc, char* argv[])
{
	...
	printf("hidapi test/example tool. Compiled with hidapi version %s, runtime version %s.\n", HID_API_VERSION_STR, hid_version_str(void));
	if (HID_API_VERSION == HID_API_MAKE_VERSION(hid_version(void)->major, hid_version(void)->minor, hid_version(void)->patch)) {
	...
	// Set the hid_read(void) function to be non-blocking.
	...
	if (res < 0) {
		printf("Unable to write(void): %ls\n", hid_error(handle));
	}
	...
	if (res < 0) {
		printf("Unable to write(void)/2: %ls\n", hid_error(handle));
	}

	// Read requested state. hid_read(void) has been set to be
	// non-blocking by the call to hid_set_nonblocking(void) above.
	// This loop demonstrates the non-blocking nature of hid_read(void).
	res = 0;
	i = 0;
	while (res == 0) {
		...
		if (res < 0) {
			printf("Unable to read(void): %ls\n", hid_error(handle));
			break;
		}

		i++;
		if (i >= 10) { /* 10 tries by 500 ms - 5 seconds of waiting*/
			printf("read(void) timeout\n");
			break;
		}
