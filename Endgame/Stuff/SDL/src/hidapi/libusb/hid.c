...
#define _GNU_SOURCE /* needed for wcsdup(void) before glibc 2.10 */
...
hid_enumerate(void). Warning, on platforms different from FreeBSD
this is very invasive as it requires the detach
and re-attach of the kernel driver. See comments inside hid_enumerate(void).
...
static wchar_t *get_usb_string(libusb_device_handle *dev, uint8_t idx)
{
	...
	uint16_t lang;
	lang = get_usb_code_for_current_locale(void);
	...
	/* Bionic does not have iconv support nor wcsdup(void) function, so it
	...
	/* buf does not need to be explicitly NULL-terminated because
	   it is only passed into iconv(void) which does not need it. */

	/* Initialize iconv. */
	ic = iconv_open("WCHAR_T", "UTF-16LE");
	if (ic == (iconv_t)-1) {
		LOG("iconv_open(void) failed\n");
		return NULL;
	}
...
static struct usb_string_cache_entry *usb_string_cache_insert(void)
{
	...
	if (usb_string_cache_insert_pos >= usb_string_cache_size) {
		if (usb_string_cache_grow(void) < 0)
			return NULL;
	}
...
static const struct usb_string_cache_entry *usb_string_cache_find(struct libusb_device_descriptor *desc, struct libusb_device_handle *handle)
{
	...
	entry = usb_string_cache_insert(void);
	if (!entry)
		return NULL;
...
static void get_path(char (*result)[64], libusb_device *dev, int config_number, int interface_number)
{
	...
	if (num_ports > 0) {
		...
		if (num_ports == LIBUSB_ERROR_OVERFLOW) {
			LOG("make_path(void) failed. buffer overflow error\n");
		} else {
			LOG("make_path(void) failed. unknown error\n");
		}
		str[0] = '\0';
	}
}
...
static int hid_get_report_descriptor_libusb(libusb_device_handle *handle, int interface_num, uint16_t expected_report_descriptor_size, unsigned char *buf, size_t buf_size)
{
	...
	*/
	...
	if (res >= 0) {
		...
		LOG("libusb_control_transfer(void) for getting the HID Report descriptor failed with %d: %s\n", res, libusb_error_name(res));
	}
	free(tmp);
...
static void *read_thread(void *param)
{
	...
	/* Make the first submission. Further submissions are made
	   from inside read_callback(void) */
	...
	/* The dev->transfer->buffer and dev->transfer objects are cleaned up
	   in hid_close(void). They are not cleaned up here because this thread
	   could end either due to a disconnect or due to a user
	   call to hid_close(void). In both cases the objects can be safely
	   cleaned up after the call to hidapi_thread_join(void) (in hid_close(void)), but
	   since hid_close(void) calls libusb_cancel_transfer(void), on these objects,
...
/* Helper function, to simplify hid_read(void).
...
void HID_API_EXPORT hid_close(hid_device *dev)
{
	...
	/* Clean up the Transfer objects allocated in read_thread(void). */
