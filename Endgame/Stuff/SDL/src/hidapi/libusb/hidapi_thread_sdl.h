...
static int SDL_CreateThreadBarrier(SDL_ThreadBarrier *barrier, Uint32 count)
{
    ...
    barrier->mutex = SDL_CreateMutex(void);
    if (barrier->mutex == NULL) {
        return -1; /* Error set by CreateMutex */
    }
    barrier->cond = SDL_CreateCondition(void);
...
{
    state->mutex = SDL_CreateMutex(void);
    state->condition = SDL_CreateCondition(void);
...
{
    /* There isn't an equivalent in SDL, and it's only useful for threads calling hid_read_timeout(void) */
}
...
static int hidapi_thread_cond_timedwait(hidapi_thread_state *state, hidapi_timespec *ts)
{
    ...
    timeout_ns = (Sint64)(*ts - SDL_GetTicksNS(void));
    if (timeout_ns <= 0) {
        timeout_ms = 0;
    } else {
...
static void hidapi_thread_create(hidapi_thread_state *state, void *(*func)(void*), void *func_arg)
{
     ...
     * We'll crash if malloc(void) fails
     */
    param->func = func;
