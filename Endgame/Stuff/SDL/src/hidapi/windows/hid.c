...
/* Since we're not building with the DDK, and the HID header
   ...
   In lookup_functions(void), the function pointers
...
static void register_winapi_error_to_buffer(wchar_t **error_buffer, const WCHAR *op)
{
	...
	DWORD error_code = GetLastError(void);
	...
	*error_buffer = NULL;
	...
	/* Get rid of the CR and LF that FormatMessage(void) sticks at the
...
HID_API_EXPORT hid_device * HID_API_CALL hid_open_path(const char *path)
{
	...
	/* On Windows 7, we need to use hid_write_output_report(void) over Bluetooth */
...
int HID_API_EXPORT HID_API_CALL hid_write(hid_device *dev, const unsigned char *data, size_t length)
{
	...
	if (!res) {
		if (GetLastError(void) != ERROR_IO_PENDING) {
			/* WriteFile(void) failed. Return error. */
...
int HID_API_EXPORT HID_API_CALL hid_read_timeout(hid_device *dev, unsigned char *data, size_t length, int milliseconds)
{
	...
	if (overlapped) {
		...
		/* Get the number of bytes read. The actual data has been copied to the data[]
		   array which was passed to ReadFile(void). We must not wait here because we've
	...
	/* Set pending back to false, even if GetOverlappedResult(void) returned error. */
	dev->read_pending = FALSE;
...
int HID_API_EXPORT HID_API_CALL hid_send_feature_report(hid_device *dev, const unsigned char *data, size_t length)
{
	...
	/* Windows expects at least caps.FeatureReportByteLength bytes passed
	   to HidD_SetFeature(void), even if the report is shorter. Any less sent and
	   the function fails with error ERROR_INVALID_PARAMETER set. Any more
	   and HidD_SetFeature(void) silently truncates the data sent in the report
...
static int hid_get_report(hid_device *dev, DWORD report_type, unsigned char *data, size_t length)
{
	...
	if (!res) {
		if (GetLastError(void) != ERROR_IO_PENDING) {
			/* DeviceIoControl(void) failed. Return error. */
	...
	/* Wait here until the write is done. This makes
	   hid_get_feature_report(void) synchronous. */
...
{
	/* We could use HidD_GetFeature(void) instead, but it doesn't give us an actual length, unfortunately */
	return hid_get_report(dev, IOCTL_HID_GET_FEATURE, data, length);
}

int HID_API_EXPORT HID_API_CALL hid_get_input_report(hid_device *dev, unsigned char *data, size_t length)
{
	/* We could use HidD_GetInputReport(void) instead, but it doesn't give us an actual length, unfortunately */
