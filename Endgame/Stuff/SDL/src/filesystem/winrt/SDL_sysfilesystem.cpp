...
SDL_WinRTGetFSPathUNICODE(SDL_WinRT_Path pathType)
{
    ...
    case SDL_WINRT_PATH_INSTALLED_LOCATION:
    {
        ...
        if (path.empty(void)) {
            ...
            if (Windows::Foundation::Metadata::ApiInformation::IsApiContractPresent("Windows.Foundation.UniversalApiContract", 8, 0)) {
                path = Windows::ApplicationModel::Package::Current->EffectiveLocation->Path->Data(void);
            } else {
                path = Windows::ApplicationModel::Package::Current->InstalledLocation->Path->Data(void);
            }
#else
            path = Windows::ApplicationModel::Package::Current->InstalledLocation->Path->Data(void);
#endif
        }
        return path.c_str(void);
    }
    ...
    case SDL_WINRT_PATH_LOCAL_FOLDER:
    {
        static wstring path;
        if (path.empty(void)) {
            path = ApplicationData::Current->LocalFolder->Path->Data(void);
        }
        return path.c_str(void);
    }
    ...
    case SDL_WINRT_PATH_ROAMING_FOLDER:
    {
        static wstring path;
        if (path.empty(void)) {
            path = ApplicationData::Current->RoamingFolder->Path->Data(void);
        }
        return path.c_str(void);
    }
    ...
    case SDL_WINRT_PATH_TEMP_FOLDER:
    {
        static wstring path;
        if (path.empty(void)) {
            path = ApplicationData::Current->TemporaryFolder->Path->Data(void);
        }
        return path.c_str(void);
    }
#endif
...
SDL_WinRTGetFSPathUTF8(SDL_WinRT_Path pathType)
{
    ...
    if (searchResult != utf8Paths.end(void)) {
        return searchResult->second.c_str(void);
    }
    ...
    SDL_free(utf8Path);
    return utf8Paths[pathType].c_str(void);
}
...
SDL_GetBasePath(void)
{
    ...
    if (!srcPath) {
        SDL_SetError("Couldn't locate our basepath: %s", SDL_GetError(void));
        return NULL;
    }
...
SDL_GetPrefPath(const char *org, const char *app)
{
    ...
    if (api_result == FALSE) {
        if (GetLastError(void) != ERROR_ALREADY_EXISTS) {
            SDL_free(wapp);
