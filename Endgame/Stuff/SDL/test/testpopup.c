...
static void loop(void)
{
    ...
    while (SDL_PollEvent(&event)) {
        if (event.type == SDL_EVENT_MOUSE_MOTION) {
            ...
            tooltip_timer = SDL_GetTicks(void) + TOOLTIP_DELAY;
            ...
            if (event.key.keysym.sym == SDLK_SPACE) {
                ...
                /* Don't process this event in SDLTest_CommonEvent(void) */
                continue;
            }
        }
    ...
    if (SDL_GetTicks(void) > tooltip_timer) {
        if (!tooltip.win) {
