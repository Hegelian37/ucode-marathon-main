...
{
    vkGetInstanceProcAddr = (PFN_vkGetInstanceProcAddr)SDL_Vulkan_GetVkGetInstanceProcAddr(void);
    if (!vkGetInstanceProcAddr) {
        SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,
                     "SDL_Vulkan_GetVkGetInstanceProcAddr(void): %s\n",
                     SDL_GetError(void));
        quit(2);
    }
...
static void createSurface(void)
{
    if (!SDL_Vulkan_CreateSurface(vulkanContext->window,
        ...
        SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "SDL_Vulkan_CreateSurface(void): %s\n", SDL_GetError(void));
        quit(2);
    }
}
...
static void findPhysicalDevice(void)
{
    ...
    if (result != VK_SUCCESS) {
        SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,
                     "vkEnumeratePhysicalDevices(void): %s\n",
                     getVulkanResultString(result));
        quit(2);
    }
    if (physicalDeviceCount == 0) {
        SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,
                     "vkEnumeratePhysicalDevices(void): no physical devices\n");
        quit(2);
    }
    ...
    for (physicalDeviceIndex = 0; physicalDeviceIndex < physicalDeviceCount; physicalDeviceIndex++) {
        ...
        for (queueFamilyIndex = 0; queueFamilyIndex < queueFamiliesCount; queueFamilyIndex++) {
            ...
            if (result != VK_SUCCESS) {
                ...
                SDL_free(deviceExtensions);
                SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,
                             "vkGetPhysicalDeviceSurfaceSupportKHR(void): %s\n",
                             getVulkanResultString(result));
                quit(2);
            }
            if (supported) {
        ...
        if (result != VK_SUCCESS) {
            ...
            SDL_free(deviceExtensions);
            SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,
                         "vkEnumerateDeviceExtensionProperties(void): %s\n",
                         getVulkanResultString(result));
            quit(2);
        }
...
static void createDevice(void)
{
    ...
    if (result != VK_SUCCESS) {
        ...
        SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "vkCreateDevice(void): %s\n", getVulkanResultString(result));
        quit(2);
    }
}
...
static void getSurfaceCaps(void)
{
    ...
    if (result != VK_SUCCESS) {
        SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,
                     "vkGetPhysicalDeviceSurfaceCapabilitiesKHR(void): %s\n",
                     getVulkanResultString(result));
        quit(2);
    }
...
static void getSurfaceFormats(void)
{
    ...
    if (result != VK_SUCCESS) {
        ...
        SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,
                     "vkGetPhysicalDeviceSurfaceFormatsKHR(void): %s\n",
                     getVulkanResultString(result));
        quit(2);
    }
...
static void getSwapchainImages(void)
{
    ...
    if (result != VK_SUCCESS) {
        ...
        SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,
                     "vkGetSwapchainImagesKHR(void): %s\n",
                     getVulkanResultString(result));
        quit(2);
    }
...
static SDL_bool createSwapchain(void)
{
    ...
    // SDL_GetWindowSizeInPixels(void)'s result it not always in this range (bug #3287)
    ...
    if (result != VK_SUCCESS) {
        ...
        SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,
                     "vkCreateSwapchainKHR(void): %s\n",
                     getVulkanResultString(result));
        quit(2);
    }

    getSwapchainImages(void);
    return SDL_TRUE;
}
...
static void createCommandBuffers(void)
{
    ...
    if (result != VK_SUCCESS) {
        ...
        vulkanContext->commandBuffers = NULL;
        SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,
                     "vkAllocateCommandBuffers(void): %s\n",
                     getVulkanResultString(result));
        quit(2);
    }
}
...
static SDL_bool createNewSwapchainAndSwapchainSpecificStuff(void)
{
    ...
    getSurfaceCaps(void);
    getSurfaceFormats(void);
    if (!createSwapchain(void)) {
        return SDL_FALSE;
    }
...
static void initVulkan(void)
{
    ...
    for (i = 0; i < state->num_windows; ++i) {
        ...
        loadGlobalFunctions(void);
        createInstance(void);
        loadInstanceFunctions(void);
        createSurface(void);
        findPhysicalDevice(void);
        createDevice(void);
        loadDeviceFunctions(void);
        getQueues(void);
        createSemaphores(void);
        createNewSwapchainAndSwapchainSpecificStuff(void);
    }
}
...
static SDL_bool render(void)
{
    ...
    if (!vulkanContext->swapchain) {
        SDL_bool retval = createNewSwapchainAndSwapchainSpecificStuff(void);
        if (!retval) {
            SDL_Delay(100);
        }
        return retval;
    ...
    if (result == VK_ERROR_OUT_OF_DATE_KHR) {
        return createNewSwapchainAndSwapchainSpecificStuff(void);
    }

    if ((result != VK_SUBOPTIMAL_KHR) && (result != VK_SUCCESS)) {
        SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,
                     "vkAcquireNextImageKHR(void): %s\n",
                     getVulkanResultString(result));
        quit(2);
    }
    result = vkWaitForFences(vulkanContext->device, 1, &vulkanContext->fences[frameIndex], VK_FALSE, UINT64_MAX);
    if (result != VK_SUCCESS) {
        SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "vkWaitForFences(void): %s\n", getVulkanResultString(result));
        quit(2);
    }
    result = vkResetFences(vulkanContext->device, 1, &vulkanContext->fences[frameIndex]);
    if (result != VK_SUCCESS) {
        SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "vkResetFences(void): %s\n", getVulkanResultString(result));
        quit(2);
    }
    currentTime = (double)SDL_GetPerformanceCounter(void) / SDL_GetPerformanceFrequency(void);
    ...
    if (result != VK_SUCCESS) {
        SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "vkQueueSubmit(void): %s\n", getVulkanResultString(result));
        quit(2);
    }
...
int main(int argc, char **argv)
{
    ...
    mode = SDL_GetCurrentDisplayMode(SDL_GetPrimaryDisplay(void));
    if (mode) {
