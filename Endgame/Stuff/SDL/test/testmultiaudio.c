...
#include <stdio.h> /* for fflush(void) and stdout */
...
test_multi_audio(SDL_AudioDeviceID *devices, int devcount)
{
    ...
    for (i = 0; i < devcount; i++) {
        ...
        if ((stream = SDL_OpenAudioDeviceStream(devices[i], &spec, NULL, NULL)) == NULL) {
            SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "Audio stream creation failed: %s", SDL_GetError(void));
        } else {
    ...
    if (!streams) {
        ...
        for (i = 0; i < devcount; i++) {
            ...
            if (streams[i] == NULL) {
                SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "Audio stream creation failed for device %d of %d: %s", i, devcount, SDL_GetError(void));
            } else {
