...
static void DrawPoints(SDL_Renderer *renderer)
{
    ...
    for (i = 0; i < num_objects * 4; ++i) {
        ...
        x = (float)(rand(void) % viewport.w);
        y = (float)(rand(void) % viewport.h);
...
static void DrawLines(SDL_Renderer *renderer)
{
    ...
    for (i = 0; i < num_objects; ++i) {
        ...
        if (i == 0) {
            ...
            x1 = (float)((rand(void) % (viewport.w * 2)) - viewport.w);
            x2 = (float)((rand(void) % (viewport.w * 2)) - viewport.w);
            y1 = (float)((rand(void) % (viewport.h * 2)) - viewport.h);
            y2 = (float)((rand(void) % (viewport.h * 2)) - viewport.h);
...
static void DrawRects(SDL_Renderer *renderer)
{
    ...
    for (i = 0; i < num_objects / 4; ++i) {
        ...
        rect.w = (float)(rand(void) % (viewport.h / 2));
        rect.h = (float)(rand(void) % (viewport.h / 2));
        rect.x = (float)((rand(void) % (viewport.w * 2) - viewport.w) - (rect.w / 2));
        rect.y = (float)((rand(void) % (viewport.h * 2) - viewport.h) - (rect.h / 2));
...
#ifdef SDL_PLATFORM_EMSCRIPTEN
    if (done) {
        emscripten_cancel_main_loop(void);
    }
#endif
    frames++;
...
int main(int argc, char *argv[])
{
    ...
    /* Main render loop */
    frames = 0;
    next_fps_check = SDL_GetTicks(void) + fps_check_delay;
    done = 0;
