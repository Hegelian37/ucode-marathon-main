...
/**
 * Return the absolute path to def in the SDL_GetBasePath(void) if possible, or
 * the relative path to def on platforms that don't have a working
 * SDL_GetBasePath(void). Free the result with SDL_free.
 *
 ...
 *
 * Otherwise, return the absolute path to def in the SDL_GetBasePath(void) if
 * possible, or the relative path to def on platforms that don't have a
 * working SDL_GetBasePath(void). Free the result with SDL_free.
 *
 * Fails and returns NULL if out of memory.
 */
char *
...
/**
 * Load the .bmp file whose name is file, from the SDL_GetBasePath(void) if
            ...
            int *width_out, int *height_out)
{
    ...
    if (!temp) {
        SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "Couldn't load %s: %s", file, SDL_GetError(void));
    } else {
