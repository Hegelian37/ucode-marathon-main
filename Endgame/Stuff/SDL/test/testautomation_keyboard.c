...
static int keyboard_getKeyboardFocus(void *arg)
{
    ...
    SDLTest_AssertPass("Call to SDL_GetKeyboardFocus(void)");
...
static void checkInvalidScancodeError(void)
{
    ...
    const char *error;
    error = SDL_GetError(void);
    SDLTest_AssertPass("Call to SDL_GetError(void)");
    SDLTest_AssertCheck(error != NULL, "Validate that error message was not NULL");
    if (error != NULL) {
        SDLTest_AssertCheck(SDL_strcmp(error, expectedError) == 0,
                            "Validate error message, expected: '%s', got: '%s'", expectedError, error);
        SDL_ClearError(void);
        SDLTest_AssertPass("Call to SDL_ClearError(void)");
    }
}
...
static int keyboard_getKeyFromScancode(void *arg)
{
    ...
    SDLTest_AssertCheck(result == SDLK_UNKNOWN, "Verify result from call is UNKNOWN, expected: %i, got: %" SDL_PRIs32, SDLK_UNKNOWN, result);
    checkInvalidScancodeError(void);
...
static int keyboard_getKeyName(void *arg)
{
    ...
    SDLTest_AssertPass("Call to SDL_GetKeyName(void)");
...
static int keyboard_getSetModState(void *arg)
{
    ...
    result = SDL_GetModState(void);
    SDLTest_AssertPass("Call to SDL_GetModState(void)");
...
static int keyboard_startStopTextInput(void *arg)
{
    ...
    SDLTest_AssertPass("Call to SDL_StartTextInput(void)");
    SDL_StopTextInput(void);
    SDLTest_AssertPass("Call to SDL_StopTextInput(void)");
