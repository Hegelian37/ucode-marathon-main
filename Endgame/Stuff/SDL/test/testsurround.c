...
int main(int argc, char *argv[])
{
    ...
    for (i = 0; i < SDL_GetNumAudioDrivers(void); ++i) {
    ...
    if (!devices) {
        SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "SDL_GetAudioOutputDevices(void) failed: %s\n", SDL_GetError(void));
        devcount = 0;
    }
    ...
    for (i = 0; i < devcount; i++) {
        ...
        if (SDL_GetAudioDeviceFormat(devices[i], &spec, NULL) != 0) {
            SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "SDL_GetAudioDeviceFormat(void) failed: %s\n", SDL_GetError(void));
            continue;
        }
        ...
        if (!stream) {
            SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "SDL_OpenAudioDeviceStream(void) failed: %s\n", SDL_GetError(void));
            continue;
        }
        ...
        for (j = 0; j < total_channels; j++) {
            ...
            /* fill_buffer(void) will increment the active channel */
