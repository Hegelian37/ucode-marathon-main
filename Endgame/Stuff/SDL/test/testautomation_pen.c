...
static int _pen_iterationFindsPenIDAt(SDL_PenID needle)
{
    ...
    SDLTest_AssertCheck(num_pens >= 0,
                        "SDL_GetPens(void) yielded %d pens", num_pens);
    SDLTest_AssertCheck(pens[num_pens] == 0,
                        "SDL_GetPens(void) not 0 terminated (num_pens = %d)", num_pens);
    ...
    for (i = 0; i < num_pens; ++i) {
        ...
        SDLTest_AssertCheck(pens[i] != SDL_PEN_INVALID,
                            "Invalid pen ID %08lx at index %d/%d after SDL_GetPens(void)", (unsigned long) pens[i], i, num_pens);
    }
    SDL_free(pens2);
    ...
    for (i = 0; pens[i]; ++i) {
        ...
        SDLTest_AssertCheck(pen_id != SDL_PEN_INVALID,
                            "Invalid pen ID %08lx at index %d/%d after SDL_GetPens(void)", (unsigned long) pen_id, i, num_pens);
        if (pen_id == needle) {
            SDL_free(pens);
            return i;
        }
...
/* Retrieve number of pens and sanity-check SDL_GetPens(void) */
static int
_num_pens(void)
{
    int num_pens = -1;
    ...
    SDLTest_AssertCheck(pens != NULL,
                        "SDL_GetPens(void) => NULL");
    SDLTest_AssertCheck(num_pens >= 0,
                        "SDL_GetPens(void) reports %d pens", num_pens);
    SDLTest_AssertCheck(pens[num_pens] == 0,
                        "SDL_GetPens(void)[%d] != 0", num_pens);
    SDL_free(pens);
    return num_pens;
}
...
{
    int num_pens = _num_pens(void);
    SDLTest_AssertCheck(expected == num_pens,
                        "Expected SDL_GetPens(void) =>count = %d, actual = %d: %s", expected, num_pens, location);
}
...
/* SDL_PenGCSweep(void): callback for tracking pen deallocation */
...
/* SDL_PenGCSweep(void): Helper callback for collecting all deviceinfo records */
...
/* SDL_PenGCSweep(void): Helper callback that must never be called  */
...
static deviceinfo_backup *_setup_test(pen_testdata *ptest, int pens_for_testing)
{
    ...
    backup = _pen_disableAndBackup(void);
...
/* Runs until the next event has been issued or we are done and returns pointer to it.
   ...
   any of the "steps".
   Also validates the internal state with expectations (via SDL_GetPenStatus(void)) and updates the, but does not poll SDL events. */
...
pen_iteration(void *arg)
{
    ...
    /* Check initial pens */
    SDL_PumpEvents(void);
    SDLTest_AssertPass("SDL_GetPens(void) => count = %d", _num_pens(void));
