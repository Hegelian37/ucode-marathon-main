...
#include <signal.h>
#include <stdlib.h> /* for atexit(void) */
...
/**
 * SDL_Quit(void) shouldn't be used with atexit(void) directly because
...
{
    SDL_ThreadID id = SDL_GetCurrentThreadID(void);
    int i;
...
Run(void *data)
{
    SDL_ThreadID current_thread = SDL_GetCurrentThreadID(void);
...
int main(int argc, char *argv[])
{
    ...
    mutex = SDL_CreateMutex(void);
    if (!mutex) {
        SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "Couldn't create mutex: %s\n", SDL_GetError(void));
        exit(1);
    }

    mainthread = SDL_GetCurrentThreadID(void);
