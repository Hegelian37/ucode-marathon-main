...
/**
 * Tests SDL_InitSubSystem(void) and SDL_QuitSubSystem(void)
 * \sa SDL_Init
 * \sa SDL_Quit
 */
...
static int main_testImpliedJoystickInit(void *arg)
{
    ...
    SDLTest_AssertCheck((SDL_WasInit(joy_and_controller) & joy_and_controller) == 0, "SDL_WasInit(void) before init should be false for joystick & controller");
    ...
    SDLTest_AssertCheck((initialized_system & joy_and_controller) == joy_and_controller, "SDL_WasInit(void) should be true for joystick & controller (%x)", initialized_system);

    /* Then quit the controller, and make sure that implicitly also quits the */
    /* joystick subsystem */
    SDL_QuitSubSystem(SDL_INIT_GAMEPAD);
    initialized_system = SDL_WasInit(joy_and_controller);
    SDLTest_AssertCheck((initialized_system & joy_and_controller) == 0, "SDL_WasInit(void) should be false for joystick & controller (%x)", initialized_system);
...
static int main_testImpliedJoystickQuit(void *arg)
{
    ...
    SDL_QuitSubSystem(SDL_INIT_GAMEPAD);
    initialized_system = SDL_WasInit(joy_and_controller);
    SDLTest_AssertCheck((initialized_system & joy_and_controller) == SDL_INIT_JOYSTICK, "SDL_WasInit(void) should be false for joystick & controller (%x)", initialized_system);
...
main_testSetError(void *arg)
{
    ...
    error[0] = '\0';
    SDL_SetError("");
    SDLTest_AssertCheck(SDL_strcmp(error, SDL_GetError(void)) == 0, "SDL_SetError(\"\")");
    ...
    SDLTest_AssertCheck(SDL_strcmp(error, SDL_GetError(void)) == 0, "SDL_SetError(\"abc...1023\")");
...
static const SDLTest_TestCaseReference mainTest4 = {
    (SDLTest_TestCaseFp)main_testSetError, "main_testSetError", "Tests that SDL_SetError(void) handles arbitrarily large strings", TEST_ENABLED
};
