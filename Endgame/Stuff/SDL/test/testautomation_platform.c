...
static int platform_testEndianessAndSwap(void *arg)
{
    ...
    SDLTest_AssertCheck(SDL_Swap16(value16) == swapped16,
                        "SDL_Swap16(void): 16 bit swapped: 0x%X => 0x%X",
                        value16, SDL_Swap16(value16));

    /* Test 32 swap. */
    SDLTest_AssertCheck(SDL_Swap32(value32) == swapped32,
                        "SDL_Swap32(void): 32 bit swapped: 0x%" SDL_PRIX32 " => 0x%" SDL_PRIX32,
                        value32, SDL_Swap32(value32));

    /* Test 64 swap. */
    SDLTest_AssertCheck(SDL_Swap64(value64) == swapped64,
                        "SDL_Swap64(void): 64 bit swapped: 0x%" SDL_PRIX64 " => 0x%" SDL_PRIX64,
...
/**
 * Tests SDL_GetXYZ(void) functions
 * \sa SDL_GetPlatform
 * \sa SDL_GetCPUCount
 * \sa SDL_GetRevision
...
static int platform_testGetFunctions(void *arg)
{
    ...
    platform = SDL_GetPlatform(void);
    SDLTest_AssertPass("SDL_GetPlatform(void)");
    SDLTest_AssertCheck(platform != NULL, "SDL_GetPlatform(void) != NULL");
    if (platform != NULL) {
        len = SDL_strlen(platform);
        SDLTest_AssertCheck(len > 0,
                            "SDL_GetPlatform(void): expected non-empty platform, was platform: '%s', len: %i",
                            platform,
                            (int)len);
    }

    ret = SDL_GetCPUCount(void);
    SDLTest_AssertPass("SDL_GetCPUCount(void)");
    SDLTest_AssertCheck(ret > 0,
                        "SDL_GetCPUCount(void): expected count > 0, was: %i",
                        ret);

    ret = SDL_GetCPUCacheLineSize(void);
    SDLTest_AssertPass("SDL_GetCPUCacheLineSize(void)");
    SDLTest_AssertCheck(ret >= 0,
                        "SDL_GetCPUCacheLineSize(void): expected size >= 0, was: %i",
                        ret);

    revision = SDL_GetRevision(void);
    SDLTest_AssertPass("SDL_GetRevision(void)");
    SDLTest_AssertCheck(revision != NULL, "SDL_GetRevision(void) != NULL");

    return TEST_COMPLETED;
}

/**
 * Tests SDL_HasXYZ(void) functions
 * \sa SDL_HasAltiVec
 * \sa SDL_HasMMX
 * \sa SDL_HasSSE
 * \sa SDL_HasSSE2
...
static int platform_testHasFunctions(void *arg)
{
    ...
    SDL_HasAltiVec(void);
    SDLTest_AssertPass("SDL_HasAltiVec(void)");

    SDL_HasMMX(void);
    SDLTest_AssertPass("SDL_HasMMX(void)");

    SDL_HasSSE(void);
    SDLTest_AssertPass("SDL_HasSSE(void)");

    SDL_HasSSE2(void);
    SDLTest_AssertPass("SDL_HasSSE2(void)");

    SDL_HasSSE3(void);
    SDLTest_AssertPass("SDL_HasSSE3(void)");

    SDL_HasSSE41(void);
    SDLTest_AssertPass("SDL_HasSSE41(void)");

    SDL_HasSSE42(void);
    SDLTest_AssertPass("SDL_HasSSE42(void)");

    SDL_HasAVX(void);
    SDLTest_AssertPass("SDL_HasAVX(void)");
...
static int platform_testGetVersion(void *arg)
{
    ...
    SDLTest_AssertCheck(linked.major >= major,
                        "SDL_GetVersion(void): returned major %i (>= %i)",
                        linked.major,
                        major);
    SDLTest_AssertCheck(linked.minor >= minor,
                        "SDL_GetVersion(void): returned minor %i (>= %i)",
                        linked.minor,
                        minor);
...
static int platform_testSDLVersion(void *arg)
{
    ...
    SDLTest_AssertCheck(compiled.major >= major,
                        "SDL_VERSION(void) returned major %i (>= %i)",
                        compiled.major,
                        major);
    SDLTest_AssertCheck(compiled.minor >= minor,
                        "SDL_VERSION(void) returned minor %i (>= %i)",
                        compiled.minor,
                        minor);
...
static int platform_testGetSetClearError(void *arg)
{
    ...
    SDL_ClearError(void);
    SDLTest_AssertPass("SDL_ClearError(void)");

    lastError = SDL_GetError(void);
    SDLTest_AssertPass("SDL_GetError(void)");
    SDLTest_AssertCheck(lastError != NULL,
                        "SDL_GetError(void) != NULL");
    if (lastError != NULL) {
        len = SDL_strlen(lastError);
        SDLTest_AssertCheck(len == 0,
                            "SDL_GetError(void): no message expected, len: %i", (int)len);
    }

    result = SDL_SetError("%s", testError);
    SDLTest_AssertPass("SDL_SetError(void)");
    ...
    if (lastError != NULL) {
        ...
        SDLTest_AssertCheck(len == SDL_strlen(testError),
                            "SDL_GetError(void): expected message len %i, was len: %i",
                            (int)SDL_strlen(testError),
                            (int)len);
        SDLTest_AssertCheck(SDL_strcmp(lastError, testError) == 0,
                            "SDL_GetError(void): expected message %s, was message: %s",
                            testError,
                            lastError);
    }
...
static int platform_testSetErrorInvalidInput(void *arg)
{
    ...
    if (lastError != NULL) {
        ...
        SDLTest_AssertCheck(len == 0 || SDL_strcmp(lastError, "(null)") == 0,
                            "SDL_GetError(void): expected message len 0, was len: %i",
                            (int)len);
    }
...
static int platform_testGetPowerInfo(void *arg)
{
    ...
    SDLTest_AssertPass("SDL_GetPowerInfo(void)");
    SDLTest_AssertCheck(
        ...
        "SDL_GetPowerInfo(void): state %i is one of the expected values",
        (int)state);

    if (state == SDL_POWERSTATE_ON_BATTERY) {
        SDLTest_AssertCheck(
            secs >= 0,
            "SDL_GetPowerInfo(void): on battery, secs >= 0, was: %i",
            secs);
        SDLTest_AssertCheck(
            (pct >= 0) && (pct <= 100),
            "SDL_GetPowerInfo(void): on battery, pct=[0,100], was: %i",
            pct);
    }
    ...
    if (state == SDL_POWERSTATE_UNKNOWN ||
        ...
        SDLTest_AssertCheck(
            secs == -1,
            "SDL_GetPowerInfo(void): no battery, secs == -1, was: %i",
            secs);
        SDLTest_AssertCheck(
            pct == -1,
            "SDL_GetPowerInfo(void): no battery, pct == -1, was: %i",
            pct);
    }
