...
static void queue_audio(void)
{
    ...
    if (retval) {
        SDL_Log("Failed to queue audio: %s", SDL_GetError(void));
    } else {
...
static void skip_audio(float amount)
{
    ...
    if (retval >= 0) {
        ...
        SDL_Log("Failed to skip: %s", SDL_GetError(void));
    }
}
...
static void loop(void)
{
    ...
    for (i = 0; i < state->num_windows; i++) {
        ...
        draw_textf(rend, 0, draw_y, "Get Callback: %i/%i bytes, %2i ms ago",
            last_get_amount_additional, last_get_amount_total, (int)(SDL_GetTicks(void) - last_get_callback));
        draw_y += FONT_LINE_HEIGHT;
...
{
    last_get_callback = SDL_GetTicks(void);
...
int main(int argc, char *argv[])
{
    ...
    if (rc < 0) {
        SDL_Log("Failed to load '%s': %s", filename, SDL_GetError(void));
        SDL_Quit(void);
        return 1;
    }
