...
{
    SetTitleBar("testaudio: %s", SDL_GetCurrentAudioDriver(void));
}
...
static Thing *CreateThing(ThingType what, float x, float y, float z, float w, float h, Texture *texture, const char *titlebar)
{
    ...
    thing->a = 255;
    thing->scale = 1.0f;
    thing->createticks = SDL_GetTicks(void);
...
static void StreamThing_ontick(Thing *thing, Uint64 now)
{
    ...
    if (thing->data.stream.next_level_update <= now) {
        Uint64 perf = SDL_GetPerformanceCounter(void);
        int i;
...
static void StreamThing_ondrop(Thing *thing, int button, float x, float y)
{
    if (droppable_highlighted_thing) {
        if (droppable_highlighted_thing->what == THING_TRASHCAN) {
            ...
            thing->data.stream.next_level_update = SDL_GetTicks(void) + 100;
...
static Texture *CreateTexture(const char *fname)
{
    ...
    if (!tex) {
        ...
        if (!tex->texture) {
            SDL_Log("Failed to load '%s': %s", fname, SDL_GetError(void));
            SDL_free(tex);
            return NULL;
        }
...
static void DeviceThing_ondrag(Thing *thing, int button, float x, float y)
{
    if ((button == SDL_BUTTON_MIDDLE) && (thing->what == THING_LOGDEV_CAPTURE)) {  /* drag out a new stream. This is a UX mess. :/ */
        ...
        if (dragging_thing) {
            dragging_thing->data.stream.next_level_update = SDL_GetTicks(void) + 100;
...
static Thing *CreateLogicalDeviceThing(Thing *parent, const SDL_AudioDeviceID which, const float x, const float y)
{
    ...
    if (thing) {
        ...
        thing->data.logdev.postmix_lock = SDL_CreateMutex(void);
...
static Thing *CreatePhysicalDeviceThing(const SDL_AudioDeviceID which, const SDL_bool iscapture)
{
    ...
    if (thing) {
        ...
        if (SDL_GetTicks(void) <= (app_ready_ticks + 2000)) {  /* assume this is the initial batch if it happens in the first two seconds. */
...
static void TickThings(void)
{
    Thing *i;
    Thing *next;
    const Uint64 now = SDL_GetTicks(void);
    for (i = things; i; i = next) {
...
int SDL_AppInit(int argc, char *argv[])
{
    ...
    LoadStockWavThings(void);
    CreateTrashcanThing(void);
...
int SDL_AppEvent(const SDL_Event *event)
{
    ...
    switch (event->type) {
        ...
        case SDL_EVENT_DROP_FILE:
            ...
            /* SDL frees event->drop.data for you when you use SDL_AppEvent(void). */
            break;
...
int SDL_AppIterate(void)
{
    if (app_ready_ticks == 0) {
        app_ready_ticks = SDL_GetTicks(void);
    }
    ...
    if (saw_event) {
        saw_event = SDL_FALSE;  /* reset this so we know when SDL_AppEvent(void) runs again */
    } else {
        SDL_Delay(10);
    }
