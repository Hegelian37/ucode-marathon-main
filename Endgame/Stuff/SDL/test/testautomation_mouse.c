...
static int mouse_getMouseState(void *arg)
{
    ...
    SDL_PumpEvents(void);
    SDLTest_AssertPass("Call to SDL_PumpEvents(void)");
...
static int mouse_createFreeCursor(void *arg)
{
    ...
    SDLTest_AssertPass("Call to SDL_CreateCursor(void)");
    SDLTest_AssertCheck(cursor != NULL, "Validate result from SDL_CreateCursor(void) is not NULL");
    if (cursor == NULL) {
        return TEST_ABORTED;
    }
    ...
    SDLTest_AssertPass("Call to SDL_DestroyCursor(void)");
...
static int mouse_createFreeColorCursor(void *arg)
{
    ...
    SDLTest_AssertPass("Call to SDL_CreateColorCursor(void)");
    SDLTest_AssertCheck(cursor != NULL, "Validate result from SDL_CreateColorCursor(void) is not NULL");
    if (cursor == NULL) {
...
static void changeCursorVisibility(SDL_bool state)
{
    ...
    SDLTest_AssertPass("Call to %s", state ? "SDL_ShowCursor(void)" : "SDL_HideCursor(void)");

    newState = SDL_CursorVisible(void);
    SDLTest_AssertPass("Call to SDL_CursorVisible(void)");
...
static int mouse_showCursor(void *arg)
{
    ...
    /* Get current state */
    currentState = SDL_CursorVisible(void);
    SDLTest_AssertPass("Call to SDL_CursorVisible(void)");
    if (currentState) {
...
static int mouse_getCursor(void *arg)
{
    ...
    /* Get current cursor */
    cursor = SDL_GetCursor(void);
    SDLTest_AssertPass("Call to SDL_GetCursor(void)");
    SDLTest_AssertCheck(cursor != NULL, "Validate result from SDL_GetCursor(void) is not NULL");
...
static int mouse_getSetRelativeMouseMode(void *arg)
{
    ...
    initialState = SDL_GetRelativeMouseMode(void);
    SDLTest_AssertPass("Call to SDL_GetRelativeMouseMode(void)");
    ...
    for (i = 0; i < 2; i++) {
        ...
        currentState = SDL_GetRelativeMouseMode(void);
        SDLTest_AssertPass("Call to SDL_GetRelativeMouseMode(void)");
...
static SDL_Window *createMouseSuiteTestWindow(void)
{
    ...
    SDLTest_AssertPass("SDL_CreateWindow(void)");
...
static void destroyMouseSuiteTestWindow(SDL_Window *window)
{
    if (window) {
        ...
        window = NULL;
        SDLTest_AssertPass("SDL_DestroyWindow(void)");
    }
}
...
static int mouse_warpMouseInWindow(void *arg)
{
    ...
    /* Create test window */
    window = createMouseSuiteTestWindow(void);
    if (!window) {
        return TEST_ABORTED;
    }
    ...
    for (i = 0; i < numPositions; i++) {
        for (j = 0; j < numPositions; j++) {
            ...
            SDL_PumpEvents(void);
            SDLTest_AssertPass("SDL_PumpEvents(void)");
        }
    }
...
static int mouse_getMouseFocus(void *arg)
{
    ...
    const SDL_bool video_driver_is_wayland = !SDL_strcmp(SDL_GetCurrentVideoDriver(void), "wayland");

    /* Get focus - focus non-deterministic */
    focusWindow = SDL_GetMouseFocus(void);
    SDLTest_AssertPass("SDL_GetMouseFocus(void)");
...
static int mouse_getDefaultCursor(void *arg)
{
    ...
    /* Get current cursor */
    cursor = SDL_GetDefaultCursor(void);
    SDLTest_AssertPass("Call to SDL_GetDefaultCursor(void)");
    SDLTest_AssertCheck(cursor != NULL, "Validate result from SDL_GetDefaultCursor(void) is not NULL");
...
static int mouse_getGlobalMouseState(void *arg)
{
    ...
    SDLTest_AssertPass("Call to SDL_GetGlobalMouseState(void)");
