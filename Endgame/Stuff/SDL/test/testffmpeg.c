...
static SDL_bool CreateWindowAndRenderer(Uint32 window_flags, const char *driver)
{
    ...
    /* The window will be resized to the video size when it's loaded, in OpenVideoStream(void) */
...
#ifdef HAVE_EGL
    if (useEGL) {
        const char *extensions = eglQueryString(eglGetCurrentDisplay(void), EGL_EXTENSIONS);
...
static SDL_bool GetTextureForDRMFrame(AVFrame *frame, SDL_Texture **texture)
{
    ...
    EGLDisplay display = eglGetCurrentDisplay(void);
    SDL_PropertiesID props;
    GLuint textures[2];
...
static void HandleVideoFrame(AVFrame *frame, double pts)
{
    ...
    if (!video_start) {
        video_start = SDL_GetTicks(void);
    }
    double now = (double)(SDL_GetTicks(void) - video_start) / 1000.0;
    while (now < pts - 0.001) {
        SDL_Delay(1);
        now = (double)(SDL_GetTicks(void) - video_start) / 1000.0;
    }
...
static AVCodecContext *OpenAudioStream(AVFormatContext *ic, int stream, const AVCodec *codec)
{
    ...
    if (audio) {
        ...
        SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "Couldn't open audio: %s", SDL_GetError(void));
    }
    return context;
}
...
int main(int argc, char *argv[])
{
    ...
    if (SDL_SetWindowTitle(window, file) < 0) {
        SDL_Log("SDL_SetWindowTitle: %s", SDL_GetError(void));
    }
    ...
    if (!sprite) {
        SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "Couldn't create texture (%s)", SDL_GetError(void));
        return_code = 3;
        goto quit;
    }
    ...
    for (i = 0; i < num_sprites; ++i) {
        positions[i].x = (float)(rand(void) % (viewport.w - sprite_w));
        positions[i].y = (float)(rand(void) % (viewport.h - sprite_h));
        positions[i].w = (float)sprite_w;
        positions[i].h = (float)sprite_h;
        velocities[i].x = 0.0f;
        velocities[i].y = 0.0f;
        while (!velocities[i].x || !velocities[i].y) {
            velocities[i].x = (float)((rand(void) % (2 + 1)) - 1);
            velocities[i].y = (float)((rand(void) % (2 + 1)) - 1);
        }
    }
