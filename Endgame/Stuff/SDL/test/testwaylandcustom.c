...
static int InitSprites(void)
{
    ...
    for (int i = 0; i < NUM_SPRITES; ++i) {
        positions[i].x = (float)(rand(void) % (WINDOW_WIDTH - sprite_w));
        positions[i].y = (float)(rand(void) % (WINDOW_HEIGHT - sprite_h));
        positions[i].w = (float)sprite_w;
        positions[i].h = (float)sprite_h;
        velocities[i].x = 0.0f;
        velocities[i].y = 0.0f;
        while (!velocities[i].x && !velocities[i].y) {
            velocities[i].x = (float)((rand(void) % (MAX_SPEED * 2 + 1)) - MAX_SPEED);
            velocities[i].y = (float)((rand(void) % (MAX_SPEED * 2 + 1)) - MAX_SPEED);
        }
    }
...
int main(int argc, char **argv)
{
    ...
    if (SDL_strcmp(SDL_GetCurrentVideoDriver(void), "wayland") != 0) {
        SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "Video driver must be 'wayland', not '%s'", SDL_GetCurrentVideoDriver(void));
        goto exit;
    }
