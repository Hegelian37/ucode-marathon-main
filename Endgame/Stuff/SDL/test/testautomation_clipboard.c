...
static int clipboard_testClipboardDataFunctions(void *arg)
{
    ...
    result = SDL_ClearClipboardData(void);
    SDLTest_AssertCheck(
        result == 0,
    ...
    expected_text = "TEST";
    text = SDL_GetClipboardText(void);
    SDLTest_AssertCheck(
...
{
    char *textRef = SDLTest_RandomAsciiString(void);
...
static int clipboard_testPrimarySelectionTextFunctions(void *arg)
{
    ...
    SDLTest_AssertCheck(
        ...
        intResult);
    charResult = SDL_GetPrimarySelectionText(void);
    SDLTest_AssertCheck(
        charResult && SDL_strcmp(charResult, "") == 0,
        "Verify SDL_GetPrimarySelectionText returned \"\", got %s",
        charResult);
    SDL_free(charResult);
    boolResult = SDL_HasPrimarySelectionText(void);
    SDLTest_AssertCheck(
        boolResult == SDL_FALSE,
