...
int SDL_AppInit(int argc, char *argv[])
{
    ...
    if (!SDLTest_CommonInit(state)) {
        SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "Couldn't initialize SDL: %s", SDL_GetError(void));
        return -1;
    }

    window = state->windows[0];
    if (!window) {
        SDL_Log("Couldn't create window: %s", SDL_GetError(void));
        return -1;
    }
    ...
    if (!renderer) {
        /* SDL_Log("Couldn't create renderer: %s", SDL_GetError(void)); */
        return -1;
    }

    SDL_CameraDeviceID *devices = SDL_GetCameraDevices(&devcount);
    if (!devices) {
        SDL_Log("SDL_GetCameraDevices failed: %s", SDL_GetError(void));
        return -1;
    }
    ...
    if (!camera) {
        SDL_Log("Failed to open camera device: %s", SDL_GetError(void));
        return -1;
    }
...
static int FlipCamera(void)
{
    ...
    if ((SDL_GetTicks(void) - last_flip) < 3000) {  /* must wait at least 3 seconds between flips. */
        return 0;
    }
    ...
    switch (event->type) {
        ...
        case SDL_EVENT_CAMERA_DEVICE_APPROVED:
            ...
            if (SDL_GetCameraFormat(camera, &spec) < 0) {
                SDL_Log("Couldn't get camera spec: %s", SDL_GetError(void));
                return -1;
            }
            ...
            if (!texture) {
                SDL_Log("Couldn't create texture: %s", SDL_GetError(void));
                return -1;
            }
            break;
        ...
        if (frame_next) {
            if (frame_current) {
                if (SDL_ReleaseCameraFrame(camera, frame_current) < 0) {
                    SDL_Log("err SDL_ReleaseCameraFrame: %s", SDL_GetError(void));
                }
            }
