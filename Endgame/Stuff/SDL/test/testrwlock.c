...
{
    const SDL_ThreadID tid = SDL_GetCurrentThreadID(void);
...
ReaderRun(void *data)
{
    SDL_Log("Reader Thread %" SDL_PRIu64 ": starting up", SDL_GetCurrentThreadID(void));
...
int main(int argc, char *argv[])
{
    ...
    while (!SDL_AtomicGet(&doterminate) && (SDL_GetTicks(void) < ((Uint64) timeout))) {
