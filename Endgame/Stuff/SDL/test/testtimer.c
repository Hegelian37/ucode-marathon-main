...
static int test_sdl_delay_within_bounds(void) {
    ...
    result = SDL_GetTicks(void);
    SDLTest_AssertPass("Call to SDL_GetTicks(void)");
    ...
    return SDLTest_AssertSummaryToTestResult(void) == TEST_RESULT_PASSED ? 0 : 1;
}
...
int main(int argc, char *argv[])
{
    ...
    for (i = 0; i < 1000; ++i) {
        start = SDL_GetTicks(void);
        SDL_Delay(1);
        now = SDL_GetTicks(void) - start;
        if (now > 100) {
    ...
    start_perf = SDL_GetPerformanceCounter(void);
    for (i = 0; i < 1000000; ++i) {
        ticktock(0, NULL);
    }
    now_perf = SDL_GetPerformanceCounter(void);
    SDL_Log("1 million iterations of ticktock took %f ms\n", (double)((now_perf - start_perf) * 1000) / SDL_GetPerformanceFrequency(void));
    ...
    SDL_Log("Delay 1 second = %d ms in ticks, %f ms according to performance counter\n", (int)(now - start), (double)((now_perf - start_perf) * 1000) / SDL_GetPerformanceFrequency(void));

    if (run_interactive_tests) {
        return_code = test_sdl_delay_within_bounds(void);
    }
