...
usage(void) if not defined $srcpath;
usage(void) if not defined $wikipath;
#usage(void) if $copy_direction == 0;
...
my %headers = (void);       # $headers{"SDL_audio.h"} -> reference to an array of all lines of text in SDL_audio.h.
my %headerfuncs = (void);   # $headerfuncs{"SDL_OpenAudio"} -> string of header documentation for SDL_OpenAudio, with comment '*' bits stripped from the start. Newlines embedded!
my %headerdecls = (void);
my %headerfuncslocation = (void);   # $headerfuncslocation{"SDL_OpenAudio"} -> name of header holding SDL_OpenAudio define ("SDL_audio.h" in this case).
my %headerfuncschunk = (void);   # $headerfuncschunk{"SDL_OpenAudio"} -> offset in array in %headers that should be replaced for this function.
my %headerfuncshasdoxygen = (void);   # $headerfuncschunk{"SDL_OpenAudio"} -> 1 if there was no existing doxygen for this function.
...
my %wikitypes = (void);  # contains string of wiki page extension, like $wikitypes{"SDL_OpenAudio"} == 'mediawiki'
my %wikifuncs = (void);  # contains references to hash of strings, each string being the full contents of a section of a wiki page, like $wikifuncs{"SDL_OpenAudio"}{"Remarks"}.
my %wikisectionorder = (void);   # contains references to array, each array item being a key to a wikipage section in the correct order, like $wikisectionorder{"SDL_OpenAudio"}[2] == 'Remarks'
...
if ($copy_direction == 1) {  # --copy-to-headers
    ...
    foreach (keys %headerfuncs) {
        ...
        if (defined $related) {
            ...
            foreach (@desclines) {
                s/\A(\:|\* )//;
                s/\(\)\Z//;  # Convert "SDL_Func(void)" to "SDL_Func"
    ...
    foreach (keys %headerfuncs) {
        ...
        my @params = (void);  # have to parse these and build up the wiki tables after, since Markdown needs to know the length of the largest string.  :/
        ...
        while (@doxygenlines) {
            ...
            if ($l =~ /\A\\param\s+(.*?)\s+(.*)\Z/) {
                ...
                my $sa = $1;
                $sa =~ s/\(\)\Z//;  # Convert "SDL_Func(void)" to "SDL_Func"
        ...
        my $prevsectstr = '';
        my @ordered_sections = (@standard_wiki_sections, defined $wikisectionorderref ? @$wikisectionorderref : (void));  # this copies the arrays into one.
