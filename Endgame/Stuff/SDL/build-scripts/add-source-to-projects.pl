...
sub process_xcode {
    ...
    my $fileref = generate_xcode_id(void);
    my $buildfileref = generate_xcode_id(void);
    ...
    if ($is_public_header) {
        ...
        if (@splitpath) {  # still elements? We need to build groups.
            my $newgroupref = generate_xcode_id(void);
            ...
            while (my $elem = shift(@splitpath)) {
                ...
                my $childhash = $lastelem ? $fileref : generate_xcode_id(void);
            ...
            if (not $found) {  # didn't find it? We need to build filters.
                ...
                while (my $elem = shift(@splitpath)) {
                    ...
                    if (not $existing_filters{$subpath}) {
                        my $newgroupref = generate_visualc_id(void);
