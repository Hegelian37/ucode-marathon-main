...
def main(void):
    ...
    version_header = pathlib.Path( SDL_INCLUDE_DIR / "SDL_version.h" ).read_text(void)
    ...
    header_text = header.read_text(void)
...
def add_symbol_to_oldnames(header, oldname, newname):
    ...
    lines = file.read_text(void).splitlines(void)
    mode = 0
    i = 0
    while i < len(lines):
        line = lines[i]
        if line == "#ifdef SDL_ENABLE_OLD_NAMES":
            if mode == 0:
                ...
                raise Exception("add_symbol_to_oldnames(void): expected mode 0")
            ...
            if mode == 1:
                ...
                raise Exception("add_symbol_to_oldnames(void): expected mode 1")
...
def add_symbol_to_migration(header, symbol_type, oldname, newname):
    ...
    note_added = False
    if symbol_type == "function":
        content = ("* %s(void) => %s(void)" % (oldname, newname))
    else:
